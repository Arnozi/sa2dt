# 0 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c"
...
# 2 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2
...
void VramFree(void *);
# 3 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 4 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2
...
# 5 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2
...
# 6 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2
...
# 8 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2
...
# 6 "../../../include/game/interactables_2/collect_rings_lap_trigger.h" 2

void CreateEntity_CollectRingsLapTrigger(MapEntity *, u16, u16, u8);
# 9 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2


# 1 "../../../include/task.h" 1
# 12 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2
...
# 14 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2
# 1 "../../../include/constants/songs.h" 1
# 15 "build/collect_rings/src/sa2/game/interactables_2/collect_rings_lap_trigger.c" 2
...
static void HandleTrigger(Sprite_LapTrigger __attribute__((unused)) *s);
...
void CreateEntity_CollectRingsLapTrigger(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, __attribute__((unused)) u8 spriteY)
{
    struct Task *t = TaskCreate(Task_LapTriggerInactive, sizeof(Sprite_LapTrigger), 0x2010, 0, ((void *)0));
    Sprite_LapTrigger *trigger = (void *)(0x03000000 + ((t)->data));
    ...
    trigger->posX = ((me->x)*8 + (spriteRegionX)*256);
    trigger->posY = ((me->y)*8 + (spriteRegionY)*256);

    trigger->left = me->d.sData[0] * 8;
    trigger->top = me->d.sData[1] * 8;
    trigger->right = trigger->left + me->d.uData[2] * 8;
    trigger->bottom = trigger->top + me->d.uData[3] * 8;
...
{
    if ((trigger->lastPlayerX < ((s32)((trigger->posX + trigger->left)*256))) && (gPlayer.qWorldX > ((s32)((trigger->posX + trigger->right)*256)))) {
        if (trigger->passedLap) {
            ...
            if (trigger->highestLap < trigger->lap) {
                ...
                if (timeDiff > (int)(((0 * 60.) + 30) * 60)) {
                    ringBonus = 5;
                } else if (timeDiff > (int)(((0 * 60.) + 20) * 60)) {
                    ringBonus = 10;
                } else {
                    ringBonus = 15;
                }
                ...
                if (!((gCurrentLevel) == ((7)*(3 + 1)) + (1)) && (Div(gRingCount, 100) != Div(prevRingCount, 100))
                    && (gGameMode == 0)) {
    ...
    } else if ((trigger->lastPlayerX > ((s32)((trigger->posX + trigger->right)*256))) && (gPlayer.qWorldX < ((s32)((trigger->posX + trigger->left)*256)))) {
        if (trigger->passedLap) {
            trigger->lap--;
        }
...
static void UpdatePlayerPosition(Sprite_LapTrigger *trigger)
{
    if (trigger->passedLap) {
        ...
        if ((gPlayer.qWorldX < ((s32)((240)*256))) && (gPlayer.qWorldY < ((s32)((288)*256)))) {
            xValue = ((s32)((1440)*256));
            yValue = ((s32)((864)*256));
        } else if ((gPlayer.qWorldX > ((s32)((1680)*256))) && (gPlayer.qWorldY > ((s32)((864)*256)))) {
            xValue = ((s32)((-1440)*256));
            yValue = ((s32)((-864)*256));
        } else {
            return;
...
static void CreateSprite_Notif_RingBonus(void)
{
    ...
    Sprite_Notif_RingBonus *notif = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &notif->s;

    notif->framesVisible = (int)(((0 * 60.) + 2) * 60);

    s->oamFlags = ((1) << 6); s->graphics.size = 0; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;; s->frameFlags = (((1) << (12)) | (0));;
...
static void Task_LapTriggerInactive(void)
{
    Sprite_LapTrigger *trigger = (void *)(0x03000000 + ((gCurTask)->data));
...
static void HandleTrigger(Sprite_LapTrigger __attribute__((unused)) *s) { gCurTask->main = Task_LapTriggerActive; }

static void Task_NotifRingBonus(void)
{
    Sprite_Notif_RingBonus *notif = (void *)(0x03000000 + ((gCurTask)->data));
...
static void TaskDestructor_NotifRingBonus(struct Task *t)
{
    Sprite_Notif_RingBonus *trigger = (void *)(0x03000000 + ((t)->data));
