# 0 "build/collect_rings/src/sa2/game/stage/player_controls.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "build/collect_rings/src/sa2/game/stage/player_controls.c"
...
# 2 "build/collect_rings/src/sa2/game/stage/player_controls.c" 2
...
# 3 "build/collect_rings/src/sa2/game/stage/player_controls.c" 2
...
# 4 "build/collect_rings/src/sa2/game/stage/player_controls.c" 2

struct ButtonConfig gPlayerControls = {};

__attribute__((unused)) void sub_802B584(u16 jump, u16 attack, u16 trick)
{
    ...
    for (i = 0; i < 3; i++) {
        switch (config[i]) {
            case 0: {
                if (i == 2)
                    controls->jump = 0x0100;
                else
                    controls->jump = 0x0001 << i;
            } break;

            case 1: {
                if (i == 2)
                    controls->attack = 0x0100;
                else
                    controls->attack = 0x0001 << i;
            } break;

            case 2: {
                if (i == 2)
                    controls->trick = 0x0100;
                else
                    controls->trick = 0x0001 << i;
            } break;
        }
    }
