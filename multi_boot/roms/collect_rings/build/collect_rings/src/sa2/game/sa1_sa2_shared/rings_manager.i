# 0 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c"
...
char *strlwr (char *);
char *strupr (char *);
# 2 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 3 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
# 1 "../../../include/core.h" 1
...
# 4 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
void EwramFree(void *);
# 5 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
# 1 "../../../include/rect.h" 1
...
# 6 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 8 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 9 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
# 1 "../../../include/game/sa1_sa2_shared/magnetic_ring.h" 1



extern void CreateMagneticRing(s16 x, s16 y);
# 10 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 7 "../../../include/game/sa1_sa2_shared/rings_manager.h" 2
# 36 "../../../include/game/sa1_sa2_shared/rings_manager.h"
extern const u8 *const gSpritePosData_rings[1];

void CreateStageRingsManager(void);
# 11 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 13 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 14 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 16 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 18 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 20 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c" 2
...
# 88 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c"
void CreateStageRingsManager(void)
{
    struct Task *t;
    RingsManager *mgr;
    ...
    u32 dataSize;
# 109 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c"
    {
        ...
        compressedRingPosData = (u8 *)(*(u8 **)(0x02000000 + 0x3300C));
        dataSize = (*(u32 *)compressedRingPosData) >> 8;
        dataSize = (dataSize + 3) >> 2;
        dataSize <<= 4;
        ewramBuffer = (u32 *)((u8 *)(0x02000000 + 0x3F000) + dataSize);
        RLUnCompWram(((u8 **)(0x02000000 + 0x33008))[0], ewramBuffer);
    }

    mgrRings = (void *)(0x03000000 + ((t)->data)) + ((size_t)&((RingsManager *)0)->rings);
    *mgrRings = ewramBuffer;

    s = (void *)(0x03000000 + ((t)->data));

    s->x = 0;
    s->y = 0;

    s->graphics.dest = (void *)((u8*)(0x6000000 + 0x10000) + 0x2E40);
    s->oamFlags = ((20) << 6);
    s->graphics.size = 0;
    s->graphics.anim = 106;
    s->variant = 0;
...
__attribute__((naked)) void Task_RingsMgrMain(void) { asm(".include " "\"asm/non_matching/game/sa1_sa2_shared/Task_RingsMgrMain_collect_rings.inc\""); if (0)

{
    ...
    if (!(gStageFlags & 0x0002)) {
        ...
        rings = *(u32 **)((void *)(0x03000000 + ((gCurTask)->data)) + ((size_t)&((RingsManager *)0)->rings));
# 186 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c"
        sp08 = 0;
# 204 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c"
        rings = *(u32 **)((void *)(0x03000000 + ((gCurTask)->data)) + ((size_t)&((RingsManager *)0)->rings));
        rm = (void *)(0x03000000 + ((gCurTask)->data));
        s = &rm->s;
        ...
        for (regionY = ({ s32 a = (((int)((p->qWorldY) >> 8))); s32 b = (rect[1]); ((a + b + 0) >> 8); });
             regionY <= ({ s32 a = (((int)((p->qWorldY) >> 8))); s32 b = (rect[3]); ((b + a + 8) >> 8); }) && regionY < v_regionCount; regionY++) {

            for (regionX = ({ s32 a = (((int)((p->qWorldX) >> 8))); s32 b = (rect[leftIndex]); ((a + b + -8) >> 8); });
                 regionX <= ({ s32 a = (((int)((p->qWorldX) >> 8))); s32 b = (rect[2]); ((b + a + 8 * 2) >> 8); }) && regionX < h_regionCount; regionX++) {
                ...
                if (offset) {
                    ...
                    while (meRing->x != (u8)(-1)) {
                        if (meRing->x != (u8)(-2)) {
                            ...
                            if (sp08 != 0
                                || (gCurrentLevel != ((7)*(3 + 1)) + (1) && !(p->moveState & 0x00000080))) {
                                if (((((rx - 8) <= (((((int)((p->qWorldX) >> 8)))) + ((Rect8 *)rect)->left) && (rx + 8) >= (((((int)((p->qWorldX) >> 8)))) + ((Rect8 *)rect)->left)) || ((rx - 8) >= (((((int)((p->qWorldX) >> 8)))) + ((Rect8 *)rect)->left) && ((((((int)((p->qWorldX) >> 8)))) + ((Rect8 *)rect)->left) + (((Rect8 *)rect)->right - ((Rect8 *)rect)->left)) >= (rx - 8))) && ((((ry - (8 * 2)) <= (((((int)((p->qWorldY) >> 8)))) + ((Rect8 *)rect)->top) && ry >= (((((int)((p->qWorldY) >> 8)))) + ((Rect8 *)rect)->top)) || ((ry - (8 * 2)) >= (((((int)((p->qWorldY) >> 8)))) + ((Rect8 *)rect)->top) && ((((((int)((p->qWorldY) >> 8)))) + ((Rect8 *)rect)->top) + (((Rect8 *)rect)->bottom - ((Rect8 *)rect)->top)) >= (ry - (8 * 2))))))) {
...

                                    {
                                        ...
                                        if (!(((gCurrentLevel) == ((7)*(3 + 1)) + (1)))) {
                                            ...
                                            if ((newLives != prevLives) && (gGameMode == 0)) {
                                                if (gNumLives < 255) {
                                                    gNumLives++;
                                                };
                                            }
        ...
        if ((!(((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2))))) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                if (i != id && gMultiplayerPlayerTasks[i] != ((void *)0)) {
                    MultiplayerPlayer *mpp = (void *)(0x03000000 + ((gMultiplayerPlayerTasks[i])->data));

                    for (regionY = ({ s32 a = (mpp->pos.y); s32 b = (mpp->s.hitboxes[0].top); ((a + b + 0) >> 8); });
                         regionY <= ({ s32 a = (mpp->pos.y); s32 b = (mpp->s.hitboxes[0].bottom); ((b + a + 8) >> 8); }) && regionY < v_regionCount; regionY++) {

                        for (regionX = ({ s32 a = (mpp->pos.x); s32 b = (mpp->s.hitboxes[0].left); ((a + b + -8) >> 8); });
                             regionX <= ({ s32 a = (mpp->pos.x); s32 b = (mpp->s.hitboxes[0].right); ((b + a + 8 * 2) >> 8); }) && regionX < h_regionCount;
                             regionX++) {
                            ...
                            if (offset != 0) {
                                ...
                                while (meRing->x != (u8)(-1)) {
                                    ...
                                    rx = ((meRing->x)*8 + (regionX)*256);
                                    ry = ((meRing->y)*8 + (regionY)*256);
                                    if (((((rx - 8) <= (((mpp->pos.x)) + (&mpp->s.hitboxes[0])->left) && (rx + 8) >= (((mpp->pos.x)) + (&mpp->s.hitboxes[0])->left)) || ((rx - 8) >= (((mpp->pos.x)) + (&mpp->s.hitboxes[0])->left) && ((((mpp->pos.x)) + (&mpp->s.hitboxes[0])->left) + ((&mpp->s.hitboxes[0])->right - (&mpp->s.hitboxes[0])->left)) >= (rx - 8))) && ((((ry - (8 * 2)) <= (((mpp->pos.y)) + (&mpp->s.hitboxes[0])->top) && ry >= (((mpp->pos.y)) + (&mpp->s.hitboxes[0])->top)) || ((ry - (8 * 2)) >= (((mpp->pos.y)) + (&mpp->s.hitboxes[0])->top) && ((((mpp->pos.y)) + (&mpp->s.hitboxes[0])->top) + ((&mpp->s.hitboxes[0])->bottom - (&mpp->s.hitboxes[0])->top)) >= (ry - (8 * 2))))))) {
                                        u8 pAnim = mpp->s.graphics.anim;
                                        u8 anims = gPlayerCharacterIdleAnims[gMultiplayerCharacters[mpp->unk56]];
                                        u8 anim = pAnim - anims;
                                        if ((anim != 28 && anim != 29) || !(mpp->unk54 & 0x4)) {
                                            CreateCollectRingEffect(rx, ry);
                                            meRing->x = (u8)(-2);
                                        }
                                    }
        ...
        regionY = ((gCamera.y) >> 8);
# 399 "build/collect_rings/src/sa2/game/sa1_sa2_shared/rings_manager.c"
        {
            for (; ((0)*8 + (regionY)*256) < gCamera.y + 160 && regionY < v_regionCount; regionY++) {

                while (0)
                    ;

                for (regionX = ((gCamera.x) >> 8); ((0)*8 + (regionX)*256) < gCamera.x + 240 && regionX < h_regionCount;
                     regionX++) {
                    u32 offset = (*((u32 *)((((u8 *)(rings)) + (((h_regionCount) * (regionY)) * (sizeof(u32)))) + ((regionX) * (sizeof(u32))))));
                    ...
                    if (offset != 0) {
                        ...
                        while (meRing->x != (u8)(-1)) {
                            ...
                            if (rx - gCamera.x < -8 || (rx - gCamera.x) + 8 > 240 + 2 * 8
                                || ry - gCamera.y < 0 || (ry - gCamera.y) - 2 * 8 > 160) {
                                meRing++;
                            } else {
                                meRing++;
                                ...
                                if ((drawCount == 0) || s->oamBaseIndex == 0xFF) {
                                    ...
                                    { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(oamDat); dmaRegs[1] = (vu32)(oamAllocated); dmaRegs[2] = (vu32)((u32)((0x8000 | 0x0000 | 0x0000 | 0x0000 | 0x0000) << 16 | ((sizeof(OamDataShort))/(16/8)))); dmaRegs[2]; };
