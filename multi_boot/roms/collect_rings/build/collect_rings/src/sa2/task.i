# 0 "build/collect_rings/src/sa2/task.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "build/collect_rings/src/sa2/task.c"
# 9 "build/collect_rings/src/sa2/task.c"
...
# 10 "build/collect_rings/src/sa2/task.c" 2
# 1 "../../../include/core.h" 1
...
# 11 "build/collect_rings/src/sa2/task.c" 2
# 1 "../../../include/task.h" 1
# 12 "build/collect_rings/src/sa2/task.c" 2
# 1 "../../../include/flags.h" 1
# 13 "build/collect_rings/src/sa2/task.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 14 "build/collect_rings/src/sa2/task.c" 2
...
u32 TasksInit(void)
{
    ...
    { vu32 tmp = (vu32)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)(gTasks); dmaRegs[2] = (vu32)((u32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((0x200)/(32/8)))); dmaRegs[2]; }; };
    ...
    if ((((void *)(0x03000000 + (cur->next))) == (void *)0x03000000)) {
        return 0;
    }

    ((struct Task *)(0x03000000 + (cur->next)))->prev = (TaskPtr32)cur;
    cur = (struct Task *)(0x03000000 + (cur->next));
...
# 93 "build/collect_rings/src/sa2/task.c"
struct Task *TaskCreate(TaskMain taskMain, u16 structSize, u16 priority, u16 flags, TaskDestructor taskDestructor)

{
    ...
    slow = gTaskPtrs[0];
    fast = slow->next;
    while (!(((void *)(0x03000000 + (fast))) == (void *)0x03000000)) {
        if (((struct Task *)(0x03000000 + (fast)))->priority > priority) {
            ((struct Task *)(0x03000000 + (fast)))->prev = (TaskPtr32)task;
            task->next = slow->next;
            task->prev = (TaskPtr32)slow;
            slow->next = (TaskPtr32)task;
            break;
        }
        slow = (struct Task *)(0x03000000 + (fast));
        fast = ((struct Task *)(0x03000000 + (fast)))->next;
    }
    return task;
}
...
void TaskDestroy(struct Task *task)
{
    ...
    if (!(task->flags & 0x0002)) {
        ...
        prev = (0x03000000 + (task->prev));
        next = (0x03000000 + (task->next));

        if (!(((struct Task *)prev) == (void *)0x03000000)) {
            if (!(((struct Task *)next) == (void *)0x03000000)) {
                if (task->dtor != ((void *)0)) {
                    task->dtor(task);
                }

                if (task == gNextTask) {
                    gNextTask = (struct Task *)(0x03000000 + ((task)->next));
                }
# 181 "build/collect_rings/src/sa2/task.c"
                prev = (0x03000000 + (task->prev));
                next = (0x03000000 + (task->next));
                ...
                if (task->data != (IwramData)((void *)0)) {
                    ...
                    IwramFree((void *)(0x03000000 + ((task)->data)));
                }
...
void TasksExec(void)
{
    ...
    if (!(gFlags & 0x800) && !((gTaskPtrs[0]) == (void *)0x03000000)) {
        while (!((gCurTask) == (void *)0x03000000)) {
            gNextTask = (struct Task *)(0x03000000 + (gCurTask->next));
    ...
    } else if (!((gTaskPtrs[0]) == (void *)0x03000000)) {
        while (!((gCurTask) == (void *)0x03000000)) {
            gNextTask = (struct Task *)(0x03000000 + (gCurTask->next));
...
{
# 261 "build/collect_rings/src/sa2/task.c"
    ...
    size = (size * sizeof(uintptr_t)) + ((size_t)&((struct IwramNode *)0)->space);
    ...
    while (1) {
        ...
        if (sizeSigned <= cur->state) {
            if (sizeSigned != cur->state) {
                s16 offset = size + ((size_t)&((struct IwramNode *)0)->space);
                if (offset > cur->state) {
                    if ((((void *)(0x03000000 + (cur->next))) == (void *)0x03000000)) {
                        return ((void *)0);
                    }
                    cur = (struct IwramNode *)(0x03000000 + (cur->next));
                    continue;
                }
        ...
        if ((((struct Task *)(0x03000000 + (cur->next))) == (void *)0x03000000)) {
            return ((void *)0);
        }
        cur = (struct IwramNode *)(0x03000000 + (cur->next));
    };
...
void IwramFree(void *p)
{
# 315 "build/collect_rings/src/sa2/task.c"
    ...
    node = (struct IwramNode *)(((u8 *)node) - ((size_t)&((struct IwramNode *)0)->space));
    slow = (struct IwramNode *)&gIwramHeap[0];
    fast = slow;
    if (node != slow) {
        do {
            slow = fast;
            fast = (struct IwramNode *)(0x03000000 + (slow->next));
        } while (node != fast);
    }
    if (node->state < 0) {
    ...
    if (fast == (struct IwramNode *)(0x03000000 + (node->next))) {
        if (fast->state > 0) {
...
static void __attribute__((unused)) sub_80028DC(void)
{
    ...
    while (!(((struct Task *)(0x03000000 + (cur->next))) == (void *)0x03000000)) {
        if (cur->state >= 0) {
            cur->next += 0;
            nextNodeOffset = cur->next;
            if (((struct IwramNode *)(0x03000000 + (cur->next)))->state >= 0) {
                cur->state += ((struct IwramNode *)(0x03000000 + (cur->next)))->state;
                cur->next = ((struct IwramNode *)(0x03000000 + (cur->next)))->next;
            } else {
                nextNodeSpace = (void *)(cur->next + (0x03000000 + (((size_t)&((struct IwramNode *)0)->space))));
                ...
                cur->next = ((struct IwramNode *)(0x03000000 + (nextNodeOffset)))->next;
                for (i = 0; i < 128; i++) {
                ...
                { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(nextNodeSpace); dmaRegs[1] = (vu32)(space); dmaRegs[2] = (vu32)((u32)((0x8000 | 0x0000 | 0x0400 | 0x0000 | 0x0000) << 16 | ((cur->state + sizeof(struct IwramNode))/(32/8)))); dmaRegs[2]; };
                {
    ...
    while (curOffset != 0) {
        ...
        if (priority >= lbound) {
            while (priority < rbound) {
                ...
                cur = (struct Task *)(0x03000000 + (prev->next));

                if (prev != gTaskPtrs[0] && prev != gTaskPtrs[1]) {
                    TaskDestroy(prev);
                }

                if (((cur) == (void *)0x03000000)) {
                    break;
                } else if (1) {
        ...
        curOffset = cur->next;
        cur = (struct Task *)(0x03000000 + (curOffset));
    }
}
# 499 "build/collect_rings/src/sa2/task.c"
static void TaskMainDummy1(void) { }
