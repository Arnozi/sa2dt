...
void CreateEntity_StageGoal(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_StageGoal *stageGoal = (void *)(0x03000000 + ((t)->data));
...
static void Task_StageGoalMain(void)
{
    Sprite_StageGoal *stageGoal = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &stageGoal->s;
    ...
    if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) {
...
{
    Sprite_StageGoalToggle *stageGoalToggle = (void *)(0x03000000 + ((gCurTask)->data));
    ...
    if ((!(((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2))))) {
        ...
        if (gGameMode == 0 && !(gPlayer.moveState & 0x00000002) && gPlayer.qSpeedGround > ((s32)((2.5)*256))) {
            ...
            if (extraScore != 0) {
                { s32 divResA, divResB; s32 oldScore = gLevelScore; gLevelScore += extraScore; divResA = Div(gLevelScore, 50000); divResB = Div(oldScore, 50000); if ((divResA != divResB) && (gGameMode == 0)) { u16 lives = divResA - divResB; lives += gNumLives;; ({ if ((lives) > 255) (lives) = 255; gNumLives = (lives); }); if (1) gMusicManagerState.unk3 = 0x10 | 0x0; } };
...
# 174 "src/game/interactables_1/stage_goal.c"
    ...
    x -= gCamera.x;
    y -= gCamera.y;
    if ((((u32)(x + (256 / 2)) > 240 + 256) || (y + (256 / 2) < 0) || (y > 160 + (256 / 2)))) {
        me->x = spriteX;
        TaskDestroy(gCurTask);
    }
}
...
static void StageGoalToggle_HandleMultiplayerFinish(void)
{
    RoomEvent *roomEvent;
    u32 count = 0;
    MultiplayerPlayer *player = (void *)(0x03000000 + ((gMultiplayerPlayerTasks[((volatile struct SioMultiCnt *)(0x4000000 + 0x128))->id])->data));
    ...
    if (!(player->unk5C & 1)) {
        ...
        for (j = 0; j < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])) && gMultiplayerPlayerTasks[j] != ((void *)0); j++) {
            MultiplayerPlayer *otherPlayer = (void *)(0x03000000 + ((gMultiplayerPlayerTasks[j])->data));
            if (otherPlayer->unk5C & 1) {
                count++;
            }
        }

        CreateMultiplayerFinishResult(((volatile struct SioMultiCnt *)(0x4000000 + 0x128))->id, count);
        player->unk5C |= 1;
...
static __attribute__((unused)) void StageGoalToggle_ForceMultiplayerFinish(void)
{
    ...
    u32 id = ((volatile struct SioMultiCnt *)(0x4000000 + 0x128))->id;

    gPlayer.itemEffect &= ~0x40;
    gPlayer.confusionTimer = 0;

    for (j = 0; j < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])) && mpTasks[j] != ((void *)0); j++) {
...
static void StageGoalToggle_PlayerReachedGoal(void)
{
    ...
    if (gMultiplayerRanks[id] != -1) {
        u32 j;
        struct Task **tasks;
        for (j = 0, tasks = gMultiplayerPlayerTasks; j < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])) && tasks[j] != ((void *)0); j++) {
            MultiplayerPlayer *otherPlayer = (void *)(0x03000000 + ((tasks[j])->data));
        ...
        if (count != 0 && (count >= (j - 1) || gGameMode == 4)) {
            ...
            if (gMultiplayerPlayerTasks[0] != ((void *)0)) {
                for (i = 0, tasks = gMultiplayerPlayerTasks; i < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])) && tasks[i] != ((void *)0); i++) {
                    MultiplayerPlayer *otherPlayer = (void *)(0x03000000 + ((tasks[i])->data));
...
{
    struct Task *t = TaskCreate(Task_StageGoalToggleMain, sizeof(Sprite_StageGoalToggle), 0x2010, 0, ((void *)0));
    Sprite_StageGoalToggle *stageGoalToggle = (void *)(0x03000000 + ((t)->data));
