# 0 "src/game/interactables_1/windmill.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_1/windmill.c"
...
# 2 "src/game/interactables_1/windmill.c" 2
...
void VramFree(void *);
# 3 "src/game/interactables_1/windmill.c" 2
...
# 4 "src/game/interactables_1/windmill.c" 2
# 1 "include/trig.h" 1
...
# 5 "src/game/interactables_1/windmill.c" 2
...
# 6 "src/game/interactables_1/windmill.c" 2
...
# 7 "src/game/interactables_1/windmill.c" 2
...
# 6 "include/game/interactables_1/windmill.h" 2

void CreateEntity_Windmill(MapEntity *, u16, u16, u8);
# 8 "src/game/interactables_1/windmill.c" 2
...
# 10 "src/game/interactables_1/windmill.c" 2
...
void CreateEntity_Windmill(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    InteractableWindmill *windmill = (void *)(0x03000000 + ((t)->data));
...
static void Task_WindmillMain(void)
{
    u8 i, j;
    SpriteTransform *transform;
    InteractableWindmill *windmill = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &windmill->center;
    MapEntity *me = windmill->base.me;

    s32 screenX, screenY;

    screenX = ((windmill->base.meX)*8 + (windmill->base.regionX)*256);
    screenY = ((me->y)*8 + (windmill->base.regionY)*256);
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            ...
            s->x = screenX + (((gSineTable[(({ s16 clamped = (sinPeriod) & (1024 - 1); clamped; })) + 256]) * ((j + 1) * 16 - 8)) >> 14);
            s->y = screenY + (((gSineTable[(({ s16 clamped = (sinPeriod) & (1024 - 1); clamped; }))]) * ((j + 1) * 16 - 8)) >> 14);
            transform->rotation = ({ s16 clamped = (sinPeriod) & (1024 - 1); clamped; });
