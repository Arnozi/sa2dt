# 0 "src/game/interactables_1/rotating_handle.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_1/rotating_handle.c"
...
# 2 "src/game/interactables_1/rotating_handle.c" 2
...
# 4 "src/game/interactables_1/rotating_handle.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 6 "src/game/interactables_1/rotating_handle.c" 2
...
# 8 "src/game/interactables_1/rotating_handle.c" 2
...
# 11 "src/game/interactables_1/rotating_handle.c" 2
...
# 12 "src/game/interactables_1/rotating_handle.c" 2
...
# 14 "src/game/interactables_1/rotating_handle.c" 2

# 1 "include/malloc_vram.h" 1
# 16 "src/game/interactables_1/rotating_handle.c" 2
...
# 19 "src/game/interactables_1/rotating_handle.c" 2
...
# 21 "src/game/interactables_1/rotating_handle.c" 2
# 1 "include/constants/char_states.h" 1
# 22 "src/game/interactables_1/rotating_handle.c" 2
# 1 "include/constants/player_transitions.h" 1
# 23 "src/game/interactables_1/rotating_handle.c" 2
# 1 "include/constants/songs.h" 1
# 24 "src/game/interactables_1/rotating_handle.c" 2
...
void CreateEntity_RotatingHandle(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    if (me->d.sData[0] >= 0) {
        ...
        Sprite_RotatingHandle *rotatingHandle = (void *)(0x03000000 + ((t)->data));
...
static void Task_Idle(void)
{
    Sprite_RotatingHandle *rotatingHandle = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &rotatingHandle->s;
    MapEntity *me = rotatingHandle->base.me;
    s32 x = ((rotatingHandle->base.meX)*8 + (rotatingHandle->base.regionX)*256);
    s32 y = ((me->y)*8 + (rotatingHandle->base.regionY)*256);

    s->x = x - gCamera.x;
    s->y = y - gCamera.y;

    if (!(gPlayer.moveState & (0x00400000 | 0x00000080)) && Coll_Player_Entity_HitboxN(s, x, y, 0, &gPlayer, 0) == 1) {
        ...
        if (gPlayer.qSpeedAirX > 0) {
            ...
            if (((int)((gPlayer.qWorldY) >> 8)) > y) {
                s->frameFlags |= ((1) << (10));
                gPlayer.charState = 45;
                rotatingHandle->quartile = 0;
            } else {
                s->frameFlags &= ~((1) << (10));
                gPlayer.charState = 46;
...
__attribute__((naked)) void Task_Rotating() { asm(".include " "\"asm/non_matching/game/interactables_1/Task_Rotating.inc\""); if (0)
{
    MapEntity *me;
    Sprite *sprite;
    ...
    rotatingHandle = (void *)(0x03000000 + ((gCurTask)->data));

    sprite = &rotatingHandle->s;
    me = rotatingHandle->base.me;

    posX = ((rotatingHandle->base.meX)*8 + (rotatingHandle->base.regionX)*256);
    posY = ((me->y)*8 + (rotatingHandle->base.regionY)*256);

    rotatingHandle->rot = (rotatingHandle->rot + rotatingHandle->rotSpeed) & 0x3FF0;

    temp3 = ({ s16 clamped = (rotatingHandle->rot) & (1024 - 1); clamped; }) + 0;
    ...
    if (gPlayer.frameInput & gPlayerControls.jump) {
        ...
        switch (rotatingHandle->quartile) {
            case 0:
                r4 = ({ s16 clamped = (0x20 - temp) & (1024 - 1); clamped; }) + 0;
                sin = (gSineTable[(temp)]);
                gPlayer.qWorldX += sin >> 1;
                cos = (gSineTable[(temp) + 256]);
            ...
            case 1:
                ...
                do {
                    ...
                    r4 = ({ s16 clamped = (temp + 0x20) & (1024 - 1); clamped; }) + 0;
                    sin = (gSineTable[(temp)]);
                ...
                break;
            case 2:
                r4 = ({ s16 clamped = (temp + 0x1E0) & (1024 - 1); clamped; }) + 0;
                sin = (gSineTable[(temp)]);
                gPlayer.qWorldX -= sin >> 1;
                cos = (gSineTable[(temp) + 256]);
                ...
                break;
            case 3: {
                r4 = ({ s16 clamped = (0x220 - temp) & (1024 - 1); clamped; }) + 0;
                sin = (gSineTable[(temp)]);
                gPlayer.qWorldX -= sin >> 1;
                cos = (gSineTable[(temp) + 256]);
                gPlayer.qWorldY -= cos >> 1;
        ...
        gPlayer.qSpeedAirX = Div((gSineTable[(r4) + 256]) << 1, 0x11);
        gPlayer.qSpeedAirY = Div((gSineTable[(r4)]) << 1, 0x11);
        gPlayer.charState = 50;
        gPlayer.unk6C = 1;
    } else {
        u8 r2;
        if (gPlayer.charState == 45) {
            r2 = Div(temp, 0x56);
            if (r2 > 0xB) {
                r2 = 0xB;
            }
            ...
            gPlayer.variant = r2;
            gPlayer.unk6C = 1;
            gPlayer.qWorldX = ((s32)((posX)*256));
            gPlayer.qWorldY = ((s32)((posY)*256));
    ...
    if ((((u16)(sprite->x + (256 / 2)) > 240 + 256) || (sprite->y + (256 / 2) < 0) || (sprite->y > 160 + (256 / 2)))) {
