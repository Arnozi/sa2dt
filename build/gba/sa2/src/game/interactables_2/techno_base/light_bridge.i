# 0 "src/game/interactables_2/techno_base/light_bridge.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_2/techno_base/light_bridge.c"
...
# 2 "src/game/interactables_2/techno_base/light_bridge.c" 2
...
# 3 "src/game/interactables_2/techno_base/light_bridge.c" 2
...
# 4 "src/game/interactables_2/techno_base/light_bridge.c" 2
...
# 5 "src/game/interactables_2/techno_base/light_bridge.c" 2
...
# 6 "src/game/interactables_2/techno_base/light_bridge.c" 2

# 1 "include/task.h" 1
# 8 "src/game/interactables_2/techno_base/light_bridge.c" 2
# 1 "include/malloc_vram.h" 1
# 9 "src/game/interactables_2/techno_base/light_bridge.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 10 "src/game/interactables_2/techno_base/light_bridge.c" 2

# 1 "include/constants/interactables.h" 1
# 12 "src/game/interactables_2/techno_base/light_bridge.c" 2
...
# 13 "src/game/interactables_2/techno_base/light_bridge.c" 2
# 1 "include/constants/songs.h" 1
# 14 "src/game/interactables_2/techno_base/light_bridge.c" 2
# 22 "src/game/interactables_2/techno_base/light_bridge.c"
typedef struct {
    Sprite sprites[2];
    s32 posX;
    s32 posY;
...
static const u16 sBridgeSprites[2][3] = {
    { 601, 0, 2 },
    { 601, 1, 2 },
};
...
void CreateEntity_LightBridge(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_LightBridge *lightBridge = (void *)(0x03000000 + ((t)->data));
    Sprite *s;
    lightBridge->type = me->d.uData[0];
    lightBridge->unk6C = me->d.uData[1];
    lightBridge->active = 0;
    lightBridge->playingSfx = 0;
    lightBridge->posX = ((me->x)*8 + (spriteRegionX)*256);
    lightBridge->posY = ((me->y)*8 + (spriteRegionY)*256);
...
{
    Sprite_LightBridge *lightBridge = (void *)(0x03000000 + ((gCurTask)->data));
...
static void RenderStraightBridge(Sprite_LightBridge *lightBridge)
{
    ...
    s16 x = ({ s32 temp = ((posX) - (cameraX)); temp += (-((28 * 8) / 2)); temp; });
    s16 maxX = ({ s32 temp = ((posX) - (cameraX)); temp += (((28 * 8) / 2)); temp; });
    s16 y = ({ s32 temp = ((lightBridge->posY) - (gCamera.y)); temp += (0); temp; });

    u8 i;
    for (i = (gStageTime >> 1) & 1; x <= maxX && x <= 246;
         x += 12, i++) {
        if ((i & 1) && x > 5) {
            s->x = x;
            s->y = y;
...
static void RenderCurvedBridge(Sprite_LightBridge *in)
{
    ...
    x = ({ s32 temp = ((posX) - (cameraX)); temp += (0); temp; });
...
static bool32 IsPlayerOutOfRange(Sprite_LightBridge *lightBridge)
{
    s32 posX = lightBridge->posX;
    s32 cameraX = gCamera.x;
    s16 x = ({ s32 temp = ((posX) - (cameraX)); temp += (0); temp; });
    s32 posY = lightBridge->posY;
    s32 cameraY = gCamera.y;
    s16 y = ({ s32 temp = ((posY) - (cameraY)); temp += (0); temp; });
    if (x + sOutOfRangeDimensions[lightBridge->type].maxX < -128
        || x + sOutOfRangeDimensions[lightBridge->type].minX > (240 + 128)
        || y + sOutOfRangeDimensions[lightBridge->type].maxY < -128
        || y + sOutOfRangeDimensions[lightBridge->type].minY > (160 + 128)) {
        return 1;
    }
