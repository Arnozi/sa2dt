# 0 "src/game/interactables_2/music_plant/chord.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_2/music_plant/chord.c"
...
# 2 "src/game/interactables_2/music_plant/chord.c" 2
# 1 "include/core.h" 1
...
# 3 "src/game/interactables_2/music_plant/chord.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 4 "src/game/interactables_2/music_plant/chord.c" 2
# 1 "include/sprite.h" 1
# 5 "src/game/interactables_2/music_plant/chord.c" 2
...
# 7 "src/game/interactables_2/music_plant/chord.c" 2
# 1 "include/task.h" 1
# 8 "src/game/interactables_2/music_plant/chord.c" 2
...
# 12 "src/game/interactables_2/music_plant/chord.c" 2
...
# 14 "src/game/interactables_2/music_plant/chord.c" 2
# 1 "include/constants/char_states.h" 1
# 15 "src/game/interactables_2/music_plant/chord.c" 2
# 1 "include/constants/player_transitions.h" 1
# 16 "src/game/interactables_2/music_plant/chord.c" 2
# 1 "include/constants/songs.h" 1
# 17 "src/game/interactables_2/music_plant/chord.c" 2
...
void CreateEntity_Chord(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_Chord *chord = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &chord->s;
    u16 i;
    ...
    s->graphics.dest = (void *)((u8*)(0x6000000 + 0x10000) + 0x3700);
    s->graphics.anim = 575;
    s->variant = 15;

    chord->posX = ((me->x)*8 + (spriteRegionX)*256);
    chord->posY = ((me->y)*8 + (spriteRegionY)*256);
    ...
    for (i = 0; i < 6; i++) {
        ...
        switch (i) {
            case 0:
            case 5: {
                element->peak = ((s16)((0)*256));
            } break;

            case 1:
            case 4: {
                element->peak = ((s16)((0.5)*256));
            } break;
...
{
    u8 r7 = 0;
    Sprite_Chord *chord = (void *)(0x03000000 + ((gCurTask)->data));
    u8 i;
    ...
    for (i = 0; i < 6; i++) {
        ChordElement *element = &chord->elements[i];
        s16 r1 = ((int)(((chord->bounceAccel - element->y) * element->peak) >> 8));
    ...
    if ((!(gPlayer.moveState & 0x00000080))) {
        gPlayer.qWorldY = ((s32)((chord->posY - 16)*256)) + chord->elements[2].y;
        gPlayer.rotation = 64;
    }
...
static void HandleBounce(Sprite_Chord *chord)
{
    if ((!(gPlayer.moveState & 0x00000080))) {
        ...
        ({ if ((gPlayer.qSpeedAirY) < (((s16)((4.0)*256)))) { gPlayer.qSpeedAirY = (((s16)((4.0)*256))); } else if ((gPlayer.qSpeedAirY) > (((s16)((12.0)*256)))) { gPlayer.qSpeedAirY = (((s16)((12.0)*256))); } });
    }

    chord->bounceAccel = (u16)gPlayer.qSpeedAirY * 2;

    if (chord->bounceAccel < ((s16)((4.0)*256))) {
        chord->bounceAccel = ((s16)((4.0)*256));
    }
...
static void Render(Sprite_Chord *chord)
{
    ...
    for (i = 0; i < 6; i++) {
        ChordElement *element = &chord->elements[i];
        s16 elY = ((element->y) >= 0 ? (element->y) : -(element->y)) >> 3;
        s32 r2, r1;
        if (i > 2)
            elY = -elY;

        r2 = chord->posX + 4;
        r2 += ((int)((element->x + elY) >> 8));
        r2 -= gCamera.x;
        s->x = r2;
...
{
    if ((!(gPlayer.moveState & 0x00000080)) && gPlayer.qSpeedAirY > 0) {
        ...
        if ((screenX <= playerX) && ((screenX + (6 * 8)) >= playerX) && ((screenY - 9) <= playerY) && ((screenY + 9) >= playerY)) {
            return 1;
        }
    }
    return 0;
...
static void TaskDestructor_Chord(struct Task __attribute__((unused)) *t) { }

static void LaunchPlayer(Sprite_Chord __attribute__((unused)) *chord)
{
    if ((!(gPlayer.moveState & 0x00000080))) {
...
static void HandleVibrateFinished(Sprite_Chord __attribute__((unused)) *chord) { gCurTask->main = Task_Idle; }
...
{
    if ((!(gPlayer.moveState & 0x00000080))) {
        if (gPlayer.qWorldX != (((s32)((chord->posX + ((6 * 8) / 2))*256)))) {
            if (gPlayer.qWorldX > (((s32)((chord->posX + ((6 * 8) / 2))*256)))) {
                gPlayer.qWorldX -= ((s32)((0.5)*256));

                if (gPlayer.qWorldX < (((s32)((chord->posX + ((6 * 8) / 2))*256))))
                    gPlayer.qWorldX = (((s32)((chord->posX + ((6 * 8) / 2))*256)));
            } else {
                gPlayer.qWorldX += ((s32)((0.5)*256));
...
static bool32 ShouldDespawn(Sprite_Chord *chord)
{
    ...
    if (((otherX < -((256 + ((6 * 8) / 2)) / 2)) || (otherX > 240 + ((256 + ((6 * 8) / 2)) / 2)) || (otherY < -(256 / 2)) || (otherY > 160 + (256 / 2)))) {
        return 1;
    }
