# 0 "src/game/interactables_2/egg_utopia/launcher.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_2/egg_utopia/launcher.c"
...
# 2 "src/game/interactables_2/egg_utopia/launcher.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 4 "src/game/interactables_2/egg_utopia/launcher.c" 2
...
void VramFree(void *);
# 6 "src/game/interactables_2/egg_utopia/launcher.c" 2
...
# 8 "src/game/interactables_2/egg_utopia/launcher.c" 2
...
# 9 "src/game/interactables_2/egg_utopia/launcher.c" 2
...
# 10 "src/game/interactables_2/egg_utopia/launcher.c" 2
...
# 11 "src/game/interactables_2/egg_utopia/launcher.c" 2

# 1 "include/task.h" 1
# 13 "src/game/interactables_2/egg_utopia/launcher.c" 2
# 1 "include/trig.h" 1
...
# 14 "src/game/interactables_2/egg_utopia/launcher.c" 2
...
# 16 "src/game/interactables_2/egg_utopia/launcher.c" 2
# 1 "include/constants/char_states.h" 1
# 17 "src/game/interactables_2/egg_utopia/launcher.c" 2
# 1 "include/constants/player_transitions.h" 1
# 18 "src/game/interactables_2/egg_utopia/launcher.c" 2
# 1 "include/constants/songs.h" 1
# 19 "src/game/interactables_2/egg_utopia/launcher.c" 2
...
typedef struct {
    ...
    s32 spritePos[3][2];
} Sprite_EggUtopia_Launcher;
# 54 "src/game/interactables_2/egg_utopia/launcher.c"
...
void CreateEntity_Launcher(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY, u32 kind)
{
    ...
    Sprite_EggUtopia_Launcher *launcher = (void *)(0x03000000 + ((t)->data));

    launcher->kind = kind;
    launcher->posX = ((me->x)*8 + (spriteRegionX)*256);
    launcher->posY = ((me->y)*8 + (spriteRegionY)*256);

    launcher->boundsLeft = me->d.sData[0] * 8;
    launcher->boundsTop = me->d.sData[1] * 8;
    launcher->boundsRight = launcher->boundsLeft + me->d.uData[2] * 8;
    launcher->boundsBottom = launcher->boundsTop + me->d.uData[3] * 8;
    ...
    switch (launcher->kind) {
        case (((0) << 1) | (0)): {
            launcher->kartX = ((s32)((launcher->posX + launcher->boundsRight)*256));
            launcher->kartY = ((s32)((launcher->posY + launcher->boundsBottom)*256));
        } break;

        case (((0) << 1) | (1)): {
            launcher->kartX = ((s32)((launcher->posX + launcher->boundsLeft)*256));
            launcher->kartY = ((s32)((launcher->posY + launcher->boundsBottom)*256));
        } break;

        case (((1) << 1) | (0)): {
            launcher->kartX = ((s32)((launcher->posX + launcher->boundsRight)*256));
            launcher->kartY = ((s32)((launcher->posY + launcher->boundsTop)*256));
        } break;
...

    {
        ...
        for (; i < (sizeof(launcher->spritePos) / sizeof((launcher->spritePos)[0])); i++) {
...

        {
            ...
            launcher->s.animSpeed = ((int)((float)(1.0)*0x10));
            launcher->s.palId = 0;
...
static void Task_Active(void)
{
    Sprite_EggUtopia_Launcher *launcher = (void *)(0x03000000 + ((gCurTask)->data));
...
static void HandleActivate(Sprite_EggUtopia_Launcher *launcher)
{
    ...
    if (((launcher->kind == (((0) << 1) | (0))) || (launcher->kind == (((1) << 1) | (0))))) {
...
static void HandleReachedEnd(Sprite_EggUtopia_Launcher *launcher)
{
    if ((!(gPlayer.moveState & 0x00000080)) && launcher->active) {
        gPlayer.moveState &= ~0x00400000;
        ...
        switch (launcher->kind) {
            case (((0) << 1) | (0)):
            case (((1) << 1) | (0)): {
                gPlayer.qSpeedAirX = -((s32)((15.0)*256));
                gPlayer.qSpeedAirY = -((s32)((3.0)*256));
            } break;
...
static void RenderKart(Sprite_EggUtopia_Launcher *launcher)
{
    ...
    if ((!(((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2))))) {
        s->x = ((int)((launcher->spritePos[1][0]) >> 8)) - gCamera.x;
        s->y = ((int)((launcher->spritePos[1][1]) >> 8)) - gCamera.y;
    } else {
        s->x = ((int)((launcher->kartX) >> 8)) - gCamera.x;
        s->y = ((int)((launcher->kartY) >> 8)) - gCamera.y;
    }
...
static bool32 IsPlayerTouchingKart(Sprite_EggUtopia_Launcher *launcher)
{
    if ((!(gPlayer.moveState & 0x00000080))) {
        ...
        if ((gStageFlags & 0x0080)) {
            if (((launcher->kind == (((0) << 1) | (0))) || (launcher->kind == (((0) << 1) | (1)))))
                return 0;
        } else {
            if (((launcher->kind == (((1) << 1) | (0))) || (launcher->kind == (((1) << 1) | (1)))))
                return 0;
        }

        someX = ((int)((launcher->kartX) >> 8)) - gCamera.x;
        someY = ((int)((launcher->kartY) >> 8)) - gCamera.y;

        playerX = ((int)((gPlayer.qWorldX) >> 8)) - gCamera.x;
        playerY = ((int)((gPlayer.qWorldY) >> 8)) - gCamera.y;

        if ((someX - 2 * 8 <= playerX) && (someX + 2 * 8 >= playerX) && (someY - 2 * 8 <= playerY)
            && (someY + 2 * 8 >= playerY))
            return 1;
    }
...
static bool16 MoveKartToEnd(Sprite_EggUtopia_Launcher *launcher)
{
    ...
    if (((launcher->kind == (((0) << 1) | (0))) || (launcher->kind == (((1) << 1) | (0))))) {
        s32 someX;
        launcher->kartX -= ((s32)((15)*256));
        someX = ((s32)((launcher->posX + launcher->boundsLeft)*256));
        ...
        s32 someX;
        launcher->kartX += ((s32)((15)*256));
        someX = ((s32)((launcher->posX + launcher->boundsRight)*256));
        if (launcher->kartX >= someX) {
            launcher->kartX = someX;
            result = 1;
        }
...
static void SetPlayerPos(Sprite_EggUtopia_Launcher *launcher)
{
    if ((!(gPlayer.moveState & 0x00000080)) && launcher->active) {
        switch (launcher->kind) {
            case (((0) << 1) | (0)): {
                gPlayer.qWorldX = launcher->kartX - ((s32)((8)*256));
                gPlayer.qWorldY = launcher->kartY - ((s32)((16)*256));
            } break;
...
static bool32 ShouldDespawn(Sprite_EggUtopia_Launcher *launcher)
{
    ...
    if (((posX + launcher->boundsRight) < -(256 / 2))
        || ((posX + launcher->boundsLeft) > 240 + (256 / 2))
        || ((posY + launcher->boundsBottom) < -(256 / 2))
        || ((posY + launcher->boundsTop) > 160 + (256 / 2)))
        return 1;
...
static void Despawn(Sprite_EggUtopia_Launcher *launcher)
{
    { launcher->base.me->x = launcher->base.meX; };
...
static bool16 MoveKartToBase(Sprite_EggUtopia_Launcher *launcher)
{
    ...
    if (((launcher->kind == (((0) << 1) | (0))) || (launcher->kind == (((1) << 1) | (0))))) {
        s32 value;
        launcher->kartX += ((s32)((1.0)*256));
        value = ((s32)((launcher->posX + launcher->boundsRight)*256));
        ...
        s32 value;
        launcher->kartX -= ((s32)((1.0)*256));
        value = ((s32)((launcher->posX + launcher->boundsLeft)*256));
        if (launcher->kartX <= value) {
            launcher->kartX = value;
            result = 1;
        }
