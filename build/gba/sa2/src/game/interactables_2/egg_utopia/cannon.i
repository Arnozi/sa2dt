# 0 "src/game/interactables_2/egg_utopia/cannon.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_2/egg_utopia/cannon.c"
...
# 2 "src/game/interactables_2/egg_utopia/cannon.c" 2
...
# 4 "src/game/interactables_2/egg_utopia/cannon.c" 2
...
# 1 "include/global.h" 1
# 5 "include/game/stage/player_controls.h" 2
...
# 6 "src/game/interactables_2/egg_utopia/cannon.c" 2
...
# 7 "src/game/interactables_2/egg_utopia/cannon.c" 2
...
# 8 "src/game/interactables_2/egg_utopia/cannon.c" 2
...
# 6 "include/game/interactables_2/egg_utopia/cannon.h" 2

void CreateEntity_Cannon(MapEntity *, u16, u16, u8);
# 9 "src/game/interactables_2/egg_utopia/cannon.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 11 "src/game/interactables_2/egg_utopia/cannon.c" 2
# 1 "include/trig.h" 1
...
# 12 "src/game/interactables_2/egg_utopia/cannon.c" 2
...
# 13 "src/game/interactables_2/egg_utopia/cannon.c" 2
...
# 15 "src/game/interactables_2/egg_utopia/cannon.c" 2
# 1 "include/constants/char_states.h" 1
# 16 "src/game/interactables_2/egg_utopia/cannon.c" 2
# 1 "include/constants/player_transitions.h" 1
# 17 "src/game/interactables_2/egg_utopia/cannon.c" 2
# 1 "include/constants/songs.h" 1
# 18 "src/game/interactables_2/egg_utopia/cannon.c" 2
...
void CreateEntity_Cannon(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_Cannon *cannon = (void *)(0x03000000 + ((t)->data));
    cannon->facingRight = me->d.sData[0];
    cannon->x = ((me->x)*8 + (spriteRegionX)*256);
    cannon->y = ((me->y)*8 + (spriteRegionY)*256);
    cannon->me = me;
    cannon->spriteX = me->x;
    cannon->spriteY = spriteY;

    if (!cannon->facingRight) {
        cannon->cannonAngle = ((int)((((double)(180)) / 360.0) * 1024));
    } else {
        cannon->cannonAngle = 0;
    }

    s = &cannon->sprite2;
    s->oamFlags = ((7) << 6); s->graphics.size = 0; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;; s->frameFlags = (((2) << (12)) | (0));;
    s->graphics.dest = (void *)(u8*)(0x6000000 + 0x10000) + 0x2C80;
    s->graphics.anim = 616;
    s->variant = 1;
...
{
    Sprite_Cannon *cannon = (void *)(0x03000000 + ((gCurTask)->data));
    if (!(!(gPlayer.moveState & 0x00000080)) || --cannon->fireTimeout == (u16)-1 || gPlayer.frameInput & (gPlayerControls.jump | gPlayerControls.attack)) {
        Fire(cannon);
    } else {
...
static void HandlePlayerEnter(Sprite_Cannon *cannon)
{
    ...
    if (!cannon->facingRight) {
        gPlayer.qWorldX = ((s32)((cannon->x + 40)*256));
        gPlayer.qWorldY = ((s32)((cannon->y)*256));
...
static void Fire(Sprite_Cannon *cannon)
{
    ...
    if ((!(gPlayer.moveState & 0x00000080))) {
        ...
        gPlayer.qWorldX += ((int)(((gSineTable[(cannon->cannonAngle) + 256])) >> 6)) * 32;
        gPlayer.qWorldY += ((int)(((gSineTable[(cannon->cannonAngle)])) >> 6)) * 32;
        gPlayer.qSpeedAirX = ((int)(((gSineTable[(cannon->cannonAngle) + 256])) >> 6)) * 15;
        gPlayer.qSpeedAirY = ((int)(((gSineTable[(cannon->cannonAngle)])) >> 6)) * 15;

        if ((gStageFlags & 0x0080)) {
            gPlayer.qSpeedAirY = -gPlayer.qSpeedAirY;
        }

        gPlayer.rotation = ((cannon->cannonAngle) / 4);
...
static void HandleMovement(Sprite_Cannon *cannon)
{
    ...
    r3 = (!cannon->facingRight) ? ((cannon->unk6E == 0) ? ((int)((((double)(225)) / 360.0) * 1024)) : ((int)((((double)(135)) / 360.0) * 1024)))
                                : ((cannon->unk6E == 0) ? ((int)((((double)(45)) / 360.0) * 1024)) : ((int)((((double)(315)) / 360.0) * 1024)));
    ...
    cannon->cannonAngle = ({ s16 clamped = (cannon->cannonAngle + temp3) & (1024 - 1); clamped; });
}
...
__attribute__((naked)) bool32 Cannon_IsPlayerTouching(Sprite_Cannon *cannon) { asm(".include " "\"asm/non_matching/game/interactables_2/egg_utopia/cannon__IsPlayerTouching.inc\""); if (0)

{
    ...
    x = r3->x - gCamera.x;
    y = r3->y - gCamera.y;
    playerX = ((int)((gPlayer.qWorldX) >> 8)) - gCamera.x;
    playerY = ((int)((gPlayer.qWorldY) >> 8)) - gCamera.y;


    if ((((((x) + s2->hitboxes[0].left) <= ((playerX) + gPlayerBodyPSI.s.hitboxes[0].left) && ((((x)) + (s2->hitboxes[0]).left) + ((s2->hitboxes[0]).right - (s2->hitboxes[0]).left)) >= ((playerX) + gPlayerBodyPSI.s.hitboxes[0].left)) || (((x) + s2->hitboxes[0].left) >= ((playerX) + gPlayerBodyPSI.s.hitboxes[0].left) && ((((playerX)) + (gPlayerBodyPSI.s.hitboxes[0]).left) + ((gPlayerBodyPSI.s.hitboxes[0]).right - (gPlayerBodyPSI.s.hitboxes[0]).left)) >= ((x) + s2->hitboxes[0].left))) && (((((y)) + s2->hitboxes[0].top) <= (((playerY)) + gPlayerBodyPSI.s.hitboxes[0].top) && ((((y)) + s2->hitboxes[0].top) + ((s2->hitboxes[0]).bottom - (s2->hitboxes[0]).top)) >= (((playerY)) + gPlayerBodyPSI.s.hitboxes[0].top)) || ((((y)) + s2->hitboxes[0].top) >= (((playerY)) + gPlayerBodyPSI.s.hitboxes[0].top) && ((((playerY)) + gPlayerBodyPSI.s.hitboxes[0].top) + ((gPlayerBodyPSI.s.hitboxes[0]).bottom - (gPlayerBodyPSI.s.hitboxes[0]).top)) >= (((y)) + s2->hitboxes[0].top))))) {
        return 1;
    }
...
static bool32 ShouldDespawn(Sprite_Cannon *cannon)
{
    ...
    if (((x < -((256 / 2))) || (x > 240 + ((256 / 2))) || (y < -((256 / 2))) || (y > 160 + ((256 / 2))))) {
        return 1;
    }
...
static void HandleSetIdle(__attribute__((unused)) Sprite_Cannon *cannon) { gCurTask->main = Task_Idle; }
...
static bool16 HandleResetMovement(Sprite_Cannon *cannon)
{
    ...
    r3 = cannon->facingRight == 0 ? ((int)((((double)(180)) / 360.0) * 1024)) : ((int)((((double)(0)) / 360.0) * 1024));
    temp2 = sub_808558C(cannon->cannonAngle, r3, 10);
    temp3 = temp2;
