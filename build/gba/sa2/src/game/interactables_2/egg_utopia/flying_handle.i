# 0 "src/game/interactables_2/egg_utopia/flying_handle.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_2/egg_utopia/flying_handle.c"
...
# 2 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
# 1 "include/core.h" 1
...
# 3 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
# 1 "include/task.h" 1
# 4 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
...
# 6 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
...
# 7 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
...
# 8 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
...
# 6 "include/game/interactables_2/egg_utopia/flying_handle.h" 2

void CreateEntity_FlyingHandle(MapEntity *, u16, u16, u8);
# 9 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
# 1 "include/malloc_vram.h" 1
# 10 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
# 1 "include/trig.h" 1
...
# 11 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 12 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
...
# 14 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
# 1 "include/constants/char_states.h" 1
# 15 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
# 1 "include/constants/player_transitions.h" 1
# 16 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
# 1 "include/constants/songs.h" 1
# 17 "src/game/interactables_2/egg_utopia/flying_handle.c" 2
...
void CreateEntity_FlyingHandle(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_FlyingHandle *flyingHandle = (void *)(0x03000000 + ((t)->data));
    flyingHandle->qOscillationTurns = 0;
    flyingHandle->speedX = 0;
    flyingHandle->speedY = 0;
    flyingHandle->activated = 0;
    flyingHandle->basePosX = ((me->x)*8 + (spriteRegionX)*256);
    flyingHandle->basePosY = ((me->y)*8 + (spriteRegionY)*256);
    ...
    flyingHandle->posX = ((s32)((((flyingHandle->boundsLeft + flyingHandle->boundsRight) >> 1) + flyingHandle->basePosX)*256));
    flyingHandle->posY = ((s32)((flyingHandle->basePosY + flyingHandle->boundsBottom)*256));
    ...
    s = &flyingHandle->s;
    s->oamFlags = ((18) << 6); s->graphics.size = 0; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;; s->frameFlags = (((2) << (12)) | (0));;
...
static void Task_ActiveMain(void)
{
    Sprite_FlyingHandle *flyingHandle = (void *)(0x03000000 + ((gCurTask)->data));
    ...
    if (!(!(gPlayer.moveState & 0x00000080))) {
        ...
        if (gPlayer.frameInput & gPlayerControls.jump) {
            if (gPlayer.heldInput & 0x0020) {
                flyingHandle->leftHeld = 1;
            } else {
                flyingHandle->leftHeld = 0;
            }
...
static void ActivatePlayerGrab(Sprite_FlyingHandle *flyingHandle)
{
    ...
    flyingHandle->activated = 1;
    flyingHandle->speedY = -((s32)((1.5)*256));
...
static void UpdatePosition(Sprite_FlyingHandle *flyingHandle)
{
    if (flyingHandle->activated) {
        if (flyingHandle->posY <= ((s32)((flyingHandle->basePosY + flyingHandle->boundsTop)*256)) && flyingHandle->speedY >= 0) {
            flyingHandle->moveOffsetY = ((int)(((gSineTable[(((flyingHandle->qOscillationTurns)*4))])) >> 6)) * 8;
            flyingHandle->qOscillationTurns += ((s32)((0.015625)*256));
            flyingHandle->speedY = 0;
        } else {
            flyingHandle->speedY += ((s32)((0.0625)*256));
            flyingHandle->speedY = (((flyingHandle->speedY) < (((s32)((3)*256)))) ? (flyingHandle->speedY) : (((s32)((3)*256))));
            flyingHandle->posY -= flyingHandle->speedY;
            if (flyingHandle->posY <= ((s32)((flyingHandle->basePosY + flyingHandle->boundsTop)*256))) {
                flyingHandle->speedY = 0;
                flyingHandle->posY = ((s32)((flyingHandle->basePosY + flyingHandle->boundsTop)*256));
                flyingHandle->qOscillationTurns = -((s32)((0.5)*256));
            }
        }
    } else {
        flyingHandle->posY = ((s32)((flyingHandle->basePosY + flyingHandle->boundsBottom)*256));
...
static void Render(Sprite_FlyingHandle *flyingHandle)
{
    ...
    if ((!(((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2))))) {
        s->x = ((int)((flyingHandle->mpPosQueue[1][0]) >> 8)) - gCamera.x;
        s->y = ((int)((flyingHandle->mpPosQueue[1][1]) >> 8)) - gCamera.y;
    } else {
        s->x = ((int)((flyingHandle->posX + flyingHandle->moveOffsetX) >> 8)) - gCamera.x;
        s->y = ((int)((flyingHandle->posY + flyingHandle->moveOffsetY) >> 8)) - gCamera.y;
    }
...
static bool32 IsPlayerTouching(Sprite_FlyingHandle *flyingHandle)
{
    if (!(!(gPlayer.moveState & 0x00000080))) {
        ...
        s16 x = ((int)((flyingHandle->posX + flyingHandle->moveOffsetX) >> 8)) - gCamera.x;
        s16 y = ((int)((flyingHandle->posY + flyingHandle->moveOffsetY) >> 8)) - gCamera.y;
        s16 playerX = (((int)((gPlayer.qWorldX) >> 8)) - gCamera.x);
        s16 playerY = (((int)((gPlayer.qWorldY) >> 8)) - gCamera.y);
        s16 dX = x - playerX;
        s16 dY = y - playerY;
...
static void Task_Deactivate(void)
{
    ...
    if (flyingHandle->cooldown == 0) {
        if (!(!(gPlayer.moveState & 0x00000080)) || !IsPlayerTouching(flyingHandle)) {
...
static void HandlePlayerDied(__attribute__((unused)) Sprite_FlyingHandle *flyingHandle) { gCurTask->main = Task_Idle; }

static void HandlePlayerKnockedOff(__attribute__((unused)) Sprite_FlyingHandle *flyingHandle) { gCurTask->main = Task_Idle; }
...
static void UpdatePlayerPos(Sprite_FlyingHandle *flyingHandle)
{
    ...
    register u32 r1 asm("r1") = p1->moveState & 0x00000001;
    ...
    if (r1) {
        player->qWorldX = (flyingHandle->posX + flyingHandle->moveOffsetX) + ((s32)((4.0)*256));
        player->qWorldY = (flyingHandle->posY + flyingHandle->moveOffsetY) + ((s32)((20.0)*256));
    } else {
        player->qWorldX = (flyingHandle->posX + flyingHandle->moveOffsetX) - ((s32)((4.0)*256));
        player->qWorldY = (flyingHandle->posY + flyingHandle->moveOffsetY) + ((s32)((20.0)*256));
    }
...
static void PlaySound(__attribute__((unused)) Sprite_FlyingHandle *flyingHandle) { m4aSongNumStartOrContinue(291); }
...
static bool32 ShouldDespawn(Sprite_FlyingHandle *flyingHandle)
{
    ...
    if (x + flyingHandle->boundsRight < -(256 / 2) || x + flyingHandle->boundsLeft > (240 + (256 / 2))
        || y + flyingHandle->boundsBottom < -(256 / 2)
        || y + flyingHandle->boundsTop > (160 + (256 / 2))) {
        return 1;
    }
...
{
    { flyingHandle->base.me->x = flyingHandle->base.meX; };
