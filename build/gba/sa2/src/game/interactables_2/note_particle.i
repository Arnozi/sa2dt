# 0 "src/game/interactables_2/note_particle.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_2/note_particle.c"
...
# 2 "src/game/interactables_2/note_particle.c" 2
...
void VramFree(void *);
# 4 "src/game/interactables_2/note_particle.c" 2
...
# 6 "src/game/interactables_2/note_particle.c" 2
...
# 7 "src/game/interactables_2/note_particle.c" 2
...
# 8 "src/game/interactables_2/note_particle.c" 2

# 1 "include/task.h" 1
# 10 "src/game/interactables_2/note_particle.c" 2
# 1 "include/trig.h" 1
...
# 11 "src/game/interactables_2/note_particle.c" 2
...
# 13 "src/game/interactables_2/note_particle.c" 2
...
# 15 "src/game/interactables_2/note_particle.c" 2
...
typedef struct {
               ...
               s16 accelX;
               s16 accelY;
               u16 framesUntilVisible;
               u16 framesUntilDestroyed;
               u8 kind;
} Sprite_NoteParticle;


static const __attribute__((aligned(4))) u16 gUnknown_080E0140[4][5] = {
    [0] = { 576, 0, 0x1B9, 0, (4 | 2) },
    [1] = { 576, 1, 0x1BB, 0, (4 | 2) },
    [2] = { 577, 0, 0x004, 1, (4 | 2) },
    [3] = { 578, 0, 0x006, 1, (0 | 2) },
...
void CreateAngledNoteParticle(s32 posX, s32 posY, u16 framesUntilVisible, u16 framesUntilDestroyed, s16 velocity, u8 quarterAngle, u8 kind)
{
    ...
    Sprite_NoteParticle *np = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &np->s;
    ...
    np->accelX = ((int)((velocity * ((int)(((gSineTable[(quarterAngle * 4) + 256])) >> 6))) >> 8));
    np->accelY = ((int)((velocity * ((int)(((gSineTable[(quarterAngle * 4)])) >> 6))) >> 8));
    np->framesUntilVisible = framesUntilVisible;
    np->framesUntilDestroyed = framesUntilDestroyed;
    np->kind = kind;

    s->oamFlags = ((6) << 6); s->graphics.size = 0; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;; s->frameFlags = (((gUnknown_080E0140[kind][4]) << (12)) | (0));;

    if (gUnknown_080E0140[kind][3] != 0) {
        s->graphics.dest = VramMalloc(gUnknown_080E0140[kind][2]);
    } else {
        s->graphics.dest = (void *)((u8*)(0x6000000 + 0x10000) + gUnknown_080E0140[kind][2] * 32);
    }
...
static void Task_8080DB8(void)
{
    ...
    if (--np->framesUntilDestroyed == (u16)-1) {
        ...
        np->s.x = (np->posX - gCamera.x) + ((int)((np->offsetX) >> 8));
        np->s.y = (np->posY - gCamera.y) + ((int)((np->offsetY) >> 8));
