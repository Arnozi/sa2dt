# 0 "src/game/water_effects.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/water_effects.c"
...
# 3 "src/game/water_effects.c" 2
# 1 "include/flags.h" 1
# 4 "src/game/water_effects.c" 2
# 1 "include/malloc_vram.h" 1
# 5 "src/game/water_effects.c" 2
# 1 "include/task.h" 1
# 6 "src/game/water_effects.c" 2
...
void InitWaterPalettes(void)
{
    ...
    Water *water = &gWater;
    WaterData *wd = (void *)(0x03000000 + ((water->t)->data));
    ...
    if ((!(((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2))))) {
        ...
        for (j = 0; j < i; j++) {
...

            {
                ...
                CopyPalette((u32 *)wd->pal[j], (u32 *)src, 16u);
            };
        ...
        animation = gAnimations[animId];
        pal = animation[0]->pal.palId;
        CopyPalette((u32 *)wd->pal[0], (u32 *)gSpritePalettes[pal], 16u);

        character = gPlayer.character;
        animId = sCharacterPalettesBoostEffect[character];
        animation = gAnimations[animId];
        pal = animation[0]->pal.palId;
        CopyPalette((u32 *)wd->pal[1], (u32 *)gSpritePalettes[pal], 16u);
    }

    animId = 554;
    animation = gAnimations[animId];
    pal = (animation[0]->pal.palId + 4);
    CopyPalette((u32 *)wd->pal[4], (u32 *)gSpritePalettes[pal], 12 * 16u);

    MaskPaletteWithUnderwaterColor_inline((u32 *)wd->pal[16], (u32 *)&gBgPalette[(0)*16u + (0)], water->mask, 16 * 16u);
}
...
static void Task_StageWaterTask(void)
{
    ...
    if ((gCurrentLevel == ((0)*(3 + 1)) + (0)) && (((int)((gPlayer.qWorldX) >> 8)) > 6665) && (((int)((gPlayer.qWorldX) >> 8)) <= 10650)) {
        water->isActive = 1;
    } else {
        water->isActive = 0;
    }
    ...
    if (water->currentWaterLevel <= cam->y) {
        water->unk2 = 0;
    } else if (water->currentWaterLevel < cam->y + 160) {
...
void CreateRunOnWaterEffect(void)
{
    ...
    RunOnWaterEffect *effect = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &effect->s;
...
static void Task_RunOnWaterEffect(void)
{
    RunOnWaterEffect *effect = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &effect->s;
    Player *p = &gPlayer;
    u32 newMovestate;

    if ((p->moveState & (0x00020000 | 0x00000080)) != 0x00020000) {
        TaskDestroy(gCurTask);
        return;
    }

    effect->x = ((int)((p->qWorldX) >> 8));
...
struct Task *CreateWaterfallSurfaceHitEffect(s32 x, s32 y)
{
    ...
    MultiplayerSpriteTask *ts = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &ts->s;
...
static void sub_8011A4C(void)
{
...

    {
        ...
        if (!unk2) {
            { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&wd->pal[16]); dmaRegs[1] = (vu32)(0x5000000); dmaRegs[2] = (vu32)((u32)((0x8000 | 0x0000 | 0x0400 | 0x0000 | 0x0000) << 16 | ((29 * 16)/(32/8)))); dmaRegs[2]; };
            { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&wd->pal[0]); dmaRegs[1] = (vu32)(((u16 *)(0x5000000 + 0x200))); dmaRegs[2] = (vu32)((u32)((0x8000 | 0x0000 | 0x0400 | 0x0000 | 0x0000) << 16 | ((0x200)/(32/8)))); dmaRegs[2]; };
