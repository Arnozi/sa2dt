# 0 "src/game/special_stage/ui.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/special_stage/ui.c"
# 1 "include/core.h" 1
...
# 2 "src/game/special_stage/ui.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 3 "src/game/special_stage/ui.c" 2
...
# 4 "src/game/special_stage/ui.c" 2
...
# 5 "src/game/special_stage/ui.c" 2
...
# 13 "include/game/special_stage/data.h"
struct UNK_8C878E8 {
    s16 id;
    s16 x;
    s16 y;
...
# 6 "src/game/special_stage/ui.c" 2
# 1 "include/game/special_stage/ui.h" 1
...
# 7 "src/game/special_stage/ui.c" 2
...
# 8 "src/game/special_stage/ui.c" 2
...
# 11 "src/game/special_stage/ui.c" 2
# 1 "include/constants/songs.h" 1
# 12 "src/game/special_stage/ui.c" 2
# 1 "include/constants/text.h" 1
# 13 "src/game/special_stage/ui.c" 2
...
{
    struct SpecialStageUI *ui = (void *)(0x03000000 + ((gCurTask)->data));
...
static void CreateStageTime(struct SpecialStageUI *ui)
{
    ...
    sub_806CA88(&ui->timeSymbol, 0, 1, 0x377, 0x1000, 240 - 48, 14, 1, 0, 0);

    unkF7E8 = &sValueSprites[timeHundreds];
    sub_806CA88(&ui->timeHundreds, 0, unkF7E8->size, unkF7E8->anim, 0x1000, 240 - 36, 18, 1, unkF7E8->variant,
                0);

    unkF7E8 = &sValueSprites[timeTens];
    sub_806CA88(&ui->timeTens, 0, unkF7E8->size, unkF7E8->anim, 0x1000, 240 - 28, 18, 1, unkF7E8->variant, 0);

    unkF7E8 = &sValueSprites[timeUnits];
    sub_806CA88(&ui->timeUnits, 0, unkF7E8->size, unkF7E8->anim, 0x1000, 240 - 20, 18, 1, unkF7E8->variant, 0);

    sub_806CA88(&unused, 0, 0, 0x379, 0, 0, 0, 0, 0, 0);
}
...
static void CreateRingCounter(struct SpecialStageUI *ui)
{
    ...
    sub_806CA88(&ui->ringsHundredsDigit, 0, sprite->size, sprite->anim, 0x1000, (240 / 2) - 20, 16, 1,
                sprite->variant, 0);

    sprite = &sValueSprites[stage->ringsTens];
    sub_806CA88(&ui->ringsTensDigit, 0, sprite->size, sprite->anim, 0x1000, (240 / 2) - 12, 16, 1,
                sprite->variant, 0);

    sprite = &sValueSprites[stage->ringsUnits];
    sub_806CA88(&ui->ringsUnitsDigit, 0, sprite->size, sprite->anim, 0x1000, (240 / 2) - 4, 16, 1,
                sprite->variant, 0);

    sub_806CA88(&ui->unk154, 0, 8, 0x378, 0x1000, (240 / 2), 14, 2, 0, 0);

    sprite = &sValueSprites[stage->ringsTargetHundreds];
    sub_806CA88(&ui->ringsTargetHundredsDigit, 0, sprite->size, sprite->anim, 0x1000, (240 / 2) + 4, 24, 1,
                sprite->variant, 0);

    sprite = &sValueSprites[stage->ringsTargetTens];
    sub_806CA88(&ui->ringsTargetTensDigit, 0, sprite->size, sprite->anim, 0x1000, (240 / 2) + 12, 24, 1,
                sprite->variant, 0);

    sprite = &sValueSprites[stage->ringsTargetUnits];
    sub_806CA88(&ui->ringsTargetUnitsDigit, 0, sprite->size, sprite->anim, 0x1000, (240 / 2) + 20, 24, 1,
                sprite->variant, 0);
}
...
void UpdateRingCounters(void)
{
    ...
    if (stage->targetReached == 0) {
        if (stage->rings >= stage->ringsTarget) {
            stage->targetReached = 1;
            m4aSongNumStart(53);
            stage->unk5C7 = 1;
            stage->unk5C8 = 120;
...
static void HandlePaused(struct SpecialStageUI *ui)
{
    ...
    s16 lang = (gLoadedSaveGame->language - 1);
    ...
    sub_806CA88(s, 1, pauseMenuVariants[lang][0], pauseMenuVariants[lang][1], 0x1000, (240 / 2), (160 / 2), 0,
                pauseMenuVariants[lang][2], 0);

    { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&gObjPalette[(15)*16u + (9)]); dmaRegs[1] = (vu32)(ui->pauseMenuPalette1); dmaRegs[2] = (vu32)((u32)((0x8000 | 0x0000 | 0x0000 | 0x0000 | 0x0000) << 16 | ((sizeof(ui->pauseMenuPalette1))/(16/8)))); dmaRegs[2]; };
    { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&gObjPalette[(15)*16u + (12)]); dmaRegs[1] = (vu32)(ui->pauseMenuPalette2); dmaRegs[2] = (vu32)((u32)((0x8000 | 0x0000 | 0x0000 | 0x0000 | 0x0000) << 16 | ((sizeof(ui->pauseMenuPalette2))/(16/8)))); dmaRegs[2]; };
    ui->wasPaused = 1;
    m4aMPlayAllStop();
    m4aSongNumStart(142);
}
...
static void SpecialStageResultsScreenCreateUI(struct SpecialStageResultsScreen *resultsScreen)
{
    ...
    s16 a = (240 / 2) - 72, b = (240 / 2) - 72, c = (240 / 2) - 72;
    s16 d = (240 / 2) + 38, e = (240 / 2) + 38, f = (240 / 2) + 38;
    s16 i;
    ...
    gSpecialStageSubMenuVramPointer = ((void *)0);

    if (stage->targetReached) {
        sub_806CA88(&resultsScreen->unk4, 1, 0x28, 893, 0, 240 + 16 + a, (160 / 2) - 48, 0, 1, 0);
        sub_806CA88(&resultsScreen->unk34, 1, gUnknown_080DF880[character].size, gUnknown_080DF880[character].anim, 0,
                    240 + 16 + a, (160 / 2) - 56, 0, gUnknown_080DF880[character].variant, 0);
    } else {
        sub_806CA88(&resultsScreen->unk4, 1, 0x20, 893, 0, 240 + 16 + a, (160 / 2) - 48, 0, 0, 0);
    }

    sub_806CA88(&resultsScreen->unk1B4, 1, 0x16, 894, 0, 240 + 16 + a, (160 / 2) - 8, 0, 0,
                0);
    sub_806CA88(&resultsScreen->unk1E4, 1, 0x16, 894, 0, 240 + 16 + b, (160 / 2) + 12, 0,
                1, 0);
    sub_806CA88(&resultsScreen->unk214, 1, 0x16, 894, 0, 240 + 16 + c, (160 / 2) + 32, 0,
                2, 0);

    for (i = 0; i < 5; i++) {
        sub_806CA88(&resultsScreen->unk244[i], 1, sValueSprites[0].size, sValueSprites[0].anim, 0, d + ({ ((i)*8) + 240 + 16; }),
                    (160 / 2) + 8, 0, sValueSprites[0].variant, 0);
    }

    for (i = 0; i < 5; i++) {
        sub_806CA88(&resultsScreen->unk334[i], 1, sValueSprites[0].size, sValueSprites[0].anim, 0, e + ({ ((i)*8) + 240 + 16; }),
                    (160 / 2) + 28, 0, sValueSprites[0].variant, 0);
    }

    for (i = 0; i < 5; i++) {
        sub_806CA88(&resultsScreen->unk424[i], 1, sValueSprites[0].size, sValueSprites[0].anim, 0, f + ({ ((i)*8) + 240 + 16; }),
                    (160 / 2) + 48, 0, sValueSprites[0].variant, 0);
    }

    for (i = 0; i < 7; i++) {
        if (chaosEmeralds & (1 << (i))) {
            sprite = &sChaosEmeraldUnlockedSprites[i];
        } else {
            sprite = &sChaosEmeraldUnlockedSprites[7];
        }

        sub_806CA88(&resultsScreen->chaosEmerald[i], 1, 9, sprite->anim, 0, i * 24 + 240 + 52, (160 / 2) - 28, 0,
                    sprite->variant, 0);
    }

    resultsScreen->animFrame = 0;
    resultsScreen->unk516 = 240 + 16;
    resultsScreen->unk518 = 240 + 16;
    resultsScreen->unk51A = 240 + 16;
    resultsScreen->unk51C = 240 + 16;
    resultsScreen->unk51E = 240 + 16;
}

void Task_ResultsScreenSequencePart1(void)
{
    struct SpecialStageResultsScreen *resultsScreen = (void *)(0x03000000 + ((gCurTask)->data));
...
void sub_807061C(s16 a)
{
    ...
    if (stage->targetReached) {
        s->x = resultsScreen->unk516 + (240 / 2) - 44;
        s = &resultsScreen->unk34;
        s->x = resultsScreen->unk516 + (240 / 2) - 116;
    } else {
        s->x = resultsScreen->unk516 + (240 / 2) - 72;
    }
}
...
void sub_8070680(s16 a)
{
    ...
    for (i = 0; i < 7; i++) {
        ...
        s32 x = (i * 24);
        s32 base = resultsScreen->unk518 + (240 / 2) - 84;
        x += base;
        s->x = x;
    }
}
...
void sub_80706D8(s16 a)
{
    ...
    s->x = resultsScreen->unk51A + (240 / 2) - 72;
    ...
    for (i = 0; i < 5; i++) {
        ...
        temp2 = (i * 8);
        temp = resultsScreen->unk51A + (240 / 2) + 38;
        s->x = temp2 + temp;
    }
}
...
void sub_8070740(s16 a)
{
    ...
    s->x = resultsScreen->unk51C + (240 / 2) - 72;

    for (i = 0; i < 5; i++) {
        s = &resultsScreen->unk334[i];
        temp2 = (i * 8);
        temp = resultsScreen->unk51C + (240 / 2) + 38;
        s->x = temp2 + temp;
    }
}
...
void sub_80707A8(s16 xPos)
{
    ...
    s->x = resultsScreen->unk51E + (240 / 2) - 72;

    for (i = 0; i < 5; i++) {
        s = &resultsScreen->unk424[i];
        temp2 = (i * 8);
        temp = resultsScreen->unk51E + (240 / 2) + 38;
        s->x = temp2 + temp;
    }
}
...
struct Task *CreateSpecialStageUI(struct SpecialStage *stage)
{
    ...
    struct SpecialStageUI *ui = (void *)(0x03000000 + ((t)->data));
    ui->stage = stage;
    ui->unk2A4 = ((void *)0);
...
struct Task *CreateSpecialStageResultsScreen(struct SpecialStage *stage)
{
    ...
    struct SpecialStageResultsScreen *resultsScreen = (void *)(0x03000000 + ((t)->data));
...
static void CreateMultiplierValue(struct SpecialStageUI *ui)
{
    ...
    sub_806CA88(&ui->multiplier, 0, unkF840->size, unkF840->anim, 0x1000, (240 / 2) - 8, (160 / 2) + 20, 2,
                unkF840->variant, 0);
}
...
void RenderMultiplier(void)
{
    struct SpecialStageUI *ui = (void *)(0x03000000 + ((gCurTask)->data));
    struct SpecialStage *stage = ui->stage;
    struct SpecialStagePlayer *player = (void *)(0x03000000 + ((stage->playerTask)->data));
...
void CreateStartText(struct SpecialStageUI *ui)
{
    ...
    sub_806CA88(s, 0, 20, 0x376, 0x1000, 240 / 2, 160 / 2, 1, 0, 0);
}
...
static void RenderPauseMenu(struct SpecialStageUI *ui)
{
    ...
    if (stage->pauseMenuCursor == 0) {
        b = (void *)((u16 *)(0x5000000 + 0x200)) + 0x1F8;
        a = (void *)((u16 *)(0x5000000 + 0x200)) + 0x1F2;
    } else {
        b = (void *)((u16 *)(0x5000000 + 0x200)) + 0x1F2;
        a = (void *)((u16 *)(0x5000000 + 0x200)) + 0x1F8;
    }

    { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(ui->pauseMenuPalette2); dmaRegs[1] = (vu32)(b); dmaRegs[2] = (vu32)((u32)((0x8000 | 0x0000 | 0x0000 | 0x0000 | 0x0000) << 16 | ((sizeof(ui->pauseMenuPalette2))/(16/8)))); dmaRegs[2]; };
    { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(ui->pauseMenuPalette1); dmaRegs[1] = (vu32)(a); dmaRegs[2] = (vu32)((u32)((0x8000 | 0x0000 | 0x0000 | 0x0000 | 0x0000) << 16 | ((sizeof(ui->pauseMenuPalette1))/(16/8)))); dmaRegs[2]; };
    DisplaySprite(s);
}
