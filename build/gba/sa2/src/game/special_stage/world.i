...
struct Task *CreateSpecialStageWorld(struct SpecialStage *stage)
{
    ...
    world = (void *)(0x03000000 + ((t)->data));
    world->stage = stage;

    world->bgTransforms = ((void *)0);
    world->unk8 = ((void *)0);
    world->qPerspectiveTable = ((void *)0);
...
void sub_806E7C0(struct SpecialStageWorld *world)
{
    ...
    world->qPerspectiveTable = EwramMalloc(160 * sizeof(s32));
    for (i = 0, unkC = world->qPerspectiveTable; i < 160; i++, unkC++) {
    ...
    world->unk8 = EwramMalloc(160 * sizeof(s32));
    for (i = 0, unk8 = world->unk8; i < 160; i++, unk8++) {
        s32 temp2 = (i - stage->cameraPitch);
        if (temp == 0) {
            *unk8 = 0;
        } else {
    ...
    for (i = stage->horizonHeight; i < 160; i++) {
    ...
    world->bgTransforms = EwramMalloc(160 * sizeof(BgAffineReg));
...
void sub_806EA04(void)
{
    struct SpecialStageWorld *world = (void *)(0x03000000 + ((gCurTask)->data));
    struct SpecialStage *stage = world->stage;
    s32 sin, cos;
    s16 worldScale;
    s32 unk5A0;
...
{
    struct SpecialStageWorld *world = (void *)(0x03000000 + ((t)->data));

    if (world->unk8 != ((void *)0)) {
        EwramFree(world->unk8);
    }

    if (world->qPerspectiveTable != ((void *)0)) {
        EwramFree(world->qPerspectiveTable);
    }

    if (world->bgTransforms != ((void *)0)) {
