# 0 "src/game/stage/results.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/stage/results.c"
...
# 3 "src/game/stage/results.c" 2
# 1 "include/malloc_vram.h" 1
# 4 "src/game/stage/results.c" 2
# 1 "include/sprite.h" 1
# 5 "src/game/stage/results.c" 2
# 1 "include/task.h" 1
# 6 "src/game/stage/results.c" 2
...
# 10 "src/game/stage/results.c" 2
# 1 "include/game/cutscenes/endings.h" 1
...
# 19 "src/game/stage/results.c" 2
# 1 "include/constants/char_states.h" 1
# 20 "src/game/stage/results.c" 2
# 1 "include/constants/songs.h" 1
# 21 "src/game/stage/results.c" 2


typedef struct {
    StageResultsBase base;
                bool8 isCountingDone;
} StageResults;
...
const u16 gAnimsGotThroughZoneAndActNames[][3] = {
    { 14, 1123, 0 }, { 14, 1123, 1 },
    { 18, 1123, (4 + (0)) }, { 18, 1123, (4 + (1)) },
    { 18, 1123, (4 + (2)) }, { 18, 1123, (4 + (3)) },
    { 18, 1123, (4 + (4)) }, { 18, 1123, (4 + (5)) },
    { 18, 1123, (4 + (6)) }, { 16, 1123, 2 },
    { 16, 1123, 3 },
};
...
u16 CreateStageResults(u32 courseTime, u16 ringCount, u8 spRingCount)
{
    ...
    } else if (courseTime < (int)(((6 * 60.) + 0) * 60)) {
...

    {
        s8 extraLevel;
        u8 level;
        bool32 isBossAct = ((gCurrentLevel) & ((3 + 1) - 1)) >> 1;
        ...
        extraLevel = gCurrentLevel;
        if (((extraLevel) == ((7)*(3 + 1)) + (0)) || ((gCurrentLevel) == ((7)*(3 + 1)) + (1))) {
    ...
    for (i = 0; i < (sizeof(outro->base.sprScores) / sizeof((outro->base.sprScores)[0])); i++) {
    ...
    if (((gCurrentLevel) == ((7)*(3 + 1)) + (0))) {
        ...
        m4aSongNumStart(404);
    } else if (((gCurrentLevel) & ((3 + 1) - 1)) == 2) {
        m4aSongNumStart(402);

        if (gPlayer.charState == 9) {
            gPlayer.charState = 32;
        }
...
static void Task_UpdateStageResults(void)
{
    StageResults *outro = (void *)(0x03000000 + ((gCurTask)->data));
    ...
    if (((gCurrentLevel) == ((7)*(3 + 1)) + (1))) {
        gBldRegs.bldCnt = (((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11) | (1 << 12) | (1 << 13)) | (2 << 6) | (1 << 5) | (1 << 0) | (1 << 1) | (1 << 2)
                           | (1 << 3));
    }
    ...
    if (counter >= 150) {
        if (outro->base.ringBonusScore != 0) {
            ...
            { s32 divResA, divResB; s32 oldScore = gLevelScore; gLevelScore += 100; divResA = Div(gLevelScore, 50000); divResB = Div(oldScore, 50000); if ((divResA != divResB) && (gGameMode == 0)) { u16 lives = divResA - divResB; lives += gNumLives;; ({ if ((lives) > 255) (lives) = 255; gNumLives = (lives); }); if (0) gMusicManagerState.unk3 = 0x10 | 0x0; } };
        }
        ...
        if (!((((gCurrentLevel) == ((7)*(3 + 1)) + (0))) || (((gCurrentLevel) == ((7)*(3 + 1)) + (1))))) {
            if (gPressedKeys & 0x0001) {
                { s32 divResA, divResB; s32 oldScore = gLevelScore; gLevelScore += outro->base.ringBonusScore; divResA = Div(gLevelScore, 50000); divResB = Div(oldScore, 50000); if ((divResA != divResB) && (gGameMode == 0)) { u16 lives = divResA - divResB; lives += gNumLives;; ({ if ((lives) > 255) (lives) = 255; gNumLives = (lives); }); if (0) gMusicManagerState.unk3 = 0x10 | 0x0; } };
                { s32 divResA, divResB; s32 oldScore = gLevelScore; gLevelScore += outro->base.spRingBonusScore; divResA = Div(gLevelScore, 50000); divResB = Div(oldScore, 50000); if ((divResA != divResB) && (gGameMode == 0)) { u16 lives = divResA - divResB; lives += gNumLives;; ({ if ((lives) > 255) (lives) = 255; gNumLives = (lives); }); if (0) gMusicManagerState.unk3 = 0x10 | 0x0; } };
                { s32 divResA, divResB; s32 oldScore = gLevelScore; gLevelScore += outro->base.timeBonusScore; divResA = Div(gLevelScore, 50000); divResB = Div(oldScore, 50000); if ((divResA != divResB) && (gGameMode == 0)) { u16 lives = divResA - divResB; lives += gNumLives;; ({ if ((lives) > 255) (lives) = 255; gNumLives = (lives); }); if (0) gMusicManagerState.unk3 = 0x10 | 0x0; } };
    ...
    if (counter > outro->base.unk16C + 309) {
        if (((gCurrentLevel) == ((7)*(3 + 1)) + (0))) {
            if ((gMPlayInfo_BGM.status & 0xFFFF) == 0) {
                (gLoadedSaveGame)->unlockedLevels[gSelectedCharacter] = ((7)*(3 + 1)) + (1);
                WriteSaveGame();
                ...
                if (((gCurrentLevel) & ((3 + 1) - 1)) == 2) {
                    ...
                    if (gCurrentLevel > (gLoadedSaveGame)->unlockedLevels[gSelectedCharacter]) {
                        ...
                        if (gSelectedCharacter == CHARACTER_SONIC) {
                            switch (((gCurrentLevel - 1) >> 2)) {
                                case 0: {
                                    (gLoadedSaveGame)->unlockedCharacters |= (1 << (CHARACTER_CREAM));
                                    CreateCharacterUnlockCutScene(0);
                                } break;

                                case 2: {
                                    (gLoadedSaveGame)->unlockedCharacters |= (1 << (CHARACTER_TAILS));
                                    CreateCharacterUnlockCutScene(2);
                                } break;

                                case 4: {
                                    (gLoadedSaveGame)->unlockedCharacters |= (1 << (CHARACTER_KNUCKLES));
                    ...
                    if ((gPlayer.moveState & 0x08000000) && (gSpecialRingCount >= 7)) {
                        ...
                        gWinRegs[5] |= ((((1 << 0) | (1 << 1) | (1 << 2) | (1 << 3)) | (1 << 4) | (1 << 5)) & ~(1 << 5));

                        gBldRegs.bldCnt = ((2 << 6) | ((1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4) | (1 << 5)) | ((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11) | (1 << 12) | (1 << 13)));
                        gBldRegs.bldY = 0x10;
...
static void AnimateResults(u16 frame)
{
    ...
    if (counter > 28) {
        u32 numDisplayedBonuses = (((gCurrentLevel) & ((3 + 1) - 1)) == 2) ? 2 : 3;
    ...
    if (counter >= 39) {
...

        {
            s16 r4_2 = r4;
            s16 pp = frame;
            StageUI_PrintIntegerAt(outro->base.timeBonusScore, r4_2 + 144 - pp, (160 / 2) + 10, 0);
        }
    }
    ...
    if (counter >= 49) {
...

        {
            s16 r4_2 = r4;
            s16 pp = frame;
            StageUI_PrintIntegerAt(outro->base.ringBonusScore, r4_2 + 144 - pp, (160 / 2) + 30, 0);
        }
    }

    if ((((gCurrentLevel) & ((3 + 1) - 1)) != 2) && (gCurrentLevel < ((7)*(3 + 1)) + (0)) && (counter >= 59)) {
        s = &outro->base.sprScores[2];

        if (counter <= 75) {
            u16 innerX = 240 - ((counter - 59) * 12);
            r4 = innerX;
        } else {
            r4 = (240 / 2) - 72;
        }
...

        {
            s16 r4_2 = r4;
            s16 pp = frame;
            StageUI_PrintIntegerAt(outro->base.spRingBonusScore, r4_2 + 144 - pp, (160 / 2) + 50, 0);
        }
    }
}
...
void StageResults_AnimateTitle(void)
{
    ...
    if (counter > 28) {
...

        {
            s32 i = 0;
            s32 x2 = (s16)x;
            for (; i < (sizeof(sStageResultsTextOffset) / sizeof((sStageResultsTextOffset)[0])); i++) {
...
{
    StageResults *outro = (void *)(0x03000000 + ((t)->data));
    if (outro->base.separator.graphics.dest != ((void *)0)) {
...
static void DestroyStageResultsGfx(void)
{
    ...
    if (outro->base.separator.graphics.dest != ((void *)0)) {
        ...
        outro->base.separator.graphics.dest = ((void *)0);
    }
}
