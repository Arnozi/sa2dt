# 0 "src/game/countdown.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/countdown.c"
...
# 3 "src/game/countdown.c" 2
# 1 "include/sprite.h" 1
# 4 "src/game/countdown.c" 2
# 1 "include/task.h" 1
# 5 "src/game/countdown.c" 2
# 1 "include/malloc_vram.h" 1
# 6 "src/game/countdown.c" 2
...
# 15 "src/game/countdown.c" 2
# 1 "include/constants/char_states.h" 1
# 16 "src/game/countdown.c" 2
# 1 "include/constants/songs.h" 1
# 17 "src/game/countdown.c" 2
# 1 "include/constants/text.h" 1
# 18 "src/game/countdown.c" 2
# 34 "src/game/countdown.c"
typedef struct {
    Sprite sprMachine;
...
const TileInfo gUnknown_080D7518[NUM_CHARACTERS] = {
    { .numTiles = (0), .anim = (((30) + CHARACTER_SONIC * (91))), .variant = (1) },

    { .numTiles = (0), .anim = (((30) + CHARACTER_CREAM * (91))), .variant = (1) },

    { .numTiles = (0), .anim = (((30) + CHARACTER_TAILS * (91))), .variant = (1) },

    { .numTiles = (0), .anim = (((30) + CHARACTER_KNUCKLES * (91))), .variant = (1) },

    { .numTiles = (0), .anim = (((30) + CHARACTER_AMY * (91))), .variant = (1) },
};
...
void CreateCourseStartCountdown(bool8 playerSkippedIntro)
{
    ...
    countdown = (void *)(0x03000000 + ((t)->data));

    countdown->speedBoost = 0;
    countdown->boostDisabled = 0;

    if (!playerSkippedIntro) {
        countdown->timer = 60 * 5 + 10;
    } else {
        countdown->timer = 60 * 3;
    }

    s = &countdown->sprCountdownDigits;
    s->graphics.dest = VramMalloc(4);
    s->graphics.anim = 1129;
    s->variant = 0;
    s->prevVariant = -1;
    s->oamFlags = ((4) << 6);
    s->graphics.size = 0;
    ...
    s->graphics.dest = VramMalloc(14); s->graphics.anim = 530; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (((1) << (10))));;;
}

void sub_8036168(void)
{
    CourseStartCountdown *countdown = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s;


    if (((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2)) && countdown->timer > (60 * 3) && gPressedKeys & (0x0001 | 0x0002)) {
        countdown->timer = 60 * 3;
    }

    if (countdown->timer == 60 * 3) {
        m4aSongNumStart(208);
    } else if (countdown->timer == 60 * 2) {
        m4aSongNumStart(209);
    } else if (countdown->timer == 60 * 1) {
        m4aSongNumStart(210);
    }
    ...
    if (--countdown->timer == 0) {
        ...
        if (countdown->speedBoost) {
            gPlayer.qSpeedGround = ((s16)((9)*256));
        } else {
            gPlayer.qSpeedGround = ((s16)((4)*256));
        }

        sub_8018818();
        CreateRaceStartMessage();
        countdown->machineScreenX = ((int)((gPlayer.qWorldX) >> 8));
        countdown->machineScreenY = ((int)((gPlayer.qWorldY) >> 8));
        m4aSongNumStart(211);
        gCurTask->main = sub_8036398;
    } else if (countdown->timer < 5) {
        if (gPressedKeys & 0x0010 && countdown->boostDisabled == 0) {
            countdown->speedBoost = 1;
        }
    } else {
        if (gPressedKeys & 0x0010) {
            countdown->boostDisabled = 1;
        }
    }
    ...
    if (countdown->timer < (60 * 3)) {
        s = &countdown->sprCountdownDigits;
        s->variant = 2 - Div(countdown->timer, 60);
        s->prevVariant = -1;
        s->x = ((((int)((gPlayer.qWorldX) >> 8)) - gCamera.x) + 24);
        s->y = ((((int)((gPlayer.qWorldY) >> 8)) - gCamera.y) - 24);
        UpdateSpriteAnimation(s);
        DisplaySprite(s);
    }

    if (countdown->timer >= (int)((1 + 1. / 6.) * 60) && countdown->timer < 3 * 60) {
        ...
        if ((!(((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2))))) {
            gPlayer.spriteInfoBody->s.palId = (((volatile struct SioMultiCnt *)(0x4000000 + 0x128)))->id;
        } else {
...
void CreateRaceStartMessage(void)
{
    ...
    RaceStartMessage *startMessage = (void *)(0x03000000 + ((t)->data));
    Sprite *s;

    startMessage->timer = 60;
    s = &startMessage->sLeft;
    s->graphics.dest = VramMalloc(64);
    s->graphics.anim = 1130;
    s->variant = 0;
    s->prevVariant = -1;
    s->oamFlags = ((4) << 6);
    s->graphics.size = 0;
    ...
    s->palId = 0;
    s->hitboxes[0].index = -1;
    s->frameFlags = gOamMatrixIndex++ | ((1) << (5)) | ((1) << (6));
...
void Task_RaceStartMessageMain(void)
{
    RaceStartMessage *startMessage = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s, *element2;
    SpriteTransform *transform;
    s16 timer;
    startMessage->timer--;
    ...
    if (timer < 16) {
        transform->qScaleX = ((s32)((2)*256)) - startMessage->timer * 16;
...
{
    CourseStartCountdown *countdown = (void *)(0x03000000 + ((t)->data));
