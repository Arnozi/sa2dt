# 0 "src/game/enemies/kyura.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/kyura.c"
...
# 3 "src/game/enemies/kyura.c" 2
# 1 "include/sprite.h" 1
# 4 "src/game/enemies/kyura.c" 2
# 1 "include/malloc_vram.h" 1
# 5 "src/game/enemies/kyura.c" 2
# 1 "include/trig.h" 1
...
typedef struct {
    ...
    u8 unk58;
    u8 unk59;
    u8 framesUntilTaskSwitch;
    u8 framesUntilProjectile;
} Sprite_Kyura;
...
void CreateEntity_Kyura(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_Kyura *kyura = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &kyura->s;
    ...
    kyura->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256));
    kyura->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));
    kyura->offsetX = 0;
    kyura->offsetY = 0;
    kyura->unk54 = 0;
    ...
    s->graphics.dest = VramMalloc(24); s->graphics.anim = 509; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
}

void Task_KyuraMain(void)
{
    Sprite_Kyura *kyura = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &kyura->s;
    ...
    kyura->offsetX = ((gSineTable[(({ register u32 r2 asm("r2") = 0x100; asm("" ::"r"(r2)); r0 + r2; }))])
    ...
    kyura->offsetY = (((gSineTable[((unk54 * 3) & r5)])) * kyura->unk59) >> 6;

    pos.x = ((int)((kyura->spawnX + kyura->offsetX) >> 8)); pos.y = ((int)((kyura->spawnY + kyura->offsetY) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;

    if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; };

    if (((((int)((kyura->spawnX) >> 8))) > gCamera.x + (240 + (256 / 2)) || (((int)((kyura->spawnX) >> 8))) < gCamera.x - (256 / 2) || (((int)((kyura->spawnY) >> 8))) > gCamera.y + ((160) + ((256) / 2)) || (((int)((kyura->spawnY) >> 8))) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = kyura->base.meX; }; TaskDestroy(gCurTask); return; }; };
    ...
    if (--kyura->framesUntilTaskSwitch == 0) {
        ...
        if (kyura->framesUntilProjectile-- == 1) {
            ...
            randomBool = ({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 1;
            init.numTiles = 3;
            init.anim = 510;
            init.variant = randomBool;
            init.x = ((s32)((pos.x + 1)*256));
            init.y = ((s32)((pos.y + 20)*256));
            init.rot = ((s32)((1.0)*256));
            init.speed = ((s32)((2.0)*256)) - (((s32)((1.0)*256)) * randomBool);
...
void Task_KyuraRecover(void)
{
    ...
    if (--kyura->framesUntilTaskSwitch == 0) {
        kyura->framesUntilTaskSwitch = 8;
        kyura->unk54 = (kyura->unk54 + 8) & (1024 - 1);
