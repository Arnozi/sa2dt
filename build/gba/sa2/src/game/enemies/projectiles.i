# 0 "src/game/enemies/projectiles.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/projectiles.c"
...
# 2 "src/game/enemies/projectiles.c" 2
...
# 3 "src/game/enemies/projectiles.c" 2
...
# 4 "src/game/enemies/projectiles.c" 2


# 1 "include/malloc_vram.h" 1
# 7 "src/game/enemies/projectiles.c" 2
# 1 "include/task.h" 1
# 8 "src/game/enemies/projectiles.c" 2
# 1 "include/trig.h" 1
...
# 9 "src/game/enemies/projectiles.c" 2
...
void CreateProjectile(ProjInit *init)
{
    ...
    ProjectileA *proj = (void *)(0x03000000 + ((t)->data));
    Sprite *s;
    ...
    velocityX = ((gSineTable[(init->rot) + 256]) * init->speed) >> 14;
    proj->velocityX = velocityX;

    velocityY = ((gSineTable[(init->rot)]) * init->speed) >> 14;
    proj->velocityY = velocityY;

    s = &proj->s;
    s->graphics.dest = VramMalloc(init->numTiles); s->graphics.anim = init->anim; s->variant = init->variant; s->oamFlags = ((8) << 6); s->graphics.size = 0;; s->x = 0; s->y = 0; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1; s->frameFlags = ((1) << (12));;
}

void CreateSeveralProjectiles(ProjInit *init, u8 count, s8 spreadAngle)
{
    struct Task *t = TaskCreate(Task_80510B0, sizeof(ProjectileB), 0x4000, 0, TaskDestructor_8051200);
    ProjectileB *proj = (void *)(0x03000000 + ((t)->data));
    Sprite *s;
    u8 i;
    ...
    for (i = 0; i < count; i++) {
        ...
        proj->velocities[i][0] = rot = ((i * spreadAngle) + init->rot) & (1024 - 1);

        proj->velocities[i][0] = ((gSineTable[(rot) + 256]) * init->speed) >> 14;
        proj->velocities[i][1] = ((gSineTable[(rot)]) * init->speed) >> 14;
...
void Task_805102C(void)
{
    ProjectileA *proj = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &proj->s;

    proj->x += proj->velocityX;
    proj->y += proj->velocityY;

    s->x = ((int)((proj->x) >> 8)) - gCamera.x;
    s->y = ((int)((proj->y) >> 8)) - gCamera.y;

    if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) {
        TaskDestroy(gCurTask);
    } else {
        Coll_Player_Projectile(s, ((int)((proj->x) >> 8)), ((int)((proj->y) >> 8)));
...
void Task_80510B0(void)
{
    ProjectileB *proj = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &proj->s;
    u8 count, i;
    ...
    for (i = 0; i < proj->count; i++) {
        ...
        s->x = ((int)((proj->positions[i].x) >> 8)) - gCamera.x;
        s->y = ((int)((proj->positions[i].y) >> 8)) - gCamera.y;

        if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) {
            proj->isActive[i] = 0;
        }

        Coll_Player_Projectile(s, ((int)((proj->positions[i].x) >> 8)), ((int)((proj->positions[i].y) >> 8)));
        DisplaySprite(s);
    }
