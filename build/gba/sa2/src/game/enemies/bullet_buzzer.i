# 0 "src/game/enemies/bullet_buzzer.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/bullet_buzzer.c"
...
# 2 "src/game/enemies/bullet_buzzer.c" 2
...
void VramFree(void *);
# 3 "src/game/enemies/bullet_buzzer.c" 2
...
# 5 "src/game/enemies/bullet_buzzer.c" 2
...
# 9 "src/game/enemies/bullet_buzzer.c" 2
...
# 11 "src/game/enemies/bullet_buzzer.c" 2
...
# 13 "src/game/enemies/bullet_buzzer.c" 2
...
void CreateEntity_BulletBuzzer(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_BulletBuzzer *bbuzzer = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &bbuzzer->s;
    ...
    bbuzzer->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); bbuzzer->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));; bbuzzer->offsetX = 0; bbuzzer->offsetY = 0;;

    bbuzzer->unk54 = ((s32)((0.0)*256));
    ...
    s->graphics.anim = 505; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (((1) << (10))));;
}
...
void Task_BulletBuzzerMain(void)
{
    Sprite_BulletBuzzer *bbuzzer = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &bbuzzer->s;
    ...
    bbuzzer->unk54++;
    bbuzzer->unk54 = ({ s16 clamped = (bbuzzer->unk54) & (1024 - 1); clamped; });

    index = ({ s16 clamped = (bbuzzer->unk54 * 5) & (1024 - 1); clamped; });
    bbuzzer->offsetX = ((int)(((gSineTable[(index) + 256]) * bbuzzer->unk5A) >> 6));

    index = ({ s16 clamped = (bbuzzer->unk54 * 3) & (1024 - 1); clamped; });
    bbuzzer->offsetY = ((int)(((gSineTable[(index)]) * bbuzzer->unk5B) >> 6));

    pos.x = ((int)((bbuzzer->spawnX + bbuzzer->offsetX) >> 8)); pos.y = ((int)((bbuzzer->spawnY + bbuzzer->offsetY) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;
    ...
    if (((((int)((bbuzzer->spawnX) >> 8))) > gCamera.x + (240 + (256 / 2)) || (((int)((bbuzzer->spawnX) >> 8))) < gCamera.x - (256 / 2) || (((int)((bbuzzer->spawnY) >> 8))) > gCamera.y + ((160) + ((256) / 2)) || (((int)((bbuzzer->spawnY) >> 8))) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = bbuzzer->base.meX; }; TaskDestroy(gCurTask); return; }; };
    value = sub_8004418(((int)((gPlayer.qWorldY) >> 8)) - pos.y, ((int)((gPlayer.qWorldX) >> 8)) - pos.x);

    if (bbuzzer->unk5E == 0) {
        if (((u16)(value - 86) < 84 && s->frameFlags & ((1) << (10)))
            || ((u16)(value - 342) < 84 && !(s->frameFlags & ((1) << (10))))) {
            bbuzzer->unk58 = value;
            bbuzzer->unk5D = 0;
            s->graphics.anim = 505;
            s->variant = 2;
...
void sub_8059B04(void)
{
    ...
    if (++bbuzzer->unk5D == 34) {
        ...
        if (s->frameFlags & ((1) << (10))) {
            init.x = ((s32)((pos.x + 10) << 8));
        } else {
            init.x = ((s32)((pos.x - 10) << 8));
        }

        init.y = ((s32)((pos.y + 14) << 8));
        init.rot = bbuzzer->unk58 - 16;
        init.speed = ((s32)((2)*256));
