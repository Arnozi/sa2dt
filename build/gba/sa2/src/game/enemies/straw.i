# 0 "src/game/enemies/straw.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/straw.c"
...
# 3 "src/game/enemies/straw.c" 2
# 1 "include/malloc_vram.h" 1
# 4 "src/game/enemies/straw.c" 2
# 1 "include/sprite.h" 1
# 5 "src/game/enemies/straw.c" 2
# 1 "include/trig.h" 1
...
void CreateEntity_Straw(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    if ((gGameMode == 1 || gDifficultyLevel != 1)) {
        ...
        Sprite_Straw *straw = (void *)(0x03000000 + ((t)->data));
        Sprite *s = &straw->s;
        ...
        straw->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); straw->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));; straw->offsetX = 0; straw->offsetY = 0;;

        rand = ({ s16 clamped = (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; })) & (1024 - 1); clamped; });
        straw->unk4C = (gSineTable[(rand) + 256]) >> 5;
        straw->unk50 = (gSineTable[(rand)]) >> 5;
        straw->unk54 = 100;
        straw->unk55 = 5;
        ...
        s->graphics.dest = VramMalloc(12); s->graphics.anim = 504; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((1) << (12)) | (0));;;
    }
}

void sub_80567F8(void)
{
    Sprite_Straw *straw = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &straw->s;
    MapEntity *me = straw->base.me;
    Vec2_32 pos = {
        .x = ((int)((straw->spawnX + straw->offsetX) >> 8)),
        .y = ((int)((straw->spawnY + straw->offsetY) >> 8)),
    };
    ...
    if (((((int)((straw->spawnX) >> 8))) > gCamera.x + (240 + (256 / 2)) || (((int)((straw->spawnX) >> 8))) < gCamera.x - (256 / 2) || (((int)((straw->spawnY) >> 8))) > gCamera.y + ((160) + ((256) / 2)) || (((int)((straw->spawnY) >> 8))) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = straw->base.meX; }; TaskDestroy(gCurTask); return; }; };

    if (--straw->unk54 == 0) {
        s32 rand = ({ s16 clamped = (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; })) & (1024 - 1); clamped; });
        straw->unk4C = (gSineTable[(rand) + 256]) >> 5;
        straw->unk50 = (gSineTable[(rand)]) >> 5;
        straw->unk54 = 100;
...
void Task_StrawMain(void)
{
    ...
    pos.x = ((int)((straw->spawnX + straw->offsetX) >> 8)); pos.y = ((int)((straw->spawnY + straw->offsetY) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;

    if (gPlayer.qWorldX < ((s32)((pos.x) << 8))) {
        straw->unk4C -= 0x10;
    } else {
        straw->unk4C += 0xB;
    }

    if (gPlayer.qWorldY < ((s32)((pos.y) << 8))) {
        straw->unk50 -= 0x10;
    } else {
        straw->unk50 += 0xB;
    }
    ...
    Player_UpdateHomingPosition(((s32)((pos.x) << 8)), ((s32)((pos.y) << 8))); { if (gPlayer.qWorldX < ((s32)((pos.x)*256))) { (s)->frameFlags &= ~(((1) << (10))); } else { (s)->frameFlags |= (((1) << (10))); } }; UpdateSpriteAnimation(s); DisplaySprite(s);;
}
