# 0 "src/game/enemies/bell.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/bell.c"
...
# 3 "src/game/enemies/bell.c" 2
# 1 "include/sprite.h" 1
# 4 "src/game/enemies/bell.c" 2
# 1 "include/task.h" 1
# 5 "src/game/enemies/bell.c" 2
# 1 "include/malloc_vram.h" 1
# 6 "src/game/enemies/bell.c" 2
...
void CreateEntity_Bell(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_Bell *bell = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &bell->s;
    bell->unk4C = 120;
    ...
    bell->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256));
    bell->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));
    ...
    s->graphics.dest = VramMalloc(20); s->graphics.anim = 493; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
}

static void Task_BellMain(void)
{
    Sprite_Bell *bell = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &bell->s;
    MapEntity *me = bell->base.me;

    Vec2_32 pos;

    pos.x = ((int)((bell->spawnX + 0) >> 8)); pos.y = ((int)((bell->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;
    ...
    if (bell->unk4C == 0) {
        if (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 1) {
            bell->unk4C = 124;
        } else {
            bell->unk4C = 180;
        }
    ...
    Player_UpdateHomingPosition(bell->spawnX, bell->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);;
}
