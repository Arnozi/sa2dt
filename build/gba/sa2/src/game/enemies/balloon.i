# 0 "src/game/enemies/balloon.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/balloon.c"
...
# 2 "src/game/enemies/balloon.c" 2
...
void VramFree(void *);
# 3 "src/game/enemies/balloon.c" 2
...
# 6 "src/game/enemies/balloon.c" 2
...
# 7 "src/game/enemies/balloon.c" 2
...
# 9 "src/game/enemies/balloon.c" 2
...
# 11 "src/game/enemies/balloon.c" 2
...
void CreateEntity_Balloon(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    if ((gGameMode == 1 || gDifficultyLevel != 1)) {
        ...
        Sprite_Balloon *balloon = (void *)(0x03000000 + ((t)->data));
        Sprite *s = &balloon->s;
        ...
        balloon->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); balloon->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));; balloon->offsetX = 0; balloon->offsetY = 0;;

        balloon->unk54 = ((s32)((0.5)*256));
        balloon->unk58 = ((s32)((0.0)*256));
        balloon->unk5E = 120;
        ...
        s->graphics.dest = VramMalloc(10); s->graphics.anim = 501; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
    }
}
...
void Task_BalloonMain(void)
{
    Sprite_Balloon *balloon = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &balloon->s;
    ...
    idx = ({ s16 clamped = (balloon->unk58 * 5) & (1024 - 1); clamped; });
    balloon->offsetX = ((gSineTable[(idx) + 256]) * balloon->unk5C) >> 6;
    idx = ({ s16 clamped = (balloon->unk58 * 3) & (1024 - 1); clamped; });
    balloon->offsetY = ((gSineTable[(idx)]) * balloon->unk5D) >> 6;

    pos.x = ((int)((balloon->spawnX + balloon->offsetX) >> 8)); pos.y = ((int)((balloon->spawnY + balloon->offsetY) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;
    if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; };
    if (((((int)((balloon->spawnX) >> 8))) > gCamera.x + (240 + (256 / 2)) || (((int)((balloon->spawnX) >> 8))) < gCamera.x - (256 / 2) || (((int)((balloon->spawnY) >> 8))) > gCamera.y + ((160) + ((256) / 2)) || (((int)((balloon->spawnY) >> 8))) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = balloon->base.meX; }; TaskDestroy(gCurTask); return; }; };

    if (((((int)((balloon->offsetX) >> 8)) <= me->d.sData[0] * 8))) {
        balloon->unk54 = 0x80;
    } else if (((((int)((balloon->offsetX) >> 8)) >= (me->d.sData[0] * 8 + me->d.uData[2] * 8)))) {
        balloon->unk54 = -0x80;
    }
...
void sub_805879C(void)
{
    ...
    if (balloon->unk5E == 45) {
        ...
        proj.anim = 502;
        proj.variant = 0;
        proj.x = ((s32)((pos.x + 1) << 8));
        proj.y = ((s32)((pos.y + 20) << 8));
        proj.rot = (u8)-1;
        proj.speed = 512;
