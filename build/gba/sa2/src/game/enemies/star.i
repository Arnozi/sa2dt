# 0 "src/game/enemies/star.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/star.c"
...
# 4 "src/game/enemies/star.c" 2
# 1 "include/malloc_vram.h" 1
# 5 "src/game/enemies/star.c" 2
...
void CreateEntity_Star(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    star->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); star->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));;
    ...
    s->graphics.dest = VramMalloc(25); s->graphics.anim = 511; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
}
# 78 "src/game/enemies/star.c"
static void Task_StarIdle(void) { { Sprite_Star *star = (void *)(0x03000000 + ((gCurTask)->data)); Sprite *s = &star->s; MapEntity *me = star->base.me; Vec2_32 pos; pos.x = ((int)((star->spawnX + 0) >> 8)); pos.y = ((int)((star->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;; { { if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; } } }; if (((pos.x) > gCamera.x + (240 + (256 / 2)) || (pos.x) < gCamera.x - (256 / 2) || (pos.y) > gCamera.y + ((160) + ((256) / 2)) || (pos.y) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = star->base.meX; }; TaskDestroy(gCurTask); return; }; }; if (--star->timer == 0) { star->timer = 60 / 3; s->graphics.anim = 511; s->variant = 1; s->prevVariant = -1; gCurTask->main = Task_StarClose; }; Player_UpdateHomingPosition(star->spawnX, star->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);; }; }

static void Task_StarClose(void)
{
    { Sprite_Star *star = (void *)(0x03000000 + ((gCurTask)->data)); Sprite *s = &star->s; MapEntity *me = star->base.me; Vec2_32 pos; pos.x = ((int)((star->spawnX + 0) >> 8)); pos.y = ((int)((star->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;; { { Coll_Player_Projectile(s, pos.x, pos.y); } }; if (((pos.x) > gCamera.x + (240 + (256 / 2)) || (pos.x) < gCamera.x - (256 / 2) || (pos.y) > gCamera.y + ((160) + ((256) / 2)) || (pos.y) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = star->base.meX; }; TaskDestroy(gCurTask); return; }; }; if (--star->timer == 0) { star->timer = 60 * 2; s->graphics.anim = 511; s->variant = 2; s->prevVariant = -1; gCurTask->main = Task_StarSpin; }; Player_UpdateHomingPosition(star->spawnX, star->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);; };
}

static void Task_StarSpin(void)
{
    { Sprite_Star *star = (void *)(0x03000000 + ((gCurTask)->data)); Sprite *s = &star->s; MapEntity *me = star->base.me; Vec2_32 pos; pos.x = ((int)((star->spawnX + 0) >> 8)); pos.y = ((int)((star->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;; { { Coll_Player_Projectile(s, pos.x, pos.y); } }; if (((pos.x) > gCamera.x + (240 + (256 / 2)) || (pos.x) < gCamera.x - (256 / 2) || (pos.y) > gCamera.y + ((160) + ((256) / 2)) || (pos.y) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = star->base.meX; }; TaskDestroy(gCurTask); return; }; }; if (--star->timer == 0) { star->timer = 60 / 3; s->graphics.anim = 511; s->variant = 3; s->prevVariant = -1; gCurTask->main = Task_StarOpen; }; Player_UpdateHomingPosition(star->spawnX, star->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);; };
}

static void Task_StarOpen(void)
{
    { Sprite_Star *star = (void *)(0x03000000 + ((gCurTask)->data)); Sprite *s = &star->s; MapEntity *me = star->base.me; Vec2_32 pos; pos.x = ((int)((star->spawnX + 0) >> 8)); pos.y = ((int)((star->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;; { { Coll_Player_Projectile(s, pos.x, pos.y); } }; if (((pos.x) > gCamera.x + (240 + (256 / 2)) || (pos.x) < gCamera.x - (256 / 2) || (pos.y) > gCamera.y + ((160) + ((256) / 2)) || (pos.y) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = star->base.meX; }; TaskDestroy(gCurTask); return; }; }; if (--star->timer == 0) { star->timer = 60 * 2; s->graphics.anim = 511; s->variant = 0; s->prevVariant = -1; gCurTask->main = Task_StarIdle; }; Player_UpdateHomingPosition(star->spawnX, star->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);; };
}
