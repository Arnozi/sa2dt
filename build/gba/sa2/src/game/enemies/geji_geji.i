# 0 "src/game/enemies/geji_geji.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/geji_geji.c"
...
# 2 "src/game/enemies/geji_geji.c" 2
...
# 4 "src/game/enemies/geji_geji.c" 2
...
# 6 "src/game/enemies/geji_geji.c" 2
...
# 7 "src/game/enemies/geji_geji.c" 2


# 1 "include/malloc_vram.h" 1
# 10 "src/game/enemies/geji_geji.c" 2
...
# 13 "src/game/enemies/geji_geji.c" 2
...
static const TileInfo gUnknown_080D8F50[4] = {
    { .numTiles = 9, .anim = 512, .variant = 3 },
    { .numTiles = 9, .anim = 512, .variant = 2 },
    { .numTiles = 12, .anim = 512, .variant = 1 },
    { .numTiles = 12, .anim = 512, .variant = 0 },
};
...
void CreateEntity_GejiGeji(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_GejiGeji *gg = (void *)(0x03000000 + ((t)->data));
    ...
    gg->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256));
    gg->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));
    gg->offsetX = 0;
    gg->offsetY = 0;
    ...
    for (i = 0; i < 64; i++) {
        gg->positions[0][i] = ((int)((gg->spawnX) >> 8));
        gg->positions[1][i] = ((int)((gg->spawnY) >> 8));
    }
    ...
    s->graphics.dest = VramMalloc(gUnknown_080D8F50[gg->unk27C].numTiles); s->graphics.anim = gUnknown_080D8F50[gg->unk27C].anim; s->variant = gUnknown_080D8F50[gg->unk27C].variant; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;
                  ;

    s = &gg->s2;
    s->x = 0;
    s->y = 0;
    s->graphics.dest = VramMalloc(gUnknown_080D8F50[gg->unk27C + 2].numTiles); s->graphics.anim = gUnknown_080D8F50[gg->unk27C + 2].anim; s->variant = gUnknown_080D8F50[gg->unk27C + 2].variant; s->oamFlags = ((20) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;
                                                                 ;
}
...
{
    u8 i;
    Sprite_GejiGeji *gg = (void *)(0x03000000 + ((gCurTask)->data));
    MapEntity *me;
    Vec2_32 pos;
    ...
    pos.x = ((int)((gg->spawnX + gg->offsetX) >> 8)); pos.y = ((int)((gg->spawnY + gg->offsetY) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;
    ...
    if (((((int)((gg->spawnX) >> 8))) > gCamera.x + (240 + (256 / 2)) || (((int)((gg->spawnX) >> 8))) < gCamera.x - (256 / 2) || (((int)((gg->spawnY) >> 8))) > gCamera.y + ((160) + ((256) / 2)) || (((int)((gg->spawnY) >> 8))) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = gg->base.meX; }; TaskDestroy(gCurTask); return; }; };

    if (gg->unk27C) {
        if (((((int)((gg->offsetY) >> 8)) <= me->d.sData[1] * 8)) && !(s->frameFlags & ((1) << (11)))) {
            gg->unk27D = 0x3C;
            gCurTask->main = sub_8058264;
        } else if (((((int)((gg->offsetY) >> 8)) >= (me->d.sData[1] * 8 + me->d.uData[3] * 8))) && (s->frameFlags & ((1) << (11)))) {
            gg->unk27D = 0x3C;
            gCurTask->main = sub_8058264;
        }
    } else {
        if (((((int)((gg->offsetX) >> 8)) <= me->d.sData[0] * 8)) && !(s->frameFlags & ((1) << (10)))) {
            gg->unk27D = 0x3C;
            gCurTask->main = sub_8058264;
        } else if ((((((int)((gg->offsetX) >> 8)) >= (me->d.sData[0] * 8 + me->d.uData[2] * 8))) && s->frameFlags & ((1) << (10)))) {
            gg->unk27D = 0x3C;
            gCurTask->main = sub_8058264;
        }
    }
...
static void sub_8058264(void)
{
    ...
    pos.x = ((int)((gg->spawnX + gg->offsetX) >> 8));
    pos.y = ((int)((gg->spawnY + gg->offsetY) >> 8));
