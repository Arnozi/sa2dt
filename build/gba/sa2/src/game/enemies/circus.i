# 0 "src/game/enemies/circus.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/circus.c"
...
void CreateEntity_Circus(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_Circus *circus = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &circus->s;
    ...
    circus->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); circus->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));;
    ...
    s->graphics.dest = VramMalloc(20); s->graphics.anim = 495; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;

    s = &circus->s2;
    s->x = 0;
    s->y = 0;
    s->graphics.dest = VramMalloc(16); s->graphics.anim = 496; s->variant = 2; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
...
void Task_CircusMain(void)
{
    Sprite_Circus *circus = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &circus->s;
    Sprite *s2 = &circus->s2;
    MapEntity *me = circus->base.me;
    Vec2_32 pos;

    pos.x = ((int)((circus->spawnX + 0) >> 8)); pos.y = ((int)((circus->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;

    if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; };

    if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = circus->base.meX; }; TaskDestroy(gCurTask); return; };

    if (--circus->unk84 == 0) {
        circus->unk88 = -((s32)((5.0)*256));
        circus->unk86 = -((s32)((8.0)*256));
        circus->unk84 = 30;
    ...
    Player_UpdateHomingPosition(circus->spawnX, circus->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);;
    s2->x = s->x;
    s2->y = s->y - ((s32)((0.125)*256));

    Coll_Player_Projectile(s2, pos.x, pos.y - ((s32)((0.125)*256)));
    DisplaySprite(s2);
}
...
void Task_8055C0C(void)
{
    ...
    s2->y = (s->y + ((int)((circus->unk86) >> 8))) - ((s32)((0.125)*256));

    s2->x = s->x;

    if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; };

    Coll_Player_Projectile(s2, pos.x, pos.y + ((int)((circus->unk86) >> 8)) - ((s32)((0.125)*256)));
