# 0 "src/game/enemies/kiki.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/kiki.c"
...
# 5 "src/game/enemies/kiki.c" 2



# 1 "include/malloc_vram.h" 1
# 9 "src/game/enemies/kiki.c" 2
...
void CreateEntity_Kiki(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_Kiki *kiki = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &kiki->s;
...
static void Task_KikiMain(void)
{
    Sprite_Kiki *kiki = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &kiki->s;
    ...
    if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y - kiki->unk3D + (256 / 2) < 0) || (s->y - kiki->unk3D > 160 + (256 / 2)))) {
        { me->x = kiki->base.meX; };
        TaskDestroy(gCurTask);
        return;
    }
    ...
    Player_UpdateHomingPosition(((s32)((x) << 8)), ((s32)((y) << 8))); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);;
}
...
static void CreateKikiProjectile(s16 x, s16 y)
{
    ...
    Kiki_Proj *proj = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &proj->s;
    ...
    proj->unk38 = x;
    if (((int)((gPlayer.qWorldX) >> 8)) < x) {
        proj->unk34 = -Div((x - ((int)((gPlayer.qWorldX) >> 8))) * 400, 800);
        if (proj->unk34 < -0x3C) {
            proj->unk34 = -60;
        }
    } else {
        proj->unk34 = Div((((int)((gPlayer.qWorldX) >> 8)) - x) * 400, 800);
...
static void Task_KikiProjMain(void)
{
    Kiki_Proj *proj = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &proj->s;
    s16 x, y;
...
static void Task_ProjPieceMain(void)
{
    ...
    if ((s->hitboxes[0].index != -1) && (s2->hitboxes[0].index != -1)) {
        ...
        x2 = ((int)((gPlayer.qWorldX) >> 8)) + s2->hitboxes[0].left;
        if ((x1 <= x2 && x1 + (s->hitboxes[0].right - s->hitboxes[0].left) >= x2)
            ...
            y2 = ((int)((gPlayer.qWorldY) >> 8)) + s2->hitboxes[0].top;
