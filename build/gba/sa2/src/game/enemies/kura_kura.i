# 0 "src/game/enemies/kura_kura.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/kura_kura.c"
...
# 2 "src/game/enemies/kura_kura.c" 2
# 1 "include/core.h" 1
...
# 3 "src/game/enemies/kura_kura.c" 2
# 1 "include/sprite.h" 1
# 4 "src/game/enemies/kura_kura.c" 2
# 1 "include/malloc_vram.h" 1
# 5 "src/game/enemies/kura_kura.c" 2
# 1 "include/trig.h" 1
...
# 6 "src/game/enemies/kura_kura.c" 2
...
# 8 "src/game/enemies/kura_kura.c" 2
...
# 11 "src/game/enemies/kura_kura.c" 2
...
# 16 "src/game/enemies/kura_kura.c" 2
...
void CreateEntity_KuraKura(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_KuraKura *kk = (void *)(0x03000000 + ((t)->data));
    Sprite *s = &kk->s;
    kk->unkB0 = 0;
    kk->unkAC = 0;
    ...
    kk->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); kk->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));;
    ...
    s->graphics.dest = VramMalloc(8); s->graphics.anim = 485; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;

    s = &kk->s1;
    s->x = 0;
    s->y = 0;

    s->graphics.dest = VramMalloc(1); s->graphics.anim = 486; s->variant = 0; s->oamFlags = ((17) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
    ...
    s->graphics.dest = VramMalloc(4); s->graphics.anim = 487; s->variant = 0; s->oamFlags = ((16) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
}
...
void Task_8052024(void)
{
    Sprite_KuraKura *kk = (void *)(0x03000000 + ((gCurTask)->data));
    Sprite *s = &kk->s;
    MapEntity *me = kk->base.me;

    Vec2_32 pos;
    pos.x = ((int)((kk->spawnX + 0) >> 8)); pos.y = ((int)((kk->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;

    (s)->frameFlags &= ~(((1) << (10)));

    if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; };
    if (((pos.x) > gCamera.x + (240 + (256 / 2)) || (pos.x) < gCamera.x - (256 / 2) || (pos.y) > gCamera.y + ((160) + ((256) / 2)) || (pos.y) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 240 + 256) || (s->y + (256 / 2) < 0) || (s->y > 160 + (256 / 2)))) { { me->x = kk->base.meX; }; TaskDestroy(gCurTask); return; }; };

    Player_UpdateHomingPosition(kk->spawnX, kk->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);;
...
void sub_805213C(Sprite_KuraKura *kk)
{
    ...
    kk->unkAC = (((int)(((gSineTable[((gStageTime * 4) & (1024 - 1))])) >> 6)) >> 1) & (1024 - 1);

    for (i = 0; i < 2; i++) {
        s1->x = (((int)((kk->spawnX) >> 8)) + ((gSineTable[(kk->unkAC)]) >> (11 - i))) - gCamera.x;
        s1->y = (((int)((kk->spawnY) >> 8)) + ((gSineTable[(kk->unkAC) + 256]) >> (11 - i))) - gCamera.y;
        DisplaySprite(s1);
    }

    pos.x = (((int)((kk->spawnX) >> 8)) + ((gSineTable[(kk->unkAC)]) >> 9)) - ((gSineTable[(kk->unkAC)]) >> 11);
    pos.y = (((int)((kk->spawnY) >> 8)) + ((gSineTable[(kk->unkAC) + 256]) >> 9)) - ((gSineTable[(kk->unkAC) + 256]) >> 11);
