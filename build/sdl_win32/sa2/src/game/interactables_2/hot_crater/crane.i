# 0 "src/game/interactables_2/hot_crater/crane.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_2/hot_crater/crane.c"
...
# 2 "src/game/interactables_2/hot_crater/crane.c" 2
...
# 4 "src/game/interactables_2/hot_crater/crane.c" 2
...
# 5 "src/game/interactables_2/hot_crater/crane.c" 2
# 1 "include/task.h" 1
...
# 6 "src/game/interactables_2/hot_crater/crane.c" 2
# 1 "include/trig.h" 1
...
# 7 "src/game/interactables_2/hot_crater/crane.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 9 "src/game/interactables_2/hot_crater/crane.c" 2
...
# 11 "src/game/interactables_2/hot_crater/crane.c" 2
...
# 14 "src/game/interactables_2/hot_crater/crane.c" 2
...
# 16 "src/game/interactables_2/hot_crater/crane.c" 2
# 1 "include/constants/char_states.h" 1
# 17 "src/game/interactables_2/hot_crater/crane.c" 2
# 1 "include/constants/player_transitions.h" 1
# 18 "src/game/interactables_2/hot_crater/crane.c" 2
...
void CreateEntity_Crane(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_HCCrane *crane = (void *)((t)->data);
    CranePart *part;
    u16 i;

    crane->unk0 = 0;
    crane->posX = ((me->x)*8 + (spriteRegionX)*256);
    crane->posY = ((me->y)*8 + (spriteRegionY)*256);
    crane->me = me;
    ...
    part->unkC = 0;
    part->unk10 = 0;
    part->s->oamFlags = ((18) << 6);

    part->s->graphics.size = 0;
    part->s->animCursor = 0;
    part->s->qAnimDelay = 0;
    part->s->prevVariant = -1;
    part->s->animSpeed = ((int)((float)(1.0)*0x10));
    part->s->palId = 0;
    part->s->hitboxes[0].index = -1;
    part->s->frameFlags = ((2) << (12));
    part->s->graphics.dest = (void *)(&VRAM[0x10000] + 0x2BC0);
    part->s->graphics.anim = 565;
    part->s->variant = 0;
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (i == 0) {
            part->unk4 = 2;
            part->s = 
# 122 "src/game/interactables_2/hot_crater/crane.c" 3 4
                     ((void *)0)
# 122 "src/game/interactables_2/hot_crater/crane.c"
                         ;
            part->rotation = 0;
            part->unkC = 0x2C00;
            part->unk10 = 0;
            ...
            if (i == 1) {
                ...
                part->s->hitboxes[0].index = -1;
                part->s->frameFlags = ((2) << (12));
                part->s->graphics.dest = (void *)(&VRAM[0x10000] + 0x2B80);
                part->s->graphics.anim = 564;
                part->s->variant = 1;
                UpdateSpriteAnimation(part->s);
            }
        }
    ...
    part->s->hitboxes[0].index = -1;
    part->s->frameFlags = ((2) << (12));
    part->s->graphics.dest = (void *)(&VRAM[0x10000] + 0x2980);
    part->s->graphics.anim = 564;
    part->s->variant = 0;
    UpdateSpriteAnimation(part->s);

    part = &crane->parts[1 + 6 + 1];
    part->s = 
# 176 "src/game/interactables_2/hot_crater/crane.c" 3 4
             ((void *)0)
# 176 "src/game/interactables_2/hot_crater/crane.c"
                 ;
    part->unk4 = 2;
    part->rotation = 0;
    part->unkC = 0x1000;
...
static void Task_Inactive(void)
{
    Sprite_HCCrane *crane = (void *)((gCurTask)->data);

    sub_8074260(crane);

    if (crane->parts[1 + 6].rotation != 256) {
        if (crane->parts[1 + 6].rotation > 256) {
            crane->parts[1 + 6].rotation--;
        } else {
            crane->parts[1 + 6].rotation++;
        }
    }
...
static void Task_CraneExtend(void)
{
    ...
    if ((gPlayer.moveState & 0x00000080) || (gPlayer.timerInvulnerability == 120)) {
    ...
    crane->unk8 += r0;
    crane->parts[0].rotation = 512 - ((u16)crane->unk8 >> 6);
...
static void Task_8073C6C(void)
{
    ...
    if (r1 != 0) {
        ...
        {
            u32 newR0 = r1 + 256;
            crane->parts[1 + 6].rotation = newR0 & (1024 - 1);
        }
    }
    ...
    if ((crane->unk6 > 0) || (result_744D0 == 0)) {
        sub_80743E4(crane);
    }
...
static void Task_8073E20(void)
{
    ...
    if (crane->unk8 != 0 || crane->unkA != 0) {
        ...
        if (crane->unk6 == 0) {
            ...
            crane->parts[1 + 6].rotation = ((temp2 - temp) >> 4) + 256;
        ...
        crane->parts[1].rotation = ({ s16 clamped = (crane->parts[1].rotation) & (1024 - 1); clamped; });
    }
...
static void HandlePlayerTouchingHook(Sprite_HCCrane *crane)
{
    ...
    { Player_HandleSpriteYOffsetChange(&gPlayer, 9); { (&gPlayer)->spriteOffsetX = 6; (&gPlayer)->spriteOffsetY = 9; } };
    gPlayer.moveState |= 0x00400000;
    ...
    ({ if ((crane->accelY) < (((s16)((7.5)*256)))) { crane->accelY = (((s16)((7.5)*256))); } else if ((crane->accelY) > (((s16)((12)*256)))) { crane->accelY = (((s16)((12)*256))); } });
...
{
    if (!(gPlayer.moveState & 0x00000080) && crane->unk0 != 0) {
        gPlayer.moveState &= ~0x00400000;
...
static void UpdatePartScreenPositions(Sprite_HCCrane *crane)
{
    ...
    screenX = ((s32)((crane->posX - gCamera.x)*256));
    screenY = ((s32)((crane->posY - gCamera.y)*256));

    for (i = 0; i < (sizeof(crane->parts) / sizeof((crane->parts)[0])); i++) {
        CranePart *part = &crane->parts[i];
        s16 cos = ((int)(((gSineTable[(sinIndex) + 256])) >> 6));
        s32 cosV2 = ((int)((part->unkC * cos) >> 8));
        s16 sin = ((int)(((gSineTable[(sinIndex)])) >> 6));
        s32 sinV2 = ((int)((part->unk10 * sin) >> 8));
        s32 diff, temp;
        ...
        temp = ((int)((part->unkC * sin) >> 8));

        screenY += temp + ((int)((part->unk10 * cos) >> 8));
...
static void Render(Sprite_HCCrane *crane)
{
    ...
    for (i = 0; i < (sizeof(crane->parts) / sizeof((crane->parts)[0])); i++) {
        ...
        if (!(part->unk4 & 0x2)) {
            part->s->x = ((int)((part->screenX) >> 8));
            part->s->y = ((int)((part->screenY) >> 8));
            ...
            if (part->unk4 & 0x1) {
                ...
                part->s->frameFlags = gOamMatrixIndex++ | ((2) << (12)) | ((1) << (5))
                    | ((1) << (6));
...
static bool32 PlayerIsTouchingHook(Sprite_HCCrane *crane)
{
    if (!(gPlayer.moveState & 0x00000080)) {
        if ((gPlayer.moveState & 0x00000002) && (gPlayer.qSpeedAirY > 0)) {
            s16 screenX = ((int)((crane->parts[1 + 6].screenX) >> 8));
            s16 screenY = ((int)((crane->parts[1 + 6].screenY) >> 8));
...
{
    crane->unk6 = (512 - crane->parts[0].rotation) << 1;
...
static void sub_8074400(Sprite_HCCrane *crane)
{
    ...
    crane->unk8 = (256 - crane->parts[1 + 6].rotation) << 4;
...
static u16 sub_8074448(Sprite_HCCrane *crane, u16 max)
{
    ...
    for (i = 0; i <= max; i++) {
        result = (result + crane->parts[i].rotation) & (1024 - 1);
    }
...
{
    u32 r2 = crane->parts[0].rotation;

    crane->parts[1].rotation = (1024 - r2) & (1024 - 1);
}
...
static void sub_8074490(Sprite_HCCrane *crane, s16 p1)
{
    ...
    for (i = 2; i < ((sizeof(crane->parts) / sizeof((crane->parts)[0])) - 1); i++) {
        CranePart *part = &crane->parts[i];

        part->unk10 += p1;
        if (part->unk10 > (((s16)((12)*256)))) {
            part->unk10 = (((s16)((12)*256)));
        } else if (part->unk10 < -(((s16)((12)*256)))) {
            part->unk10 = -(((s16)((12)*256)));
        }
    }
}
...
{
    if (!(gPlayer.moveState & 0x00000080) && (crane->unk0 != 0)) {
        gPlayer.qWorldY = crane->parts[1 + 6 + 1].screenY + ((s32)((gCamera.y + 24)*256));

        if (gPlayer.moveState & 0x00000001) {
            gPlayer.qWorldX = crane->parts[1 + 6 + 1].screenX + ((s32)((gCamera.x + 6)*256));
        } else {
            gPlayer.qWorldX = crane->parts[1 + 6 + 1].screenX + ((s32)((gCamera.x - 6)*256));
        }
    }
...
static bool32 ShouldDespawn(Sprite_HCCrane *crane)
{
    ...
    if (((screenX + 64) < -(256 / 2)) || ((screenX - 64) > (426 + (256 / 2)))
        || ((screenY + 64) < -(256 / 2)) || ((screenY - 64) > (240 + (256 / 2)))) {
        return 1;
    }
