# 0 "src/game/interactables_2/music_plant/note_block.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_2/music_plant/note_block.c"
...
# 2 "src/game/interactables_2/music_plant/note_block.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 3 "src/game/interactables_2/music_plant/note_block.c" 2
# 1 "include/task.h" 1
...
# 4 "src/game/interactables_2/music_plant/note_block.c" 2
# 1 "include/trig.h" 1
...
# 5 "src/game/interactables_2/music_plant/note_block.c" 2
...
# 7 "src/game/interactables_2/music_plant/note_block.c" 2
...
# 10 "src/game/interactables_2/music_plant/note_block.c" 2
...
# 11 "src/game/interactables_2/music_plant/note_block.c" 2
...
# 13 "src/game/interactables_2/music_plant/note_block.c" 2
# 1 "include/constants/char_states.h" 1
# 14 "src/game/interactables_2/music_plant/note_block.c" 2
# 1 "include/constants/player_transitions.h" 1
# 15 "src/game/interactables_2/music_plant/note_block.c" 2
# 1 "include/constants/songs.h" 1
# 16 "src/game/interactables_2/music_plant/note_block.c" 2
...
static const u16 sAnims[7][3] = {
    { 575, 0, 0x16C }, { 575, 1, 0x170 }, { 575, 2, 0x174 },
    { 575, 3, 0x178 }, { 575, 4, 0x17C }, { 575, 5, 0x180 },
    { 575, 6, 0x184 },
};

static const s16 sBounceSpeeds[7] = {
    ((s16)((4.5 + 0. / 8.)*256)),
    ((s16)((4.5 + 3. / 8.)*256)),
    ((s16)((4.5 + 6. / 8.)*256)),
    ((s16)((4.5 + 9. / 8.)*256)),
...
const u16 sSfxGlockenspiel[7 + 1] = {
    313, 314, 315, 316,
    317, 318, 318, 0,
};
...
void CreateEntity_NoteBlock(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_NoteBlock *block = (void *)((t)->data);
    Sprite *s = &block->s;
    ...
    s->graphics.dest = &((u8 *)&VRAM[0x10000])[sAnims[block->type][2] * 32];
    s->graphics.anim = sAnims[block->type][0];
    s->variant = sAnims[block->type][1];

    block->posX = ((me->x)*8 + (spriteRegionX)*256);
    block->posY = ((me->y)*8 + (spriteRegionY)*256);
...
{
    Sprite_NoteBlock *block = (void *)((gCurTask)->data);

    u32 frame = block->bounceFrame++;

    switch (frame) {
        case 0: {
            block->bounceOffsetX = ((int)(((gSineTable[(block->bounceSin * 4) + 256])) >> 6)) * 4;
            block->bounceOffsetY = ((int)(((gSineTable[(block->bounceSin * 4)])) >> 6)) * 4;
        } break;
...
static void LaunchPlayer(Sprite_NoteBlock *block)
{
    ...
    block->bounceSin = ((int)((((double)(270)) / 360.0) * 1024)) / 4;
    gPlayer.qSpeedAirY = -(sBounceSpeeds[block->type]);
    gPlayer.charState = 57;
    ...
    if (--block->health == 1) {
        block->s.graphics.dest = &((u8 *)&VRAM[0x10000])[sAnims[(sizeof(sAnims) / sizeof((sAnims)[0])) - 1][2] * 32];
        block->s.graphics.anim = sAnims[(sizeof(sAnims) / sizeof((sAnims)[0])) - 1][0];
        block->s.variant = sAnims[(sizeof(sAnims) / sizeof((sAnims)[0])) - 1][1];
...
static void TaskDestructor_NoteBlock(struct Task *__attribute__((unused)) t) { }
...
static void UpdateBouncePosition(Sprite_NoteBlock *block)
{
    ...
    s->x = block->posX - gCamera.x + ((int)((block->bounceOffsetX) >> 8));
    s->y = block->posY - gCamera.y + ((int)((block->bounceOffsetY) >> 8));
}
...
static bool32 ShouldDespawn(Sprite_NoteBlock *block)
{
    s32 screenX, screenY;
    u16 otherX, otherY;
# 219 "src/game/interactables_2/music_plant/note_block.c"
    screenX = block->posX;
    screenX -= gCamera.x;
    screenY = block->posY;
    screenY -= gCamera.y;
    ...
    if ((screenX < -128 || screenX > (128 + 426)) || (screenY < -128 || screenY > (240 + 128))) {
...
{
    if (!(gPlayer.moveState & 0x00000080) && (block->health != 0)) {
        s32 posX, posY;
        u16 otherX, otherY;
        posX = ((int)((gPlayer.qWorldX) >> 8)) + 16;
        posX -= block->posX;
        posY = ((int)((gPlayer.qWorldY) >> 8)) + 24;
        posY -= block->posY;
