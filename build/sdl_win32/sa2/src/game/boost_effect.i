# 0 "src/game/boost_effect.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/boost_effect.c"
...
# 3 "src/game/boost_effect.c" 2
# 1 "include/flags.h" 1
# 4 "src/game/boost_effect.c" 2
# 1 "include/malloc_vram.h" 1
# 5 "src/game/boost_effect.c" 2
...
# 1 "include/sprite.h" 1
# 7 "include/data/sprite_data.h" 2
...
# 15 "src/game/boost_effect.c" 2
# 1 "include/constants/anim_commands.h" 1
# 16 "src/game/boost_effect.c" 2
...
# 39 "src/game/boost_effect.c"
static PlayerState __attribute__((aligned(16))) sPlayerStateBuffer[16] = { 0 };
static Vec2_32 sPlayerPosBuffer[16] = { 0 };
static u8 __attribute__((aligned(4))) sPlayerStateBufferIndex = 0;
static u8 __attribute__((aligned(4))) sPlayerPosBufferIndex = 0;

const u8 gUnknown_080D5674[3] = { 2, 4, 6 };

const AnimId sCharacterPalettesBoostEffect[NUM_CHARACTERS] = {
    ((74) + CHARACTER_SONIC * (91)), ((74) + CHARACTER_CREAM * (91)),
    ((74) + CHARACTER_TAILS * (91)), ((74) + CHARACTER_KNUCKLES * (91)),
    ((74) + CHARACTER_AMY * (91)),
};
...
void sub_801561C(void)
{
    ...
    oldPlayerMovestate &= ~0x80000000;

    if ((gStageFlags & 0x0080)) {
        oldPlayerMovestate |= 0x80000000;
    }

    for (i = 0; i < (s32)(sizeof(sPlayerStateBuffer) / sizeof((sPlayerStateBuffer)[0])); i++) {
...
void BoostEffect_StorePlayerState(void)
{
    ...
    sPlayerStateBufferIndex = ((sPlayerStateBufferIndex + ((1))) % (unsigned)(sizeof(sPlayerStateBuffer) / sizeof((sPlayerStateBuffer)[0])));
    i = sPlayerStateBufferIndex;

    oldMovestate &= ~0x80000000;
...
void sub_8015750(void)
{
    ...
    for (i = 0; i < (s32)(sizeof(sPlayerPosBuffer) / sizeof((sPlayerPosBuffer)[0])); i++) {
...
void BoostEffect_StorePlayerPos(void)
{
    ...
    sPlayerPosBufferIndex = ((sPlayerPosBufferIndex + ((1))) % (unsigned)(sizeof(sPlayerPosBuffer) / sizeof((sPlayerPosBuffer)[0])));
...
void GetPreviousPlayerPos(Vec2_32 *pos, u8 pastFrameDelta)
{
    s32 index = ((sPlayerPosBufferIndex + (-pastFrameDelta)) % (unsigned)(sizeof(sPlayerPosBuffer) / sizeof((sPlayerPosBuffer)[0])));
...
{
    s32 index = ((sPlayerStateBufferIndex + (-pastFrameDelta)) % (unsigned)(sizeof(sPlayerStateBuffer) / sizeof((sPlayerStateBuffer)[0])));
...
static inline void sub_8015B64_inline(AnimId anim, u16 palId)
{
    ...
    if (*pAnim++ == -2) {
        ...
        DmaSet(3, &gRefSpriteTables->palettes[animPalId * 16], &gObjPalette[(0)*16u + (insertOffset)], (u32)((0x8000 | 0x0000 | 0x0400 | 0x0000 | 0x0000) << 16 | ((numColors * sizeof(u16))/(32/8))));
...
void CreateBoostEffectTasks(void)
{
    ...
    if (((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2)) && !gBoostEffectTasksCreated && !((((gCurrentLevel) & ((3 + 1) - 1)) == 2) || (((gCurrentLevel) == ((7)*(3 + 1)) + (0)) && !gFinalBossActive) || (((gCurrentLevel) == ((7)*(3 + 1)) + (1))))) {
        gBoostEffectTasksCreated = 1;

        for (i = 0; i < (sizeof(gUnknown_080D5674) / sizeof((gUnknown_080D5674)[0])); i++) {
            struct Task *t = TaskCreate(Task_80159C8, sizeof(PlayerActions), 0x4000, 0, TaskDestructor_8015B50);
            PlayerActions *actions = (void *)((t)->data);
            ...
            actions->transform.qScaleY = +((s32)((1)*256));
        }
...
void Task_80159C8(void)
{
    PlayerActions *actions = (void *)((gCurTask)->data);
    Sprite *s = &actions->s;
    ...
    if (!(gPlayer.moveState & 0x04000000)) {
        if (gPlayer.moveState & 0x08000000) {
    ...
    if ((!(gPlayer.moveState & 0x00000080))) {
        if (gPlayer.isBoosting || (gPlayer.moveState & 0x02000000)) {
            ...
            s->x = ((int)((actions->pos.x) >> 8)) - gCamera.x;
            s->y = ((int)((actions->pos.y) >> 8)) - gCamera.y;

            transform->x = s->x;
            transform->y = s->y;
            UpdateSpriteAnimation(s);

            if ((((s)->frameFlags & (((1) << (5)))) >> ((5)))) {
                (s)->frameFlags &= ~(((0x1F) << (0)));
                s->frameFlags |= (gOamMatrixIndex++) | ((1) << (5));

                if (actions->plState.moveState & 0x00000001) {
                    transform->qScaleX = +((s32)((1)*256));
                } else {
                    transform->qScaleX = -((s32)((1)*256));
                }

                actions->plState.moveState &= 0x80000000;
