# 0 "src/game/special_stage/physics.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/special_stage/physics.c"
# 1 "include/core.h" 1
...
# 2 "src/game/special_stage/physics.c" 2
...
# 3 "src/game/special_stage/physics.c" 2
...
# 4 "src/game/special_stage/physics.c" 2
...
# 5 "src/game/special_stage/physics.c" 2
...
# 6 "src/game/special_stage/physics.c" 2
...
# 1 "include/task.h" 1
# 7 "include/game/special_stage/guard_robo.h" 2
...
# 7 "src/game/special_stage/physics.c" 2
...
# 10 "src/game/special_stage/physics.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 11 "src/game/special_stage/physics.c" 2
...
# 13 "src/game/special_stage/physics.c" 2
# 1 "include/constants/songs.h" 1
# 14 "src/game/special_stage/physics.c" 2
...
{
    struct SpecialStagePhysics *physics = (void *)((gCurTask)->data);
...
# 55 "src/game/special_stage/physics.c" 3 4
       ((void *)0)
# 55 "src/game/special_stage/physics.c"
           , sub_806F9CC, sub_806F9CC, sub_806F9CC, sub_806F9CC, sub_806EFB4, sub_806F034,
        sub_806F3C4, sub_806F36C, sub_806F9E4, sub_806F0C4, sub_806F154, sub_806F1E8, PlayerStatePhysics_SlowToStop,
        sub_806FB00, sub_806F300, sub_806F300, 
# 57 "src/game/special_stage/physics.c" 3 4
                                              ((void *)0)
# 57 "src/game/special_stage/physics.c"
                                                  ,
    };
    ...
    if (player->state < 11) {
        ...
        s32 sin = (gSineTable[(arrowAngle)]) * 4;
        s32 cos = (gSineTable[(arrowAngle) + 256]) * 4;

        screenX = sin * 20;
        screenY = cos * 20;

        screenX = ((int)((screenX) >> 0x10)) + (426 / 2) - 8;
        screenY = -(((int)((screenY) >> 0x10)) >> 1) + 120;

        player->roboArrow.x = screenX;
        player->roboArrow.y = screenY;

        if (arrowAngle > 256 && arrowAngle < 768) {
            player->roboArrow.oamFlags = ((0) << 6);
        } else {
            player->roboArrow.oamFlags = ((10) << 6);
        }
    ...
    gBgScrollRegs[2][1] = -((int)((player->q16WorldY) >> 0x10));
    gBgScrollRegs[2][0] = -((int)((player->q16WorldX) >> 0x10));
}

void HandleMovementControls(__attribute__((unused)) u32 unused)
{
    struct SpecialStagePhysics *physics = (void *)((gCurTask)->data);
    struct SpecialStagePlayer *player = (void *)((physics->stage->playerTask)->data);

    s32 speed = player->speed;
    s32 maxSpeed = player->maxSpeed;

    u16 bearing = player->bearing;
    s32 sin = (gSineTable[(bearing)]);
    s32 cos = (gSineTable[(bearing) + 256]);
...
static void PlayerStatePhysics_SlowToStop(void)
{
    ...
    {
        s32 temp2 = ((int)((sin * speed) >> 8)) >> 2;
        s32 temp3 = ((int)((cos * speed) >> 8)) >> 2;
...
void sub_806F3C4(void)
{
    ...
    if (player->q16WorldX <= ((s32)((48)*0x10000))) {
        player->q16WorldX = ((s32)((48)*0x10000));
    }

    if (player->q16WorldX >= ((s32)((976)*0x10000))) {
        player->q16WorldX = ((s32)((976)*0x10000));
    }

    if (player->q16WorldY <= ((s32)((48)*0x10000))) {
        player->q16WorldY = ((s32)((48)*0x10000));
    }

    if (player->q16WorldY >= ((s32)((976)*0x10000))) {
        player->q16WorldY = ((s32)((976)*0x10000));
    }
...
void sub_806F468(void)
{
    ...
    s16 playerX = ((int)((player->q16WorldX) >> 0x10));
    s16 playerY = ((int)((player->q16WorldY) >> 0x10));
            ...
            if (playerY >= worldElements->y && playerY < (worldElements->y + worldElements->length)) {
                switch (worldElements->type) {
                    case 0:
                        ...
                        return;
                    case 1:
                        player->speed = player->speed < 1536 ? 1536 : player->speed;
...
void HandleBoost2(void)
{
    ...
    s32 sin = (gSineTable[(b2)]);
    s32 cos = (gSineTable[(b2) + 256]);
...
static s16 CalcGuardRoboPointerAngle(struct SpecialStage *stage)
{
    ...
    u16 angle = -player->bearing & (1024 - 1);
    ...
    for (i = 256; i > 0; i >>= 1) {
        ...
        cos = (gSineTable[(angle) + 256]) >> 6;

        if ((sin * qDy + cos * qDx) > 0) {
            angle = (angle + i) & (1024 - 1);
        } else {
            if ((sin * qDy + cos * qDx) >= 0) {
                s32 a = -sin * qDx;
                s32 b = cos * qDy;
...
{
    struct Task *t = TaskCreate(Task_PhysicsMain, sizeof(struct SpecialStagePhysics), 0x4000, 0, 
# 662 "src/game/special_stage/physics.c" 3 4
                                                                                                ((void *)0)
# 662 "src/game/special_stage/physics.c"
                                                                                                    );
    struct SpecialStagePhysics *physics = (void *)((t)->data);
