...
{
    struct SpecialStageUI *ui = (void *)((gCurTask)->data);
...
static void CreateStageTime(struct SpecialStageUI *ui)
{
    ...
    sub_806CA88(&ui->timeSymbol, 0, 1, 0x377, 0x1000, 426 - 48, 14, 1, 0, 0);

    unkF7E8 = &sValueSprites[timeHundreds];
    sub_806CA88(&ui->timeHundreds, 0, unkF7E8->size, unkF7E8->anim, 0x1000, 426 - 36, 18, 1, unkF7E8->variant,
                0);

    unkF7E8 = &sValueSprites[timeTens];
    sub_806CA88(&ui->timeTens, 0, unkF7E8->size, unkF7E8->anim, 0x1000, 426 - 28, 18, 1, unkF7E8->variant, 0);

    unkF7E8 = &sValueSprites[timeUnits];
    sub_806CA88(&ui->timeUnits, 0, unkF7E8->size, unkF7E8->anim, 0x1000, 426 - 20, 18, 1, unkF7E8->variant, 0);
...
static void CreateRingCounter(struct SpecialStageUI *ui)
{
    ...
    sub_806CA88(&ui->ringsHundredsDigit, 0, sprite->size, sprite->anim, 0x1000, (426 / 2) - 20, 16, 1,
                sprite->variant, 0);

    sprite = &sValueSprites[stage->ringsTens];
    sub_806CA88(&ui->ringsTensDigit, 0, sprite->size, sprite->anim, 0x1000, (426 / 2) - 12, 16, 1,
                sprite->variant, 0);

    sprite = &sValueSprites[stage->ringsUnits];
    sub_806CA88(&ui->ringsUnitsDigit, 0, sprite->size, sprite->anim, 0x1000, (426 / 2) - 4, 16, 1,
                sprite->variant, 0);

    sub_806CA88(&ui->unk154, 0, 8, 0x378, 0x1000, (426 / 2), 14, 2, 0, 0);

    sprite = &sValueSprites[stage->ringsTargetHundreds];
    sub_806CA88(&ui->ringsTargetHundredsDigit, 0, sprite->size, sprite->anim, 0x1000, (426 / 2) + 4, 24, 1,
                sprite->variant, 0);

    sprite = &sValueSprites[stage->ringsTargetTens];
    sub_806CA88(&ui->ringsTargetTensDigit, 0, sprite->size, sprite->anim, 0x1000, (426 / 2) + 12, 24, 1,
                sprite->variant, 0);

    sprite = &sValueSprites[stage->ringsTargetUnits];
    sub_806CA88(&ui->ringsTargetUnitsDigit, 0, sprite->size, sprite->anim, 0x1000, (426 / 2) + 20, 24, 1,
                sprite->variant, 0);
}
...
static void HandlePaused(struct SpecialStageUI *ui)
{
    ...
    sub_806CA88(s, 1, pauseMenuVariants[lang][0], pauseMenuVariants[lang][1], 0x1000, (426 / 2), (240 / 2), 0,
                pauseMenuVariants[lang][2], 0);

    DmaSet(3, &gObjPalette[(15)*16u + (9)], ui->pauseMenuPalette1, (u32)((0x8000 | 0x0000 | 0x0000 | 0x0000 | 0x0000) << 16 | ((sizeof(ui->pauseMenuPalette1))/(16/8))));
    DmaSet(3, &gObjPalette[(15)*16u + (12)], ui->pauseMenuPalette2, (u32)((0x8000 | 0x0000 | 0x0000 | 0x0000 | 0x0000) << 16 | ((sizeof(ui->pauseMenuPalette2))/(16/8))));
    ui->wasPaused = 1;
    m4aMPlayAllStop();
    m4aSongNumStart(142);
}
...
static void SpecialStageResultsScreenCreateUI(struct SpecialStageResultsScreen *resultsScreen)
{
    ...
    s16 a = (426 / 2) - 72, b = (426 / 2) - 72, c = (426 / 2) - 72;
    s16 d = (426 / 2) + 38, e = (426 / 2) + 38, f = (426 / 2) + 38;
    s16 i;
...
# 359 "src/game/special_stage/ui.c" 3 4
                                     ((void *)0)
# 359 "src/game/special_stage/ui.c"
                                         ;

    if (stage->targetReached) {
        sub_806CA88(&resultsScreen->unk4, 1, 0x28, 893, 0, 426 + 16 + a, (240 / 2) - 48, 0, 1, 0);
        sub_806CA88(&resultsScreen->unk34, 1, gUnknown_080DF880[character].size, gUnknown_080DF880[character].anim, 0,
                    426 + 16 + a, (240 / 2) - 56, 0, gUnknown_080DF880[character].variant, 0);
    } else {
        sub_806CA88(&resultsScreen->unk4, 1, 0x20, 893, 0, 426 + 16 + a, (240 / 2) - 48, 0, 0, 0);
    }

    sub_806CA88(&resultsScreen->unk1B4, 1, 0x16, 894, 0, 426 + 16 + a, (240 / 2) - 8, 0, 0,
                0);
    sub_806CA88(&resultsScreen->unk1E4, 1, 0x16, 894, 0, 426 + 16 + b, (240 / 2) + 12, 0,
                1, 0);
    sub_806CA88(&resultsScreen->unk214, 1, 0x16, 894, 0, 426 + 16 + c, (240 / 2) + 32, 0,
                2, 0);

    for (i = 0; i < 5; i++) {
        sub_806CA88(&resultsScreen->unk244[i], 1, sValueSprites[0].size, sValueSprites[0].anim, 0, d + ({ ((i)*8) + 426 + 16; }),
                    (240 / 2) + 8, 0, sValueSprites[0].variant, 0);
    }

    for (i = 0; i < 5; i++) {
        sub_806CA88(&resultsScreen->unk334[i], 1, sValueSprites[0].size, sValueSprites[0].anim, 0, e + ({ ((i)*8) + 426 + 16; }),
                    (240 / 2) + 28, 0, sValueSprites[0].variant, 0);
    }

    for (i = 0; i < 5; i++) {
        sub_806CA88(&resultsScreen->unk424[i], 1, sValueSprites[0].size, sValueSprites[0].anim, 0, f + ({ ((i)*8) + 426 + 16; }),
                    (240 / 2) + 48, 0, sValueSprites[0].variant, 0);
    }
    ...
    for (i = 0; i < 7; i++) {
        ...
        sub_806CA88(&resultsScreen->chaosEmerald[i], 1, 9, sprite->anim, 0, i * 24 + 426 + 52, (240 / 2) - 28, 0,
                    sprite->variant, 0);
    }

    resultsScreen->animFrame = 0;
    resultsScreen->unk516 = 426 + 16;
    resultsScreen->unk518 = 426 + 16;
    resultsScreen->unk51A = 426 + 16;
    resultsScreen->unk51C = 426 + 16;
    resultsScreen->unk51E = 426 + 16;
}

void Task_ResultsScreenSequencePart1(void)
{
    struct SpecialStageResultsScreen *resultsScreen = (void *)((gCurTask)->data);
...
void sub_807061C(s16 a)
{
    ...
    if (stage->targetReached) {
        s->x = resultsScreen->unk516 + (426 / 2) - 44;
        s = &resultsScreen->unk34;
        s->x = resultsScreen->unk516 + (426 / 2) - 116;
    } else {
        s->x = resultsScreen->unk516 + (426 / 2) - 72;
    }
}
...
void sub_8070680(s16 a)
{
    ...
    for (i = 0; i < 7; i++) {
        ...
        s32 x = (i * 24);
        s32 base = resultsScreen->unk518 + (426 / 2) - 84;
        x += base;
        s->x = x;
    }
}
...
void sub_80706D8(s16 a)
{
    ...
    s->x = resultsScreen->unk51A + (426 / 2) - 72;
    ...
    for (i = 0; i < 5; i++) {
        ...
        temp2 = (i * 8);
        temp = resultsScreen->unk51A + (426 / 2) + 38;
        s->x = temp2 + temp;
    }
}
...
void sub_8070740(s16 a)
{
    ...
    s->x = resultsScreen->unk51C + (426 / 2) - 72;

    for (i = 0; i < 5; i++) {
        s = &resultsScreen->unk334[i];
        temp2 = (i * 8);
        temp = resultsScreen->unk51C + (426 / 2) + 38;
        s->x = temp2 + temp;
    }
}
...
void sub_80707A8(s16 xPos)
{
    ...
    s->x = resultsScreen->unk51E + (426 / 2) - 72;

    for (i = 0; i < 5; i++) {
        s = &resultsScreen->unk424[i];
        temp2 = (i * 8);
        temp = resultsScreen->unk51E + (426 / 2) + 38;
        s->x = temp2 + temp;
    }
}
...
struct Task *CreateSpecialStageUI(struct SpecialStage *stage)
{
    ...
    struct SpecialStageUI *ui = (void *)((t)->data);
    ui->stage = stage;
    ui->unk2A4 = 
# 658 "src/game/special_stage/ui.c" 3 4
                ((void *)0)
# 658 "src/game/special_stage/ui.c"
                    ;
...
struct Task *CreateSpecialStageResultsScreen(struct SpecialStage *stage)
{
    ...
    struct SpecialStageResultsScreen *resultsScreen = (void *)((t)->data);
...
static void CreateMultiplierValue(struct SpecialStageUI *ui)
{
    ...
    sub_806CA88(&ui->multiplier, 0, unkF840->size, unkF840->anim, 0x1000, (426 / 2) - 8, (240 / 2) + 20, 2,
                unkF840->variant, 0);
}
...
void RenderMultiplier(void)
{
    struct SpecialStageUI *ui = (void *)((gCurTask)->data);
    struct SpecialStage *stage = ui->stage;
    struct SpecialStagePlayer *player = (void *)((stage->playerTask)->data);
...
void CreateStartText(struct SpecialStageUI *ui)
{
    ...
    sub_806CA88(s, 0, 20, 0x376, 0x1000, 426 / 2, 240 / 2, 1, 0, 0);
}
...
void sub_8070DE0(struct SpecialStageUI *ui)
{
    ...
    ui->unk2A4 = 
# 761 "src/game/special_stage/ui.c" 3 4
                ((void *)0)
# 761 "src/game/special_stage/ui.c"
                    ;
    ui->unk2A8 = 0;
}
...
static void RenderPauseMenu(struct SpecialStageUI *ui)
{
    ...
    if (stage->pauseMenuCursor == 0) {
        b = (void *)((u16*)&PLTT[0x200/sizeof(uint16_t)]) + 0x1F8;
        a = (void *)((u16*)&PLTT[0x200/sizeof(uint16_t)]) + 0x1F2;
    } else {
        b = (void *)((u16*)&PLTT[0x200/sizeof(uint16_t)]) + 0x1F2;
        a = (void *)((u16*)&PLTT[0x200/sizeof(uint16_t)]) + 0x1F8;
    }

    DmaSet(3, ui->pauseMenuPalette2, b, (u32)((0x8000 | 0x0000 | 0x0000 | 0x0000 | 0x0000) << 16 | ((sizeof(ui->pauseMenuPalette2))/(16/8))));
    DmaSet(3, ui->pauseMenuPalette1, a, (u32)((0x8000 | 0x0000 | 0x0000 | 0x0000 | 0x0000) << 16 | ((sizeof(ui->pauseMenuPalette1))/(16/8))));
    DisplaySprite(s);
}
...
static void HandleUnpaused(struct SpecialStageUI *ui)
{
    ui->wasPaused = 0;
    gSpecialStageSubMenuVramPointer = 
# 787 "src/game/special_stage/ui.c" 3 4
                                     ((void *)0)
# 787 "src/game/special_stage/ui.c"
                                         ;
    m4aMPlayAllContinue();
}
