...
extern const ACmd **const gAnimations[(1133 + 1)];
extern const SpriteOffset *const gSpriteDimensions[(1133 + 1)];
extern const OamDataShort *const gSpriteOamData[(1133 + 1)];
...
void CreateMultiplayerSinglePakResultsScreen(u32 a)
{
    ...
    CpuFastSet(bgStageTileset, (void *)&VRAM[0], ((0x8000)/(32/8) & 0x1FFFFF));
    CpuFastSet(tilemaps, (void *)EWRAM_START + 0x33000, ((0x40000 - 0x33000)/(32/8) & 0x1FFFFF));
...
void sub_8081FB0(void)
{
    ...
    { u32 tmp = (u32)(0); DmaSet(3, &tmp, (void *)VRAM + 0x9FE0, (u32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((64)/(32/8)))); };
    { u32 tmp = (u32)(0); DmaSet(3, &tmp, (void *)VRAM + 0xCFE0, (u32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((64)/(32/8)))); };
}
    ...
    if (++resultsScreen->unk430 > 0xF0) {
        ...
        if (resultsScreen->unk434) {
            for (i = 0; i < 3; i++) {
                ...
                s->graphics.dest = (void *)(&VRAM[0x10000] + 0x2500 + (i * 0x180));

                s->x = 0;
                s->y = 0;
                s->oamFlags = ((4) << 6);
                s->graphics.size = 0;
# 242 "src/game/multiboot/collect_rings/results.c"
...
# 323 "src/game/multiboot/collect_rings/results.c"
        for (i = 0; i < 4; i++) {
            if (!(gMultiSioStatusFlags & (0x0001 << (i + 8)))) {
                if (gMultiplayerRanks[i] & 1) {
                    sub_80078D4(3, i * 40, (i + 1) * 40, 426 - resultsScreen->unk430, 240 - i * 40);
                } else {
                    sub_80078D4(3, i * 40, (i + 1) * 40, resultsScreen->unk430 - 426, 240 - i * 40);
                }
            } else {
                if (gMultiplayerRanks[i] & 1) {
                    sub_80078D4(3, gMultiplayerRanks[i] * 40, (gMultiplayerRanks[i] + 1) * 40, 426 - resultsScreen->unk430,
                                (i * 5 - gMultiplayerRanks[i] * 5) * 8);
                } else {
                    sub_80078D4(3, gMultiplayerRanks[i] * 40, (gMultiplayerRanks[i] + 1) * 40, resultsScreen->unk430 - 426,
                                (i * 5 - gMultiplayerRanks[i] * 5) * 8);
                }
            }
        }
...
void sub_80823FC(void)
{
    ...
    s32 val2 = 0;
    u8 val = 0;
    struct MultiplayerSinglePakResultsScreen *resultsScreen = (void *)((gCurTask)->data);
    ...
    if (packet->pat0.unk0 == 0x4012) {
        ...
        gSelectedCharacter = ((volatile struct SioMultiCnt *)(REG_BASE + 0x128))->id;
        ...
        for (i = 0; i < 4 && (((gMultiplayerConnections) >> (i)) & 1); i++) {
            if (!((0x0001 << (i)) & gMultiSioStatusFlags)) {
                ...
                if (i == ((volatile struct SioMultiCnt *)(REG_BASE + 0x128))->id || packet->pat0.unk0 == 0x4010) {
                    val2++;
                }
            }
        }
...
void sub_8082788(void)
{
    ...
    for (i = 0; i < 4; i++) {
        if (!(gMultiSioStatusFlags & (0x0001 << (i + 8)))) {
            sub_80078D4(3, i * 40, (i + 1) * 40, 0, 240 - i * 40);
        } else {
...
{
    struct Task *t
        = TaskCreate(Task_MultiplayerSinglePakResultsScreenInit, sizeof(struct MultiplayerSinglePakResultsScreen), 0x2000, 0, 
# 613 "src/game/multiboot/collect_rings/results.c" 3 4
                                                                                                                             ((void *)0)
# 613 "src/game/multiboot/collect_rings/results.c"
                                                                                                                                 );
    struct MultiplayerSinglePakResultsScreen *resultsScreen = (void *)((t)->data);
...
void sub_8082B80(struct MultiplayerSinglePakResultsScreen *resultsScreen)
{
    ...
    for (i = 0; i < (s32)(sizeof(resultsScreen->unk80) / sizeof((resultsScreen->unk80)[0])); i++) {
        ...
        sub_8082CEC(&resultsScreen->unk80[i].unk0, &VRAM[0x10000] + (i * 0x800), anim, 0, 0x78, (i * 40) + 20, ((16) << 6), i, 0x1000);
    }
}
...
void sub_8082BF8(struct MultiplayerSinglePakResultsScreen *resultsScreen)
{
    ...
    for (i = 0; i < (s32)(sizeof(resultsScreen->unk160) / sizeof((resultsScreen->unk160)[0])); i++) {
        sub_8082CEC(&resultsScreen->unk160[i], &VRAM[0x10000] + (i * 4 + 0x100) * 0x20, 1105, i, 0, 0, ((4) << 6), 0,
                    0x1000);
    }
}
...
void sub_8082C58(struct MultiplayerSinglePakResultsScreen *resultsScreen)
{
    ...
    for (i = 0; i < (s32)(sizeof(resultsScreen->unk370) / sizeof((resultsScreen->unk370)[0])); i++) {
        sub_8082CEC(&resultsScreen->unk370[i], &VRAM[0x10000] + 0x2500 + i * 0xC0, 1099, i, 0, 0,
                    ((4) << 6), 0, 0x1000);
    }
}
