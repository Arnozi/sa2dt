# 0 "src/game/cutscenes/credits_end.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/cutscenes/credits_end.c"
...
# 2 "src/game/cutscenes/credits_end.c" 2
# 1 "include/core.h" 1
...
# 3 "src/game/cutscenes/credits_end.c" 2
# 1 "include/sprite.h" 1
# 4 "src/game/cutscenes/credits_end.c" 2
# 1 "include/task.h" 1
# 5 "src/game/cutscenes/credits_end.c" 2
...
# 6 "src/game/cutscenes/credits_end.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 7 "src/game/cutscenes/credits_end.c" 2
...
# 8 "src/game/cutscenes/credits_end.c" 2
# 1 "include/game/cutscenes/credits_end.h" 1





void CreateCreditsEndCutScene(u8);
# 9 "src/game/cutscenes/credits_end.c" 2
...
# 10 "src/game/cutscenes/credits_end.c" 2
...
# 11 "src/game/cutscenes/credits_end.c" 2
...
# 12 "src/game/cutscenes/credits_end.c" 2
...
# 13 "src/game/cutscenes/credits_end.c" 2
...
# 15 "src/game/cutscenes/credits_end.c" 2
# 1 "include/constants/songs.h" 1
# 16 "src/game/cutscenes/credits_end.c" 2
# 1 "include/constants/text.h" 1
# 17 "src/game/cutscenes/credits_end.c" 2
# 1 "include/constants/tilemaps.h" 1
# 18 "src/game/cutscenes/credits_end.c" 2
...
void CreateCreditsEndCutScene(u8 creditsVariant)
{
    ...
    s32 r6 = 1;
    struct Task *t;
    struct CreditsEndCutScene *scene = 
# 82 "src/game/cutscenes/credits_end.c" 3 4
                                      ((void *)0)
# 82 "src/game/cutscenes/credits_end.c"
                                          ;
    ScreenFade *fade;
    gDispCnt = 0x1040;
    ...
    { u32 tmp = (u32)(0); DmaSet(3, &tmp, (((u8*)&VRAM[0]) + (0x4000 * (r6))), (u32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((sizeof(Background) * 256)/(32/8)))); };
    ...
    for (i = 0; i < 2; i++) {
        scene->unk170[0][i] = 0;
        scene->unk170[1][i] = ((s32)((180)*256));


        scene->unk170[2][i] = ((s32)((200)*256));
    }
    ...
    scene->unk16C = &VRAM[0x10000];

    if (scene->creditsVariant == 2) {
...

        {
            ...
            scene->unk16C += (gUnknown_080E12D0[0].numTiles * 32);
...

        {
            ...
            s = &scene->unkF0;
            s->graphics.dest = scene->unk16C;
            scene->unk16C += (gUnknown_080E12D0[1].numTiles * 32);
...

        {
            ...
            s = &scene->unk120;
            s->graphics.dest = scene->unk16C;
            scene->unk16C += (gUnknown_080E12D0[2].numTiles * 32);
...
{
    struct CreditsEndCutScene *scene = (void *)((gCurTask)->data);
...
static void Task_HandleGameCompletion(void)
{
    ...
    if (scene->creditsVariant == 1) {
        if (!gLoadedSaveGame->completedCharacters[CHARACTER_AMY]) {
            gLoadedSaveGame->completedCharacters[CHARACTER_AMY] = 1;
            WriteSaveGame();
        }

        for (i = 0; i < NUM_CHARACTERS; i++) {
            if (gLoadedSaveGame->unlockedLevels[i] > ((7)*(3 + 1)) + (0)) {
                zonesCompleteCharacters |= (1 << (i));
            }
        }



        if (zonesCompleteCharacters >= ((1 << (CHARACTER_SONIC)) | (1 << (CHARACTER_CREAM)) | (1 << (CHARACTER_TAILS)) | (1 << (CHARACTER_KNUCKLES))) && (gLoadedSaveGame->chaosEmeralds[0] & (1 << (7)))
        ...
        if (gSelectedCharacter != CHARACTER_AMY) {
            if ((gLoadedSaveGame->chaosEmeralds[gSelectedCharacter] & (1 << (7)))) {
                gLoadedSaveGame->completedCharacters[gSelectedCharacter] = 1;
                ...
                if ((charactersCompleted == 1 && gLoadedSaveGame->chaoGardenUnlocked)
                    ...
                    || (charactersCompleted >= 4 && gLoadedSaveGame->unlockedCharacters > ((1 << (CHARACTER_SONIC)) | (1 << (CHARACTER_CREAM)) | (1 << (CHARACTER_TAILS)) | (1 << (CHARACTER_KNUCKLES))))) {
                    scene->hasAllEmeralds = 1;
                    scene->delayFrames = 180;
                } else {
                    if (charactersCompleted == 1) {
                        gLoadedSaveGame->chaoGardenUnlocked = 1;
                    } else if (charactersCompleted == 2) {
                        gLoadedSaveGame->soundTestUnlocked = 1;
                    } else if (charactersCompleted == 3) {
                        gLoadedSaveGame->bossTimeAttackUnlocked = 1;
                    } else if (charactersCompleted == 4) {
                        gLoadedSaveGame->unlockedCharacters |= (1 << (CHARACTER_AMY));
                    }
                    WriteSaveGame();
                    scene->hasAllEmeralds = 0;
                    scene->delayFrames = 105;
                    scene->fade.bldCnt = ((3 << 6) | ((1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4) | (1 << 5)) | ((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11) | (1 << 12) | (1 << 13)));
                }
            } else {
                scene->hasAllEmeralds = 0;
                scene->fade.bldCnt = ((3 << 6) | ((1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4) | (1 << 5)) | ((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11) | (1 << 12) | (1 << 13)));
        ...
        if (!gLoadedSaveGame->extraEndingCreditsPlayed) {
            gLoadedSaveGame->extraEndingCreditsPlayed = 1;
            WriteSaveGame();
        }
        scene->hasAllEmeralds = 1;
        scene->delayFrames = 180;
...
static void Task_SequenceEnd(void)
{
    ...
    if (scene->hasAllEmeralds == 0) {
        CreateMissingChaosEmaraldsCutScene();
        TaskDestroy(gCurTask);
    } else if (scene->hasAllEmeralds == 1) {
        CreateTitleScreen();
        TaskDestroy(gCurTask);
    }
}

static void TaskDestroy_CreditsEndCutScene(__attribute__((unused)) struct Task *t)
{
...
static void UpdateMessageLine1Pos(struct CreditsEndCutScene *scene)
{
    if (scene->sonicAnimFrame == 11) {
        ...
        if (scene->unk170[1][1] > ((s32)((130)*256))) {
            scene->unk170[1][1] -= ((s32)((3)*256));
        }
    }
}
...
static void UpdateMessageLine2Pos(struct CreditsEndCutScene *scene)
{
    if (scene->sonicAnimFrame == 11) {
        ...
        if (scene->unk170[2][1] > ((s32)((148)*256))) {
            scene->unk170[2][1] -= ((s32)((3)*256));
        }
    }
}
