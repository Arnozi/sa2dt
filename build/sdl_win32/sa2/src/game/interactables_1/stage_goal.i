# 0 "src/game/interactables_1/stage_goal.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_1/stage_goal.c"
...
# 2 "src/game/interactables_1/stage_goal.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 3 "src/game/interactables_1/stage_goal.c" 2
...
void VramFree(void *);
# 6 "src/game/interactables_1/stage_goal.c" 2
...
# 8 "src/game/interactables_1/stage_goal.c" 2
...
# 9 "src/game/interactables_1/stage_goal.c" 2
...
# 11 "src/game/interactables_1/stage_goal.c" 2
...
# 14 "src/game/interactables_1/stage_goal.c" 2
...
# 16 "src/game/interactables_1/stage_goal.c" 2
...
# 17 "src/game/interactables_1/stage_goal.c" 2
...
# 18 "src/game/interactables_1/stage_goal.c" 2


# 1 "include/task.h" 1
# 21 "src/game/interactables_1/stage_goal.c" 2
...
# 23 "src/game/interactables_1/stage_goal.c" 2
# 1 "include/constants/player_transitions.h" 1
# 24 "src/game/interactables_1/stage_goal.c" 2
...
void CreateEntity_StageGoal(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_StageGoal *stageGoal = (void *)((t)->data);
...
static void Task_StageGoalMain(void)
{
    Sprite_StageGoal *stageGoal = (void *)((gCurTask)->data);
    Sprite *s = &stageGoal->s;
    MapEntity *me = stageGoal->base.me;

    s32 x = ((stageGoal->base.meX)*8 + (stageGoal->base.regionX)*256);
    s32 y = ((me->y)*8 + (stageGoal->base.regionY)*256);

    s->x = x - gCamera.x;
    s->y = y - gCamera.y;

    if ((((u16)(s->x + (256 / 2)) > 426 + 256) || (s->y + (256 / 2) < 0) || (s->y > 240 + (256 / 2)))) {
        me->x = stageGoal->base.meX;
        TaskDestroy(gCurTask);
        return;
    }

    if ((!(gPlayer.moveState & 0x00000080)) && !(gPlayer.moveState & 0x00400000)) {
...
{
    Sprite_StageGoalToggle *stageGoalToggle = (void *)((gCurTask)->data);
    ...
    s32 x = ((spriteX)*8 + (regionX)*256);
    s32 y = ((me->y)*8 + (regionY)*256);

    if ((!(((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2))))) {
        if (x <= ((int)((gPlayer.qWorldX) >> 8)) && !(gPlayer.moveState & (0x08000000 | 0x00000008))) {
            gPlayer.transition = 10;
            gStageGoalX = x;
            StageGoalToggle_HandleMultiplayerFinish();
        }
    } else if (x <= ((int)((gPlayer.qWorldX) >> 8)) && !(gPlayer.moveState & 0x08000000)) {
        gPlayer.transition = 10;
        gStageFlags |= 0x0020 | 0x0001;
        gStageGoalX = x;

        if (gGameMode == 0 && !(gPlayer.moveState & 0x00000002) && gPlayer.qSpeedGround > ((s32)((2.5)*256))) {
            u32 extraScore;
            if (gPlayer.qSpeedGround <= ((s32)((4.0)*256))) {
                extraScore = 200;
            } else if (gPlayer.qSpeedGround <= ((s32)((9.0)*256))) {
                extraScore = 300;
            } else if (gPlayer.qSpeedGround <= ((s32)((10.0)*256))) {
                extraScore = 500;
            } else {
                extraScore = 800;
            }
            ...
            if (extraScore != 0) {
                { s32 divResA, divResB; s32 oldScore = gLevelScore; gLevelScore += extraScore; divResA = ({((50000) != 0) ? ((s32)(gLevelScore) / (s32)(50000)) : 0;}); divResB = ({((50000) != 0) ? ((s32)(oldScore) / (s32)(50000)) : 0;}); if ((divResA != divResB) && (gGameMode == 0)) { u16 lives = divResA - divResB; lives += gNumLives;; ({ if ((lives) > 255) (lives) = 255; gNumLives = (lives); }); if (1) gMusicManagerState.unk3 = 0x10 | 0x0; } };

                CreateStageGoalBonusPointsAnim(((int)((gPlayer.qWorldX) >> 8)), ((int)((gPlayer.qWorldY) >> 8)), extraScore);
            }
        }
# 174 "src/game/interactables_1/stage_goal.c"
    }

    x -= gCamera.x;
    y -= gCamera.y;
    if ((((u32)(x + (256 / 2)) > 426 + 256) || (y + (256 / 2) < 0) || (y > 240 + (256 / 2)))) {
        me->x = spriteX;
        TaskDestroy(gCurTask);
    }
}
...
static void StageGoalToggle_HandleMultiplayerFinish(void)
{
    RoomEvent *roomEvent;
    u32 count = 0;
    MultiplayerPlayer *player = (void *)((gMultiplayerPlayerTasks[((volatile struct SioMultiCnt *)(REG_BASE + 0x128))->id])->data);
    gPlayer.itemEffect &= ~0x40;
    gPlayer.confusionTimer = 0;

    if (!(player->unk5C & 1)) {
        u32 j;
        for (j = 0; j < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])) && gMultiplayerPlayerTasks[j] != 
# 194 "src/game/interactables_1/stage_goal.c" 3 4
                                                                                             ((void *)0)
# 194 "src/game/interactables_1/stage_goal.c"
                                                                                                 ; j++) {
            MultiplayerPlayer *otherPlayer = (void *)((gMultiplayerPlayerTasks[j])->data);
...
static __attribute__((unused)) void StageGoalToggle_ForceMultiplayerFinish(void)
{
    u32 thing = 0;
    ...
    u32 id = ((volatile struct SioMultiCnt *)(REG_BASE + 0x128))->id;

    gPlayer.itemEffect &= ~0x40;
    gPlayer.confusionTimer = 0;

    for (j = 0; j < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])) && mpTasks[j] != 
# 230 "src/game/interactables_1/stage_goal.c" 3 4
                                                                         ((void *)0)
# 230 "src/game/interactables_1/stage_goal.c"
                                                                             ; j++) {

        if ((gMultiplayerConnections & (0x10 << (j))) >> ((j + 4))
                != (gMultiplayerConnections & (0x10 << (((volatile struct SioMultiCnt *)(REG_BASE + 0x128))->id))) >> (((volatile struct SioMultiCnt *)(REG_BASE + 0x128))->id + 4)
    ...
    for (j = 0; j < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])) && gMultiplayerPlayerTasks[j] != 
# 240 "src/game/interactables_1/stage_goal.c" 3 4
                                                                                         ((void *)0)
# 240 "src/game/interactables_1/stage_goal.c"
                                                                                             ; j++) {
...
static void StageGoalToggle_PlayerReachedGoal(void)
{
    ...
    if (gMultiplayerRanks[id] != -1) {
        u32 j;
        struct Task **tasks;
        for (j = 0, tasks = gMultiplayerPlayerTasks; j < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])) && tasks[j] != 
# 268 "src/game/interactables_1/stage_goal.c" 3 4
                                                                                                            ((void *)0)
# 268 "src/game/interactables_1/stage_goal.c"
                                                                                                                ; j++) {
            MultiplayerPlayer *otherPlayer = (void *)((tasks[j])->data);
            if (otherPlayer->unk54 & 0x100) {
                count++;
            }
        }

        if (count != 0 && (count >= (j - 1) || gGameMode == 4)) {
            struct Task **tasks;
            u32 i;

            if (gMultiplayerPlayerTasks[0] != 
# 279 "src/game/interactables_1/stage_goal.c" 3 4
                                             ((void *)0)
# 279 "src/game/interactables_1/stage_goal.c"
                                                 ) {
                for (i = 0, tasks = gMultiplayerPlayerTasks; i < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])) && tasks[i] != 
# 280 "src/game/interactables_1/stage_goal.c" 3 4
                                                                                                                    ((void *)0)
# 280 "src/game/interactables_1/stage_goal.c"
                                                                                                                        ; i++) {
                    MultiplayerPlayer *otherPlayer = (void *)((tasks[i])->data);
                    if (!(otherPlayer->unk5C & 1) && gGameMode != 4) {
                        otherPlayer->unk5C |= 1;
...
{
    struct Task *t = TaskCreate(Task_StageGoalToggleMain, sizeof(Sprite_StageGoalToggle), 0x2010, 0, 
# 298 "src/game/interactables_1/stage_goal.c" 3 4
                                                                                                    ((void *)0)
# 298 "src/game/interactables_1/stage_goal.c"
                                                                                                        );
    Sprite_StageGoalToggle *stageGoalToggle = (void *)((t)->data);
