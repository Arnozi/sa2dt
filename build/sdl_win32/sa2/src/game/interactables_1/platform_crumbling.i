# 0 "src/game/interactables_1/platform_crumbling.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/interactables_1/platform_crumbling.c"
...
# 2 "src/game/interactables_1/platform_crumbling.c" 2
...
void VramFree(void *);
# 3 "src/game/interactables_1/platform_crumbling.c" 2
...
# 4 "src/game/interactables_1/platform_crumbling.c" 2
# 1 "include/task.h" 1
...
# 5 "src/game/interactables_1/platform_crumbling.c" 2
...
# 7 "src/game/interactables_1/platform_crumbling.c" 2
...
# 9 "src/game/interactables_1/platform_crumbling.c" 2
...
# 10 "src/game/interactables_1/platform_crumbling.c" 2
...
# 11 "src/game/interactables_1/platform_crumbling.c" 2
...
# 12 "src/game/interactables_1/platform_crumbling.c" 2
...
# 14 "src/game/interactables_1/platform_crumbling.c" 2
...
# 16 "src/game/interactables_1/platform_crumbling.c" 2
...
void CreateEntity_PlatformCrumbling(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_019 *platform = (void *)((t)->data);
    ...
    s->graphics.anim = sInt019_AnimationIds[gCurrentLevel % (sizeof(sInt019_AnimationIds) / sizeof((sInt019_AnimationIds)[0]))];
...
{
    Sprite_019 *platform = (void *)((gCurTask)->data);
    SpriteBase *base = &platform->base;
    Sprite *s = &platform->s;
    MapEntity *me = base->me;
    s16 screenX, screenY;

    screenX = ((base->meX)*8 + (base->regionX)*256);
    screenY = ((me->y)*8 + (base->regionY)*256);
    ...
    if ((screenX > gCamera.x + 426 + (256 / 2) || (screenX < gCamera.x - (256 / 2))
         || (screenY > gCamera.y + 240 + (256 / 2)) || (screenY < gCamera.y - (256 / 2)))
        && ((((u16)(s->x + (256 / 2)) > 426 + 256) || (s->y + (256 / 2) < 0) || (s->y > 240 + (256 / 2))))) {
        me->x = base->meX;
...
void Task_805E35C(void)
{
    ...
    screenX = ((platform->base.meX)*8 + (platform->base.regionX)*256);
    screenY = ((me->y)*8 + (platform->base.regionY)*256);
...
void Task_805E480(void)
{
    ...
    otherPos = (gCamera.y - screenY) + 240;
    ...
    for (y = 0; y < 4; y++) {
        for (x = 0; x < 8; r6++, x++) {
            ...
            oamAllocated = OamMalloc(((((s)->oamFlags) & 0x7C0) >> 6));
            if (iwram_end == oamAllocated)
                return;
# 235 "src/game/interactables_1/platform_crumbling.c"
            oamAllocated->split.y = (r4 + ((y * 8) + s->y));
            ...
            if (s->frameFlags & ((1) << (10))) {
                oamAllocated->split.x = (s->x - x * 8 - 8);
                oamAllocated->split.matrixNum = 0x8;
            } else {
                oamAllocated->split.x = (s->x + x * 8);
            }

            oamAllocated->split.tileNum = (((uintptr_t)((s->graphics.dest)) - (uintptr_t)&VRAM[0x10000]) / (32)) + r6;
            oamAllocated->split.priority = (((s)->frameFlags & (((3) << (12)))) >> ((12)));
...
void Task_805E6A4(void)
{
    ...
    for (y = 0; y < 4; y++) {
        for (x = 0; x < 8; r6++, x++) {
            ...
            if (s->frameFlags & ((1) << (10))) {
                (oamAllocated)->split.x = ((s->x - x * 8 - 8)); (oamAllocated)->split.matrixNum = ((1) ? 0x08 : 0);;
            } else {
                (oamAllocated)->split.x = ((s->x + x * 8 + 0)); (oamAllocated)->split.matrixNum = ((0) ? 0x08 : 0);;
            }
