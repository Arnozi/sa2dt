...
static void Task_StarIdle(void) { { Sprite_Star *star = (void *)((gCurTask)->data); Sprite *s = &star->s; MapEntity *me = star->base.me; Vec2_32 pos; pos.x = ((int)((star->spawnX + 0) >> 8)); pos.y = ((int)((star->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;; { { if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; } } }; if (((pos.x) > gCamera.x + (426 + (256 / 2)) || (pos.x) < gCamera.x - (256 / 2) || (pos.y) > gCamera.y + ((240) + ((256) / 2)) || (pos.y) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 426 + 256) || (s->y + (256 / 2) < 0) || (s->y > 240 + (256 / 2)))) { { me->x = star->base.meX; }; TaskDestroy(gCurTask); return; }; }; if (--star->timer == 0) { star->timer = 60 / 3; s->graphics.anim = 511; s->variant = 1; s->prevVariant = -1; gCurTask->main = Task_StarClose; }; Player_UpdateHomingPosition(star->spawnX, star->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);; }; }

static void Task_StarClose(void)
{
    { Sprite_Star *star = (void *)((gCurTask)->data); Sprite *s = &star->s; MapEntity *me = star->base.me; Vec2_32 pos; pos.x = ((int)((star->spawnX + 0) >> 8)); pos.y = ((int)((star->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;; { { Coll_Player_Projectile(s, pos.x, pos.y); } }; if (((pos.x) > gCamera.x + (426 + (256 / 2)) || (pos.x) < gCamera.x - (256 / 2) || (pos.y) > gCamera.y + ((240) + ((256) / 2)) || (pos.y) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 426 + 256) || (s->y + (256 / 2) < 0) || (s->y > 240 + (256 / 2)))) { { me->x = star->base.meX; }; TaskDestroy(gCurTask); return; }; }; if (--star->timer == 0) { star->timer = 60 * 2; s->graphics.anim = 511; s->variant = 2; s->prevVariant = -1; gCurTask->main = Task_StarSpin; }; Player_UpdateHomingPosition(star->spawnX, star->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);; };
}
