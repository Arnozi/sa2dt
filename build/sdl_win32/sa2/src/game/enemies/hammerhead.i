# 0 "src/game/enemies/hammerhead.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/hammerhead.c"
...
# 2 "src/game/enemies/hammerhead.c" 2
...
void VramFree(void *);
# 3 "src/game/enemies/hammerhead.c" 2
# 1 "include/task.h" 1
...
# 4 "src/game/enemies/hammerhead.c" 2
# 1 "include/trig.h" 1
...
# 5 "src/game/enemies/hammerhead.c" 2
...
# 12 "src/game/enemies/hammerhead.c" 2
...
void CreateEntity_Hammerhead(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Enemy_Hammerhead *hammerhead = (void *)((t)->data);
    ...
    s->graphics.dest = VramMalloc(36); s->graphics.anim = 499; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
}

static void Task_Hammerhead(void)
{
    Player *p = &gPlayer;
    Enemy_Hammerhead *hammerhead = (void *)((gCurTask)->data);
    Sprite *s = &hammerhead->s;
    ...
    posX = ((hammerhead->base.meX)*8 + (hammerhead->base.regionX)*256);
    posY = ((me->y)*8 + (hammerhead->base.regionY)*256);

    s->x = posX - gCamera.x;

    if ((!(((gGameMode == 0) || (gGameMode == 1) || (gGameMode == 2))))) {
        s->y = (posY - gCamera.y) + ((int)((hammerhead->unk50[2]) >> 8));
    } else {
        s->y = (posY - gCamera.y) + ((int)((prevUnk48) >> 8));
    }
    ...
    if (!(p->moveState & 0x00400000)) {
        s32 flags = Coll_Player_Platform(s, posX, posY + ((int)((hammerhead->unk48) >> 8)), p);
    ...
    if (Coll_Player_Enemy_Attack(s, posX, posY + ((int)((hammerhead->unk48) >> 8)), 1) == 1) {
        TaskDestroy(gCurTask);
    } else {
        posX -= gCamera.x;
        posY -= gCamera.y;

        if (((s->x < -((256 / 2))) || (s->x > 426 + ((256 / 2))) || ((s->y) + (256) < 0) || (s->y > 240 + (256)))
            && ((posX < -((256 / 2))) || (posX > 426 + ((256 / 2))) || ((posY) + (256) < 0) || (posY > 240 + (256)))) {
            { me->x = hammerhead->base.meX; };
            TaskDestroy(gCurTask);
        } else {
...
static void TaskDestructor_Hammerhead(struct Task *t)
{
    ...
    if ((gPlayer.moveState & 0x00000008) && (gPlayer.stoodObj == s)) {
        ...
        gPlayer.stoodObj = 
# 135 "src/game/enemies/hammerhead.c" 3 4
                          ((void *)0)
# 135 "src/game/enemies/hammerhead.c"
                              ;
    }
}
