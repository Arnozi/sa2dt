# 0 "src/game/enemies/gohla.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/gohla.c"
...
# 4 "src/game/enemies/gohla.c" 2



# 1 "include/malloc_vram.h" 1
# 8 "src/game/enemies/gohla.c" 2
...
void CreateEntity_Gohla(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    if (gGameMode == 1 || gDifficultyLevel != 1) {
        ...
        gohla->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256));
        gohla->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));
        gohla->offsetX = 0;
        gohla->offsetY = ((s32)((sub_801F07C(((int)((gohla->spawnY) >> 8)), ((int)((gohla->spawnX) >> 8)), gohla->unk8C, 8, 
# 60 "src/game/enemies/gohla.c" 3 4
                        ((void *)0)
# 60 "src/game/enemies/gohla.c"
                        , sub_801EE64))*256));
        gohla->projX = 0;
        gohla->projY = 0;
        gohla->projZ = 0;
        ...
        s->graphics.dest = VramMalloc(9); s->graphics.anim = 483; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;

        s = &gohla->proj;
        s->x = 0;
        s->y = 0;
        s->graphics.dest = VramMalloc(6); s->graphics.anim = 484; s->variant = 0; s->oamFlags = ((17) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
    }
}
...
static void sub_8051928(void)
{
    ...
    Sprite_Gohla *gohla = (void *)((gCurTask)->data);
    Sprite *s = &gohla->s;

    MapEntity *me = gohla->base.me;
    Vec2_32 pos;

    if ((((s)->frameFlags & (((1) << (10)))) >> ((10)))) {
        gohla->offsetX += 0x80;
    } else {
        gohla->offsetX -= 0x80;
    }

    { s32 delta = sub_801F07C(((int)((gohla->spawnY + gohla->offsetY) >> 8)), ((int)((gohla->spawnX + gohla->offsetX) >> 8)), gohla->unk8C, 8, 
# 96 "src/game/enemies/gohla.c" 3 4
   ((void *)0)
# 96 "src/game/enemies/gohla.c"
   , sub_801EE64); if (delta < 0) { gohla->offsetY += ((s32)((delta)*256)); delta = sub_801F100(((int)((gohla->spawnY + gohla->offsetY) >> 8)), ((int)((gohla->spawnX + gohla->offsetX) >> 8)), gohla->unk8C, 8, sub_801EC3C);; } if (delta > 0) { gohla->offsetY += ((s32)((delta)*256)); } };

    pos.x = ((int)((gohla->spawnX + gohla->offsetX) >> 8)); pos.y = ((int)((gohla->spawnY + gohla->offsetY) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;
    if (CheckPlayerHit(gohla, pos.x, pos.y)) {
        TaskDestroy(gCurTask);
        return;
    }

    if (((((int)((gohla->spawnX) >> 8))) > gCamera.x + (426 + (256 / 2)) || (((int)((gohla->spawnX) >> 8))) < gCamera.x - (256 / 2) || (((int)((gohla->spawnY) >> 8))) > gCamera.y + ((240) + ((256) / 2)) || (((int)((gohla->spawnY) >> 8))) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 426 + 256) || (s->y + (256 / 2) < 0) || (s->y > 240 + (256 / 2)))) { { me->x = gohla->base.meX; }; TaskDestroy(gCurTask); return; }; };
    if (((((int)((gohla->offsetX) >> 8)) <= me->d.sData[0] * 8)) || ((((int)((gohla->offsetX) >> 8)) >= (me->d.sData[0] * 8 + me->d.uData[2] * 8)))) {
...
static void sub_8051AF0(void)
{
    ...
    s32 delta = sub_801F100(((int)((gohla->spawnY + gohla->offsetY) >> 8)), ((int)((gohla->spawnX + gohla->offsetX) >> 8)), gohla->unk8C, 8, sub_801EC3C);;

    if (delta < 0) {
        gohla->offsetY += ((s32)((delta)*256));
        delta = sub_801F100(((int)((gohla->spawnY + gohla->offsetY) >> 8)), ((int)((gohla->spawnX + gohla->offsetX) >> 8)), gohla->unk8C, 8, sub_801EC3C);;
    }
...
static void RenderProjectiles(Sprite_Gohla *gohla, s16 x, s16 y, u32 flags)
{
    ...
    gohla->projY = (gohla->projY + 24) & (1024 - 1);
    gohla->projZ = (gohla->projZ + 32) & (1024 - 1);

    if (flags & ((1) << (10))) {
        (proj)->frameFlags |= (((1) << (10)));
    } else {
        (proj)->frameFlags &= ~(((1) << (10)));
    }
    ...
    for (i = 0; i < 4; i++) {
        ...
        proj->x = x + ((gSineTable[(pos[0]) + 256]) >> 10);
        proj->y = y + ((gSineTable[(pos[1])]) >> 13);
