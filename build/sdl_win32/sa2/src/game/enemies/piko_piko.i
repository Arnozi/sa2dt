# 0 "src/game/enemies/piko_piko.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/piko_piko.c"
...
# 2 "src/game/enemies/piko_piko.c" 2
...
void VramFree(void *);
# 3 "src/game/enemies/piko_piko.c" 2
...
# 5 "src/game/enemies/piko_piko.c" 2
...
# 7 "src/game/enemies/piko_piko.c" 2
...
# 8 "src/game/enemies/piko_piko.c" 2
...
# 10 "src/game/enemies/piko_piko.c" 2
...
# 12 "src/game/enemies/piko_piko.c" 2
...
void CreateEntity_PikoPiko(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    if ((gGameMode == 1 || gDifficultyLevel != 1)) {
        ...
        Sprite_PikoPiko *piko = (void *)((t)->data);
        Sprite *s = &piko->s;
        ...
        piko->speedX = -((s32)((1.0)*256));

        piko->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); piko->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));; piko->offsetX = 0; piko->offsetY = ((s32)((sub_801F07C(((int)((piko->spawnY) >> 8)), ((int)((piko->spawnX) >> 8)), piko->clampParam, 8, 
# 50 "src/game/enemies/piko_piko.c" 3 4
       ((void *)0)
# 50 "src/game/enemies/piko_piko.c"
       , sub_801EE64))*256));;
        ...
        s->graphics.dest = VramMalloc(30); s->graphics.anim = 494; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
    }
}

static void Task_PikoPiko(void)
{
    Sprite_PikoPiko *piko = (void *)((gCurTask)->data);
    Sprite *s = &piko->s;
    ...
    { s32 delta = sub_801F07C(((int)((piko->spawnY + piko->offsetY) >> 8)), ((int)((piko->spawnX + piko->offsetX) >> 8)), piko->clampParam, 8, 
# 69 "src/game/enemies/piko_piko.c" 3 4
   ((void *)0)
# 69 "src/game/enemies/piko_piko.c"
   , sub_801EE64); if (delta < 0) { piko->offsetY += ((s32)((delta)*256)); delta = sub_801F100(((int)((piko->spawnY + piko->offsetY) >> 8)), ((int)((piko->spawnX + piko->offsetX) >> 8)), piko->clampParam, 8, sub_801EC3C);; } if (delta > 0) { piko->offsetY += ((s32)((delta)*256)); } };
    pos.x = ((int)((piko->spawnX + piko->offsetX) >> 8)); pos.y = ((int)((piko->spawnY + piko->offsetY) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;

    if (!(Coll_Player_Enemy(s, pos.x, pos.y, 1, &gPlayer) == 1)) {
        if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0) == 1) { TaskDestroy(gCurTask); return; };
    }

    if (((((int)((piko->spawnX) >> 8))) > gCamera.x + (426 + (256 / 2)) || (((int)((piko->spawnX) >> 8))) < gCamera.x - (256 / 2) || (((int)((piko->spawnY) >> 8))) > gCamera.y + ((240) + ((256) / 2)) || (((int)((piko->spawnY) >> 8))) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 426 + 256) || (s->y + (256 / 2) < 0) || (s->y > 240 + (256 / 2)))) { { me->x = piko->base.meX; }; TaskDestroy(gCurTask); return; }; };

    if (((((int)((piko->offsetX) >> 8)) <= me->d.sData[0] * 8)) && (piko->speedX < 0)) {
        ({ s32 temp = piko->speedX; piko->speedX = -temp; });
    } else if (((((int)((piko->offsetX) >> 8)) >= (me->d.sData[0] * 8 + me->d.uData[2] * 8))) && (piko->speedX > 0)) {
        ({ s32 temp = piko->speedX; piko->speedX = -temp; });
    }
