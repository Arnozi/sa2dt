# 0 "src/game/enemies/kubinaga.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/kubinaga.c"
...
# 2 "src/game/enemies/kubinaga.c" 2
...
void VramFree(void *);
# 3 "src/game/enemies/kubinaga.c" 2
...
# 4 "src/game/enemies/kubinaga.c" 2
# 1 "include/trig.h" 1
...
# 5 "src/game/enemies/kubinaga.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 6 "src/game/enemies/kubinaga.c" 2
...
# 8 "src/game/enemies/kubinaga.c" 2
...
# 11 "src/game/enemies/kubinaga.c" 2

# 1 "include/game/enemies/projectiles.h" 1
...
# 13 "src/game/enemies/kubinaga.c" 2
...
# 17 "src/game/enemies/kubinaga.c" 2
# 1 "include/constants/songs.h" 1
# 18 "src/game/enemies/kubinaga.c" 2
...
void CreateEntity_Kubinaga(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    k->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); k->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));;
    s->x = ((me->x)*8 + (spriteRegionX)*256);
    s->y = ((me->y)*8 + (spriteRegionY)*256);

    s->graphics.dest = VramMalloc(6); s->graphics.anim = gUnknown_080D8F30[me->d.uData[0] & 1][0]; s->variant = gUnknown_080D8F30[me->d.uData[0] & 1][1]; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
    ...
    s->x = 0;
    s->y = 0;
    s->graphics.dest = VramMalloc(4); s->graphics.anim = 489; s->variant = 0; s->oamFlags = ((20) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
    UpdateSpriteAnimation(s);
...
static void sub_80524D0(void)
{
    Sprite_Kubinaga *k = (void *)((gCurTask)->data);
    Sprite *s = &k->sBase;
    MapEntity *me = k->base.me;
    Vec2_32 pos;

    pos.x = ((int)((k->spawnX + 0) >> 8)); pos.y = ((int)((k->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;
    if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; };
    if (((pos.x) > gCamera.x + (426 + (256 / 2)) || (pos.x) < gCamera.x - (256 / 2) || (pos.y) > gCamera.y + ((240) + ((256) / 2)) || (pos.y) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 426 + 256) || (s->y + (256 / 2) < 0) || (s->y > 240 + (256 / 2)))) { { me->x = k->base.meX; }; TaskDestroy(gCurTask); return; }; };
    ...
    if (k->unkB8 != 0) {
        ...
        k->unkBE = 0;
        if (k->unkC0 == 0) {
            k->unkBA = sub_8004418((((int)((gPlayer.qWorldY) >> 8)) - pos.y) + 10, ((int)((gPlayer.qWorldX) >> 8)) - pos.x);
        } else if ((k->unkC0 & 1)) {
            k->unkBA = sub_8004418((((int)((gPlayer.qWorldY) >> 8)) - pos.y), (((int)((gPlayer.qWorldX) >> 8)) - pos.x) + 10);
        } else {
            k->unkBA = sub_8004418((((int)((gPlayer.qWorldY) >> 8)) - pos.y), (((int)((gPlayer.qWorldX) >> 8)) - pos.x) - 10);
        }
...
static void sub_80526C8(void)
{
    ...
    pos.x = ((int)((k->spawnX) >> 8));
    pos.y = ((int)((k->spawnY) >> 8));
    pos2.x = ((int)((k->headX) >> 8));
    pos2.y = ((int)((k->headY) >> 8));
    ...
    if (Coll_Player_Enemy_Attack(sBase, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; };
    if (Coll_Player_Enemy_Attack(sHead, pos2.x, pos2.y, 0)) { TaskDestroy(gCurTask); return; };

    if (((pos.x) > gCamera.x + (426 + (256 / 2)) || (pos.x) < gCamera.x - (256 / 2) || (pos.y) > gCamera.y + ((240) + ((256) / 2)) || (pos.y) < gCamera.y - (256 / 2))) { if ((((u16)(sBase->x + (256 / 2)) > 426 + 256) || (sBase->y + (256 / 2) < 0) || (sBase->y > 240 + (256 / 2)))) { { me->x = k->base.meX; }; TaskDestroy(gCurTask); return; }; };
    ...
    if (sBase->variant == 0) {
        sBase->frameFlags ^= ((1) << (10));
        DisplaySprite(sBase);
    } else {
        sBase->frameFlags ^= ((1) << (11));
        DisplaySprite(sBase);
    }
...
static void sub_8052CC8(Sprite_Kubinaga *k)
{
    ...
    s32 cos = ((gSineTable[(k->unkBA) + 256]) * 7) >> 5;
    s32 sin = ((gSineTable[(k->unkBA)]) * 7) >> 5;
    Vec2_32 pos;

    u8 val = ({((0xE00) != 0) ? ((s32)(k->unkBE) / (s32)(0xE00)) : 0;});
    u8 i;

    for (i = 0; i < val; i++) {

        pos.x = ((int)(((i + 1) * cos + k->spawnX) >> 8));
        pos.y = ((int)(((i + 1) * sin + k->spawnY) >> 8));
    ...
    k->headX = (((gSineTable[(k->unkBA) + 256]) * ((int)((k->unkBE) >> 8))) >> 6) + k->spawnX;
    k->headY = (((gSineTable[(k->unkBA)]) * ((int)((k->unkBE) >> 8))) >> 6) + k->spawnY;
    ...
    if (k->unkC0 == 0) {
        ...
        transform->rotation = sub_8004418(((int)((gPlayer.qWorldY) >> 8)) - pos.y + 10, ((int)((gPlayer.qWorldX) >> 8)) - pos.x);
    } else {
        if (k->unkC0 & 1) {

            sHead->x = sHead->x + 10;
            transform->rotation = sub_8004418(((int)((gPlayer.qWorldY) >> 8)) - pos.y, (((int)((gPlayer.qWorldX) >> 8)) - pos.x) + 10);
        } else {
