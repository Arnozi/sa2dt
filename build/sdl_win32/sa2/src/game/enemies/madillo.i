# 0 "src/game/enemies/madillo.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/madillo.c"
...
# 2 "src/game/enemies/madillo.c" 2
...
void VramFree(void *);
# 3 "src/game/enemies/madillo.c" 2
...
# 4 "src/game/enemies/madillo.c" 2
...
# 5 "src/game/enemies/madillo.c" 2
...
# 6 "src/game/enemies/madillo.c" 2
...
# 11 "src/game/enemies/madillo.c" 2
...
void CreateEntity_Madillo(MapEntity *me, u16 spriteRegionX, u16 spriteRegionY, u8 spriteY)
{
    ...
    Sprite_Madillo *madillo = (void *)((t)->data);
    Sprite *s = &madillo->s;
    ...
    if (me->d.sData[1] != 0) {
        madillo->clampParam = 1;
    } else {
        madillo->clampParam = 0;
    }

    madillo->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); madillo->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));; madillo->offsetX = 0; madillo->offsetY = ((s32)((sub_801F07C(((int)((madillo->spawnY) >> 8)), ((int)((madillo->spawnX) >> 8)), madillo->clampParam, 8, 
# 45 "src/game/enemies/madillo.c" 3 4
   ((void *)0)
# 45 "src/game/enemies/madillo.c"
   , sub_801EE64))*256));;

    madillo->unk4C = -((s32)((1.5)*256));
    madillo->unk51 = 120;
    ...
    s->graphics.dest = VramMalloc(16); s->graphics.anim = 503; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
}

static void Task_MadilloMain(void)
{
    Sprite_Madillo *madillo = (void *)((gCurTask)->data);
    Sprite *s = &madillo->s;
    ...
    {
        s32 delta = sub_801F100(((int)((madillo->spawnY + madillo->offsetY) >> 8)), ((int)((madillo->spawnX + madillo->offsetX) >> 8)), madillo->clampParam, 8, sub_801EC3C);;
        if (delta < 0) {
            madillo->offsetY += ((s32)((delta)*256));
            delta = sub_801F100(((int)((madillo->spawnY + madillo->offsetY) >> 8)), ((int)((madillo->spawnX + madillo->offsetX) >> 8)), madillo->clampParam, 8, sub_801EC3C);;
        }
    ...
    pos.x = ((int)((madillo->spawnX + madillo->offsetX) >> 8));
    pos.y = ((int)((madillo->spawnY + madillo->offsetY) >> 8));
    s->x = pos.x - gCamera.x;
    s->y = pos.y - gCamera.y;

    if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 0)) { TaskDestroy(gCurTask); return; };
    if (((((int)((madillo->spawnX) >> 8))) > gCamera.x + (426 + (256 / 2)) || (((int)((madillo->spawnX) >> 8))) < gCamera.x - (256 / 2) || (((int)((madillo->spawnY) >> 8))) > gCamera.y + ((240) + ((256) / 2)) || (((int)((madillo->spawnY) >> 8))) < gCamera.y - (256 / 2))) { if ((((u16)(s->x + (256 / 2)) > 426 + 256) || (s->y + (256 / 2) < 0) || (s->y > 240 + (256 / 2)))) { { me->x = madillo->base.meX; }; TaskDestroy(gCurTask); return; }; };

    if (((s32)((pos.y - 50)*256)) < gPlayer.qWorldY) {
        if (((s32)((pos.y + 50)*256)) > gPlayer.qWorldY) {
            if (((((s32)((pos.x) << 8))) > gPlayer.qWorldX) && (((s32)((pos.x - 120)*256)) < (gPlayer.qWorldX))) {
                if (((int)((madillo->offsetX) >> 8)) > me->d.sData[0] * 8) {
            ...
            } else if (((s32)((pos.x + 120)*256)) > gPlayer.qWorldX) {
                if (((int)((madillo->offsetX) >> 8)) < (me->d.sData[0] + me->d.uData[2]) * 8) {
                    gCurTask->main = Task_8056230;
                    s->graphics.anim = 503;
                    s->variant = 1;
                    s->prevVariant = -1;
    ...
    Player_UpdateHomingPosition((((s32)((pos.x) << 8))), ((s32)((pos.y) << 8))); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);;
}
static void Task_8056230(void)
{
    Sprite_Madillo *madillo = (void *)((gCurTask)->data);
    ...
    { s32 delta = sub_801F07C(((int)((madillo->spawnY + madillo->offsetY) >> 8)), ((int)((madillo->spawnX + madillo->offsetX) >> 8)), madillo->clampParam, 8, &sp0[0], sub_801EE64); if (delta < 0) { madillo->offsetY += ((s32)((delta)*256)); delta = sub_801F100(((int)((madillo->spawnY + madillo->offsetY) >> 8)), ((int)((madillo->spawnX + madillo->offsetX) >> 8)), madillo->clampParam, 8, sub_801EC3C);; } if (delta > 0) { madillo->offsetY += ((s32)((delta)*256)); } };
    pos.x = ((int)((madillo->spawnX + madillo->offsetX) >> 8)); pos.y = ((int)((madillo->spawnY + madillo->offsetY) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;

    p = &gPlayer;
    s2 = &p->spriteInfoBody->s;

    if ((s2->hitboxes[0].index != -1)) {
        if ((((((pos.x) + s->hitboxes[0].left) <= ((((int)((p->qWorldX) >> 8))) + s2->hitboxes[0].left) && ((((pos.x)) + (s->hitboxes[0]).left) + ((s->hitboxes[0]).right - (s->hitboxes[0]).left)) >= ((((int)((p->qWorldX) >> 8))) + s2->hitboxes[0].left)) || (((pos.x) + s->hitboxes[0].left) >= ((((int)((p->qWorldX) >> 8))) + s2->hitboxes[0].left) && ((((((int)((p->qWorldX) >> 8)))) + (s2->hitboxes[0]).left) + ((s2->hitboxes[0]).right - (s2->hitboxes[0]).left)) >= ((pos.x) + s->hitboxes[0].left))) && (((((pos.y)) + s->hitboxes[0].top) <= (((((int)((p->qWorldY) >> 8)))) + s2->hitboxes[0].top) && ((((pos.y)) + s->hitboxes[0].top) + ((s->hitboxes[0]).bottom - (s->hitboxes[0]).top)) >= (((((int)((p->qWorldY) >> 8)))) + s2->hitboxes[0].top)) || ((((pos.y)) + s->hitboxes[0].top) >= (((((int)((p->qWorldY) >> 8)))) + s2->hitboxes[0].top) && ((((((int)((p->qWorldY) >> 8)))) + s2->hitboxes[0].top) + ((s2->hitboxes[0]).bottom - (s2->hitboxes[0]).top)) >= (((pos.y)) + s->hitboxes[0].top))))) {
            if ((p->itemEffect & 0x2) == 0x00) {
                Coll_DamagePlayer(p);
            }
        }
    }
    ...
    if ((s->frameFlags & ((1) << (10)) && ((int)((madillo->offsetX) >> 8)) > (me->d.sData[0] + me->d.uData[2]) * 8)
        || (!(s->frameFlags & ((1) << (10))) && ((int)((madillo->offsetX) >> 8)) < me->d.sData[0] * 8)) {
...
static void Task_80564BC(void)
{
    ...
    madillo->unk4C = ({((100) != 0) ? ((s32)(madillo->unk4C * 90) / (s32)(100)) : 0;});
