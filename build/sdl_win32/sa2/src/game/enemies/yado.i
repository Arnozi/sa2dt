# 0 "src/game/enemies/yado.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/enemies/yado.c"
...
# 14 "src/game/enemies/yado.c" 2
# 1 "include/constants/char_states.h" 1
# 15 "src/game/enemies/yado.c" 2
# 1 "include/constants/player_transitions.h" 1
# 16 "src/game/enemies/yado.c" 2
# 1 "include/constants/songs.h" 1
# 17 "src/game/enemies/yado.c" 2
...
# 63 "src/game/enemies/yado.c"
    ...
    yado->spawnX = ((s32)((((me->x)*8 + (spriteRegionX)*256))*256)); yado->spawnY = ((s32)((((me->y)*8 + (spriteRegionY)*256))*256));;
    ...
    s->graphics.dest = VramMalloc(12); s->graphics.anim = 497; s->variant = 0; s->oamFlags = ((18) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((2) << (12)) | (0));;;
}

void Task_YadoMain(void)
{
    Sprite_Yado *yado = (void *)((gCurTask)->data);
    Sprite *s = &yado->s;
    MapEntity *me = yado->base.me;
    Vec2_32 pos;

    pos.x = ((int)((yado->spawnX + 0) >> 8)); pos.y = ((int)((yado->spawnY + 0) >> 8)); s->x = pos.x - gCamera.x; s->y = pos.y - gCamera.y;;

    if (Coll_Player_Entity_HitboxN(s, pos.x, pos.y, 0, &gPlayer, 0) == 1) {
        m4aSongNumStart(153);
        gPlayer.qSpeedAirY = -((s32)((9.0)*256));
    ...
    if ((((u16)(s->x + (256 / 2)) > 426 + 256) || (s->y + (256 / 2) < 0) || (s->y > 240 + (256 / 2)))) { { me->x = yado->base.meX; }; TaskDestroy(gCurTask); return; };
    ...
    if (--yado->unk4C == 0) {
        ...
        s->variant = 1;
        s->prevVariant = -1;
    } else if ((((((int)((gPlayer.qWorldX) >> 8)) < pos.x) && (s->frameFlags & ((1) << (10)))) || ((((int)((gPlayer.qWorldX) >> 8)) > pos.x) && (~s->frameFlags & ((1) << (10)))))) {
    ...
    Player_UpdateHomingPosition(yado->spawnX, yado->spawnY); { {} }; UpdateSpriteAnimation(s); DisplaySprite(s);;
}
...
void Task_8055084(void)
{
    ...
    if (!(gPlayer.moveState & 0x00000002)) {
        if (Coll_Player_Enemy_Attack(s, pos.x, pos.y, 1) == 1) {
...
# 155 "src/game/enemies/yado.c"
    ...
    if (--yado->unk4C == 0) {
        ...
        if (s->frameFlags & ((1) << (10))) {
            pinit.x = ((s32)((pos.x + 6) << 8));
            pinit.rot = 0;
        } else {
            pinit.x = ((s32)((pos.x - 5) << 8));
            pinit.rot = ((s32)((2.0)*256));
        }
        pinit.y = ((s32)((pos.y - 6) << 8));
        pinit.speed = ((s32)((1.5)*256));
