...
void CreateMultiplayerFinishResult(u8 sioId, u8 count)
{
...

    {
        ...
        for (; i < (sizeof(gMultiplayerPlayerTasks) / sizeof((gMultiplayerPlayerTasks)[0])); i++) {
            if (*mpt == 
# 108 "src/game/multiplayer/finish.c" 3 4
                       ((void *)0)
# 108 "src/game/multiplayer/finish.c"
                           )
                break;
            else
                mpt++;
...
# 219 "src/game/multiplayer/finish.c"
    {
        struct Task *mpt;
        struct Task *t = TaskCreate(Task_801A04C, sizeof(Finish2), 0x2000, 0, 
# 221 "src/game/multiplayer/finish.c" 3 4
                                                                                        ((void *)0)
# 221 "src/game/multiplayer/finish.c"
                                                                                            );
...
# 362 "src/game/multiplayer/finish.c" 3 4
                                                 ((void *)0)
# 362 "src/game/multiplayer/finish.c"
                                                     ) {
                    break;
                }
        ...
        for (i = 0; i < 4; i++) {
            if (gMultiplayerPlayerTasks[i] == 
# 383 "src/game/multiplayer/finish.c" 3 4
                                             ((void *)0)
# 383 "src/game/multiplayer/finish.c"
                                                 ) {
                break;
            }
    ...
    if (f2->unk0 >= 0x1000) {
...

        {
            ...
            for (pid = 0; pid < 4; pid++) {
                ...
                if (pid == ((volatile struct SioMultiCnt *)(REG_BASE + 0x128))->id)
                    continue;

                if (gMultiplayerRanks[((volatile struct SioMultiCnt *)(REG_BASE + 0x128))->id] < gMultiplayerRanks[pid]) {
                    foeResult = 0;
                } else if (gMultiplayerRanks[((volatile struct SioMultiCnt *)(REG_BASE + 0x128))->id] > gMultiplayerRanks[pid]) {
                    foeResult = 1;
                    ownResult = 1;
                } else {
                    foeResult = 2;
