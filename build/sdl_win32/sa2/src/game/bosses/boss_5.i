# 0 "src/game/bosses/boss_5.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/game/bosses/boss_5.c"
# 1 "include/core.h" 1
...
# 2 "src/game/bosses/boss_5.c" 2
# 1 "include/sprite.h" 1
# 3 "src/game/bosses/boss_5.c" 2
# 1 "include/task.h" 1
# 4 "src/game/bosses/boss_5.c" 2
# 1 "include/malloc_vram.h" 1
# 5 "src/game/bosses/boss_5.c" 2
# 1 "include/trig.h" 1
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 24 "src/game/bosses/boss_5.c" 2
# 1 "include/constants/songs.h" 1
# 25 "src/game/bosses/boss_5.c" 2
...
# 27 "src/game/bosses/boss_5.c" 2
# 1 "include/flags.h" 1
# 28 "src/game/bosses/boss_5.c" 2
...
typedef struct {
               ...
               s32 unkDC[6][2];
                ...
                ScreenFade screenTransition;

                Sprite disk;
                Sprite cabin;

                Hitbox cabinReserved[2];
                ...
                Sprite smackParticles[8];

                void *vram;
} EggSaucer;


const u16 gUnknown_080D7F94[2] = { 4 * 60, 8 * 60 };
...
static const u16 gUnknown_080D7FF0[][16u] = {
...
void CreateEggSaucer(void)
{
    ...
    fade->flags = 2;
    fade->speed = 0;
    fade->bldCnt = ((1 << 13) | (1 << 12) | (3 << 6) | (1 << 5) | (1 << 0) | (1 << 1)
                    | (1 << 2) | (1 << 3));
    fade->bldAlpha = 0;
...
# 252 "src/game/bosses/boss_5.c"
    ...
    boss->cabinDiskAngle = ((int)((((double)(120)) / 360.0) * 1024));
    boss->gunDiskAngle = ((int)((((double)(0)) / 360.0) * 1024));
    boss->armDiskAngle = ((int)((((double)(240)) / 360.0) * 1024));

    boss->gunAngle = ((int)((((double)(180)) / 360.0) * 1024));
    ...
    s->graphics.anim = 685; s->variant = 0; s->oamFlags = ((31) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((3) << (12)) | (((1) << (18))));;
    ...
    s->oamFlags = ((20) << 6); s->graphics.size = 0; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;; s->frameFlags = (((1) << (12)) | (0));;
    ...
    s->graphics.anim = 669; s->variant = 0; s->oamFlags = ((21) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((1) << (12)) | (0));;
    ...
    s->graphics.dest = vram;
    vram += 25 * 32;
    s->graphics.anim = 671; s->variant = 0; s->oamFlags = ((22) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((1) << (12)) | (0));;
    ...
    vram += 64 * 32;
    s->graphics.anim = 682; s->variant = 0; s->oamFlags = ((23) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;;
    s->frameFlags = gOamMatrixIndex++ | ((1) << (12)) | ((1) << (5)) | ((1) << (6));
    ...
    s->graphics.dest = vram;
    vram += 4 * 32;
    s->graphics.anim = 683; s->variant = 0; s->oamFlags = ((19) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((1) << (12)) | (0));;
    ...
    s->graphics.dest = vram;
    vram += 20 * 32;
    s->graphics.anim = 670; s->variant = 0; s->oamFlags = ((20) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((1) << (12)) | (0));;
    UpdateSpriteAnimation(s);
    ...
    s->graphics.dest = vram;
    vram += 4 * 32;
    s->graphics.anim = 681; s->variant = 0; s->oamFlags = ((19) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((1) << (12)) | (0));;
    UpdateSpriteAnimation(s);
    ...
    s->y = 0;
    s->graphics.dest = vram;
    s->graphics.anim = 672; s->variant = 0; s->oamFlags = ((12) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;;
    ...
    for (i = 0; i < 8; i++) {
        ...
        vramBase += gUnknown_080D7FB0[i].numTiles * 32;

        s->graphics.anim = gUnknown_080D7FB0[i].anim; s->variant = gUnknown_080D7FB0[i].variant; s->oamFlags = ((11) << 6); s->graphics.size = 0;; s->animCursor = 0; s->qAnimDelay = 0; s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;; s->frameFlags = (((1) << (12)) | (0));;
    }
}

void Task_EggSaucerIntro(void)
{
    EggSaucer *boss = (void *)((gCurTask)->data);
...
void sub_80435BC(void)
{
    ...
    if ((gCurrentLevel != ((7)*(3 + 1)) + (0) && gSelectedCharacter == CHARACTER_SONIC)
        && gLoadedSaveGame->unlockedLevels[CHARACTER_SONIC] <= gCurrentLevel) {
        if (boss->unk15 == 0 && ((int)((boss->cabinX) >> 8)) - gCamera.x < 50) {
            boss->unk15 = 1;
        }
    } else {
        if (boss->unk15 == 0 && ((int)((boss->cabinX) >> 8)) - gCamera.x < 50) {
            boss->unk15 = 1;
            CreateEggmobileEscapeSequence(((int)((boss->cabinX) >> 8)) - gCamera.x, ((int)((boss->cabinY) >> 8)) - gCamera.y, ((2) << (12)));
        }
    }
    sub_8045898(boss);

    if (((int)((boss->cabinX) >> 8)) - gCamera.x < -200 && boss->unk15 != 0) {
...
void sub_80436E4(EggSaucer *boss)
{
    ...
    s32 x2, y2;
    boss->cabinX = x + (((gSineTable[(boss->cabinDiskAngle) + 256]) * 5) >> 3);
    boss->cabinY = y + (((gSineTable[(boss->cabinDiskAngle)]) * 5) >> 3);

    boss->unkD4 = ((s16)((5.5)*256));
    boss->unkD6 = -((s16)((4.5)*256));

    x += (((gSineTable[(boss->armDiskAngle) + 256]) * 5) >> 3);
    y += (((gSineTable[(boss->armDiskAngle)]) * 5) >> 3);

    boss->armBaseX = x;
    boss->armBaseY = y;

    boss->unkD8 = ((s16)((4.75)*256));
    boss->unkDA = -((s16)((4)*256));
    ...
    for (i = 0; i < 6; i++) {
        ...
        x2 += ((s32)((boss->unk36[0][idx])*256));
        y2 += ((s32)((boss->unk36[1][idx])*256));

        boss->unkDC[i][0] = x + (x2 >> 3);
        boss->unkDC[i][1] = y + (y2 >> 3);

        boss->unk10C[i][0] = ((s16)((5.25)*256)) - (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 255)
        ...
        boss->unk10C[i][1] = -((s16)((4)*256)) - (i * ((s16)((0.25)*256)));
    }

    idx = (boss->unkB6 + 229) & 31;
    x2 += boss->unk36[0][idx] * ((s32)((0.5)*256));
    y2 += boss->unk36[1][idx] * ((s32)((0.5)*256));
    boss->handX = x + ((x2) >> 3);
    boss->handY = y + ((y2) >> 3);

    boss->unk12C = ((s16)((5)*256));
    boss->unk12E = -((s16)((5.75)*256));
}
...
void sub_80438C4(EggSaucer *boss)
{
    ...
    s = &boss->cabin;
    s->x = x + (((gSineTable[(boss->cabinDiskAngle) + 256]) * 5) >> 11);
    s->y = y + (((gSineTable[(boss->cabinDiskAngle)]) * 5) >> 11);
    UpdateSpriteAnimation(s);
    DisplaySprite(s);
    ...
    if (boss->gunHealth > 0) {
        s = &boss->gunBase;
        s->x = x + (((gSineTable[(boss->gunDiskAngle) + 256]) * 5) >> 11);
        s->y = y + (((gSineTable[(boss->gunDiskAngle)]) * 5) >> 11);
        UpdateSpriteAnimation(s);
        DisplaySprite(s);

        s = &boss->gun;
        transform = &boss->transform;
        s->x = x + (((gSineTable[(boss->gunDiskAngle) + 256]) * 5) >> 11) - 2;
        s->y = y + (((gSineTable[(boss->gunDiskAngle)]) * 5) >> 11) - 19;
        ...
        if ((!(gPlayer.moveState & 0x00000080)) && (boss->unk1C > 0 && boss->unk1C < 12)) {
            s = &boss->gunCharge;
            UpdateSpriteAnimation(s);
            DisplaySprite(s);
        }
    ...
    s = &boss->armBase;
    x += ((gSineTable[(boss->armDiskAngle) + 256]) * 5) >> 11;
    y += ((gSineTable[(boss->armDiskAngle)]) * 5) >> 11;
...
void sub_8043BEC(EggSaucer *boss)
{
    ...
    s = &boss->cabin;
    s->x = ((int)((boss->cabinX) >> 8)) - gCamera.x;
    s->y = ((int)((boss->cabinY) >> 8)) - gCamera.y;
    UpdateSpriteAnimation(s);
    DisplaySprite(s);

    if (boss->unk15 == 0
        || (gCurrentLevel != ((7)*(3 + 1)) + (0) && gSelectedCharacter == CHARACTER_SONIC
            && gLoadedSaveGame->unlockedLevels[CHARACTER_SONIC] <= gCurrentLevel)) {
        s = &boss->pilot;
        s->x = ((int)((boss->cabinX) >> 8)) - gCamera.x;
        s->y = ((int)((boss->cabinY) >> 8)) - gCamera.y;
    ...
    s = &boss->armBase;
    s->x = ((int)((boss->armBaseX) >> 8)) - gCamera.x;
    s->y = ((int)((boss->armBaseY) >> 8)) - gCamera.y;
    DisplaySprite(s);

    s = &boss->armSegment;
    for (i = 0; i < 6; i++) {
        s->x = ((int)((boss->unkDC[i][0]) >> 8)) - gCamera.x;
        s->y = ((int)((boss->unkDC[i][1]) >> 8)) - gCamera.y;
        s->oamFlags = ((19 - i) << 6);
        DisplaySprite(s);
    }

    s = &boss->hand;
    s->x = ((int)((boss->handX) >> 8)) - gCamera.x;
    s->y = ((int)((boss->handY) >> 8)) - gCamera.y;
    UpdateSpriteAnimation(s);
    DisplaySprite(s);
}
...
void sub_8043D94(EggSaucer *boss, s32 x, s32 y)
{
    ...
    temp = (((gSineTable[(boss->unkC0 * 0x10) + 256]) * 0xF) >> 0xD) + 0x10;

    for (i = 0; i < 8; i++) {
        s = &boss->smackParticles[i];
        s->x = ((temp * (gSineTable[(i * 128) + 256])) >> 14) + x;
        s->y = ((temp * (gSineTable[(i * 128)])) >> 14) + y;
...
void sub_8043E2C(EggSaucer *boss)
{
    ...
    if (boss->gunHealth == 0 && boss->unk20 == 0) {
        ...
        bottomX = ((int)((x) >> 8));
        bottomY = ((int)((y) >> 8)) + 10;
        ground = sub_801E4E4(bottomY, bottomX, 1, 8, 
# 745 "src/game/bosses/boss_5.c" 3 4
                                                    ((void *)0)
# 745 "src/game/bosses/boss_5.c"
                                                        , sub_801EE64);
        if (ground < 0) {
            boss->unk2C += ((s32)((ground)*256));
            boss->unk22 = ({((100) != 0) ? ((s32)(-(boss->unk22 * 80)) / (s32)(100)) : 0;});
            boss->unk20 = 1;
        }
        ...
        s = &boss->gun;
        transform = &boss->transform;
        s->x = (((int)((x) >> 8)) - gCamera.x) - 2;
        s->y = (((int)((y) >> 8)) - gCamera.y) - 19;
        s->frameFlags = gOamMatrixIndex++ | 0x1060;
        ...
        if (({((3) != 0) ? ((s32)(gStageTime) % (s32)(3)) : 0;}) == 0) {
            u32 rand;

            rand = ({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; });
            explosion.spawnX = (((int)((x) >> 8)) - gCamera.x) + (rand & 15) - 8;
            rand = ({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; });
            explosion.spawnY = (((int)((y) >> 8)) - gCamera.y) + (rand & 15) - 8;
            explosion.velocity = 0;

            explosion.rotation = ({ 1055 - (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 63); });
            explosion.speed = ((s16)((5)*256));
...
{
    u8 i;
    EggSaucer *boss = (void *)((gActiveBossTask)->data);
...
void HandleCollision(EggSaucer *boss)
{
    ...
    x = ((int)((boss->x) >> 8)) + (((gSineTable[(boss->cabinDiskAngle) + 256]) * 5) >> 11);
    y = ((int)((boss->y) >> 8)) + (((gSineTable[(boss->cabinDiskAngle)]) * 5) >> 11);
    ...
    Player_UpdateHomingPosition(((s32)((x) << 8)), ((s32)((y) << 8)));

    if (boss->unk13 == 0 && Coll_Cheese_Enemy_Attack(s, x, y, 0, &gPlayer) == 1) {
        sub_8045368(boss);
    ...
    if (boss->gunHealth != 0 && boss->unk1F == 0) {
        s = &boss->gunBase;
        x = ((int)((boss->x) >> 8)) + (((gSineTable[(boss->gunDiskAngle) + 256]) * 5) >> 11);
        y = ((int)((boss->y) >> 8)) + (((gSineTable[(boss->gunDiskAngle)]) * 5) >> 11);
        ...
        if (Coll_Player_Boss_Attack(s, x, y, 0, &gPlayer) == 1) {
            ...
            if (boss->gunHealth == 0) {
                { s32 divResA, divResB; s32 oldScore = gLevelScore; gLevelScore += 500; divResA = ({((50000) != 0) ? ((s32)(gLevelScore) / (s32)(50000)) : 0;}); divResB = ({((50000) != 0) ? ((s32)(oldScore) / (s32)(50000)) : 0;}); if ((divResA != divResB) && (gGameMode == 0)) { u16 lives = divResA - divResB; lives += gNumLives;; ({ if ((lives) > 255) (lives) = 255; gNumLives = (lives); }); if (1) gMusicManagerState.unk3 = 0x10 | 0x0; } };
            }
        } else {
            if (Coll_Player_Enemy(s, x, y, 0, &gPlayer) == 1) {
                sub_80452F8(boss);
        ...
        boss->unk28 = ((s32)((x - 5) << 8));
        boss->unk2C = ((s32)((y + 1) << 8));
    }

    s = &boss->armBase;
    x = ((int)((boss->x) >> 8)) + (((gSineTable[(boss->armDiskAngle) + 256]) * 5) >> 11);
    y = ((int)((boss->y) >> 8)) + (((gSineTable[(boss->armDiskAngle)]) * 5) >> 11);
...
static void SaucerUpdatePosAndRotate(EggSaucer *boss)
{
    ...
    bottomX = ((int)((x) >> 8));
    bottomY = ((int)((y) >> 8)) + (100 / 2);

    boss->y += ((s32)((sub_801E4E4(bottomY, bottomX, 1, 8, 0, sub_801EE64))*256));

    boss->cabinDiskAngle = ({ (boss->cabinDiskAngle + ((int)((((double)(0.7038123167155426)) / 360.0) * 1024))) & (1024 - 1); });
    boss->gunDiskAngle = ({ (boss->gunDiskAngle + ((int)((((double)(0.7038123167155426)) / 360.0) * 1024))) & (1024 - 1); });
    boss->armDiskAngle = ({ (boss->armDiskAngle + ((int)((((double)(0.7038123167155426)) / 360.0) * 1024))) & (1024 - 1); });
}
...
void sub_8044540(EggSaucer *boss)
{
    ...
    bottomX = ((int)((boss->x) >> 8));
    bottomY = ((int)((boss->y) >> 8)) + (100 / 2);
    ...
    bottomX = ((int)((boss->cabinX) >> 8));
    bottomY = ((int)((boss->cabinY) >> 8));
    res = sub_801E4E4(bottomY, bottomX, 1, 8, 0, sub_801EE64);
    if (res < 0) {
        boss->cabinY += ((s32)((res) << 8));
        boss->unkD6 = ({((100) != 0) ? ((s32)(-0x46 * boss->unkD6) / (s32)(100)) : 0;});
        boss->unkD4 -= 24;
    }
    ...
    bottomX = ((int)((boss->armBaseX) >> 8));
    bottomY = ((int)((boss->armBaseY) >> 8));
    res = sub_801E4E4(bottomY, bottomX, 1, 8, 0, sub_801EE64);
    if (res < 0) {
        boss->armBaseY += ((s32)((res) << 8));
        boss->unkDA = ({((100) != 0) ? ((s32)(-0x41 * boss->unkDA) / (s32)(100)) : 0;});
        boss->unkD8 -= 34;
    }
    ...
    for (i = 0; i < 6; i++) {
        ...
        bottomX = ((int)((boss->unkDC[i][0]) >> 8));
        bottomY = ((int)((boss->unkDC[i][1]) >> 8));
        res = sub_801E4E4(bottomY, bottomX, 1, 8, 0, sub_801EE64);
        if (res < 0) {
            boss->unkDC[i][1] += ((s32)((res) << 8));
            boss->unk10C[i][1] = ({((100) != 0) ? ((s32)(boss->unk10C[i][1] * (i - 0x41)) / (s32)(100)) : 0;});
    ...
    bottomX = ((int)((boss->handX) >> 8));
    bottomY = ((int)((boss->handY) >> 8));
    res = sub_801E4E4(bottomY, bottomX, 1, 8, 0, sub_801EE64);
    if (res < 0) {
        boss->handY += ((s32)((res) << 8));
        boss->unk12E = ({((100) != 0) ? ((s32)(-60 * boss->unk12E) / (s32)(100)) : 0;});
        boss->unk12C -= 52;
    }
}
...
void sub_8044784(EggSaucer *boss)
{
    ...
    if (!(!(gPlayer.moveState & 0x00000080))) {
        if (fade->brightness != ((s32)((32)*256))) {
            fade->brightness = ((s32)((32)*256));
    ...
    x = (((int)((boss->x) >> 8)) - gCamera.x);
    x += (((gSineTable[(boss->gunDiskAngle) + 256]) * 5) >> 11);
    x += (((gSineTable[(boss->gunAngle) + 256]) * 3) >> 11);

    y = (((int)((boss->y) >> 8)) - gCamera.y);
    y += ((gSineTable[(boss->gunDiskAngle)]) * 5) >> 11;
    y -= 18;
    y += ((gSineTable[(boss->gunAngle)]) * 3) >> 11;

    someBool = 0;
    if (boss->chargeBeamWidth != 0 && boss->unk1C == 0) {
        fade->brightness = ((s32)((32)*256)) - ((s16)((--boss->chargeBeamWidth)*256));
        ...
        if (boss->unk1C == 0) {
            ...
            fade->brightness = ((s32)((32)*256)) - ((tmp - 11) * ((s32)((0.5)*256)));
    ...
    x = ({
        s32 temp3;
        x = (((int)((gPlayer.qWorldX) >> 8)));
        temp3 = x + 2;
        temp3 - (((int)((boss->x) >> 8)) + (((gSineTable[(boss->gunDiskAngle) + 256]) * 5) >> 11));
    });
    temp = sub_8004418(y, x);
    if (gStageTime & 1) {
        if (boss->gunAngle < temp) {
            if (boss->gunAngle < ((int)((((double)(202.5)) / 360.0) * 1024))) {
                boss->gunAngle += 1;
            }
        } else {
            if (boss->gunAngle > ((int)((((double)(157.5)) / 360.0) * 1024))) {
                boss->gunAngle -= 1;
            }
        }
    }
    if (someBool && (boss->gunAngle - ((int)((((double)(5.625)) / 360.0) * 1024))) < temp && (boss->gunAngle + ((int)((((double)(5.625)) / 360.0) * 1024))) > temp) {
...
static void sub_8044A14(EggSaucer *boss)
{
    ...
    s32 val;
    s32 temp;
    temp = (gSineTable[((gStageTime << 4) & (1024 - 1))]);
    boss->unk32 = ({ s16 clamped = (((temp) >> 9) + 1000) & (1024 - 1); clamped; });
    boss->unk36[0][boss->unkB6] = boss->unkB8;
    boss->unk36[1][boss->unkB6] = boss->unkBA;

    val = ((gSineTable[(boss->unk32) + 256]) * 5) >> 11;
    ...
    val = ((gSineTable[(boss->unk32)]) * 5) >> 11;
    if (boss->unkBA < val) {
        boss->unkBA += 2;
        if (boss->unkBA > val) {
            boss->unkBA = val;
...
static void sub_8044B28(EggSaucer *boss)
{
    ...
    val = ((gSineTable[(boss->unk32) + 256]) * 5) >> 10;
    ...
    val = ((gSineTable[(boss->unk32)]) * 5) >> 10;
    if (boss->unkBA < val) {
        boss->unkBA += 2;
        if (boss->unkBA > val) {
            boss->unkBA = val;
    ...
    if (++boss->unkBC >= 0x50) {
        if (boss->gunHealth != 0 || (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 1)) {
            boss->unkBC = 0;
...
static void sub_8044CBC(EggSaucer *boss)
{
    ...
    if (boss->armStateTimer == 64) {
        ...
        x = ((int)((gPlayer.qWorldX) >> 8)) - x;
        y = ((int)((gPlayer.qWorldY) >> 8)) - y;
...
static void sub_8044EB0(EggSaucer *boss)
{
    ...
    index = ({ s16 clamped = (boss->armStateTimer << 4) & (1024 - 1); clamped; });
    temp = (gSineTable[(index)]);
    boss->unk32 = ({ s16 clamped = (((temp * 5) >> 11) + boss->unk32) & (1024 - 1); clamped; });
    boss->unk36[0][boss->unkB6] = boss->unkB8;
    boss->unk36[1][boss->unkB6] = boss->unkBA;
...
static void sub_8044FE4(EggSaucer *boss)
{
    ...
    val = ((gSineTable[(boss->unk32) + 256]) * 5) >> 9;
...
static void sub_80451C4(EggSaucer *boss)
{
    ...
    val = (((gSineTable[(boss->unk32) + 256]) * 0x13) - (boss->armStateTimer >> 4)) >> 11;
    ...
    val = (((gSineTable[(boss->unk32)]) * 0x13) - (boss->armStateTimer >> 4)) >> 11;
    if (boss->unkBA < val) {
        boss->unkBA += 10;
        if (boss->unkBA > val) {
            boss->unkBA = val;
...
void sub_8045564(EggSaucer *boss)
{
    ...
    if (boss->unk15 == 0) {
        ...
        if (({((7) != 0) ? ((s32)(gStageTime) % (s32)(7)) : 0;}) == 0) {
            rand = ({ (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 0x1f) + 0x18; }) + 0;
            index = ({ s16 clamped = (gStageTime * 900) & (1024 - 1); clamped; }) + 0;
            e.spawnX = (((int)((boss->x) >> 8)) - gCamera.x);
            e.spawnX += ((rand * (gSineTable[(index) + 256])) >> 14);
            e.spawnY = (((int)((boss->y) >> 8)) - gCamera.y);
            e.spawnY += ((rand * (gSineTable[(index)])) >> 14);
            e.velocity = 0;
            e.rotation = ({ 0x407 - (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 0x3F); }) + 0;
            e.speed = 0x500;
            e.vram = (void *)&VRAM[0x10000] + 0x2980;
            e.anim = 619;
            e.variant = 0;
            e.unk4 = 0;
        ...
        if (({((10) != 0) ? ((s32)(gStageTime) % (s32)(10)) : 0;}) == 0) {
            u8 val2;
            rand = ({ (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 0x1f) + 0x18; }) + 0;
            index = ({ s16 clamped = (gStageTime * 900) & (1024 - 1); clamped; }) + 0;
            val2 = ({((6) != 0) ? ((s32)(gStageTime) % (s32)(6)) : 0;});
            e.spawnX = (((int)((boss->x) >> 8)) - gCamera.x);
            e.spawnX += ((rand * (gSineTable[(index) + 256])) >> 14);
            e.spawnY = (((int)((boss->y) >> 8)) - gCamera.y);
            e.spawnY += ((rand * (gSineTable[(index)])) >> 14);
            e.velocity = 0x40;
            e.rotation = ({ (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 0x3FF); }) + 0;
            e.speed = 0x600;
            e.vram = (void *)&VRAM[0x10000] + (gTileInfoBossScrews[val2][0] * 0x20);
        ...
        if (({((9) != 0) ? ((s32)(gStageTime) % (s32)(9)) : 0;}) == 0) {
            u32 r4;
            rand = ({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 0xf;

            e.spawnX = (((int)((boss->cabinX) >> 8)) - gCamera.x);
            e.spawnX += rand & 0xf;
            e.spawnX -= 8;
            rand = ({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) + 0;
            e.spawnY = (((int)((boss->cabinY) >> 8)) - gCamera.y);
            e.spawnY += (rand & 0xf);
            e.spawnY -= 8;
            ...
            e.rotation = ({ 0x407 - (({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 63); }) + 0;
            e.speed = 0x500;
            e.vram = (void *)&VRAM[0x10000] + 0x2980;
            e.anim = 619;
            e.variant = 0;
        ...
        if (({((0xB) != 0) ? ((s32)(gStageTime) % (s32)(0xB)) : 0;}) == 0) {
            u32 r3;
            rand = ({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) & 0xF;
            e.spawnX = (((int)((boss->armBaseX) >> 8)) - gCamera.x);
            e.spawnX += rand & new_var;
            e.spawnX -= 8;
            rand = ({ gPseudoRandom = (gPseudoRandom * 0x196225) + 0x3C6EF35F; gPseudoRandom; }) + 0;
            e.spawnY = (((int)((boss->armBaseY) >> 8)) - gCamera.y);
...
void sub_8045898(EggSaucer *boss)
{
    ...
    if (boss->unk15 == 0) {
        ...
        if (boss->unk13 != 0) {
            for (i = 0; i < 16u; i++) {
                gObjPalette[(8)*16u + (i)] = (gUnknown_080D7FF0[val][i]);;
            }
        } else {
            for (i = 0; i < 16u; i++) {
                gObjPalette[(8)*16u + (i)] = (gUnknown_080D7FF0[1][i]);;
            }
        }
        ...
        if (boss->unk1F != 0) {
            boss->unk1F--;
            for (i = 0; i < 16u; i++) {
                gObjPalette[(9)*16u + (i)] = (gUnknown_080D7FF0[val][i]);;
            }
        } else {
            for (i = 0; i < 16u; i++) {
                gObjPalette[(9)*16u + (i)] = (gUnknown_080D7FF0[1][i]);;
            }
        }
...
{
    if (boss->gunHealth == 0 && boss->armDiskAngle != ((boss->cabinDiskAngle + 0x200) & (1024 - 1))) {
        boss->armDiskAngle = ({ s16 clamped = (boss->armDiskAngle + 1) & (1024 - 1); clamped; });
    }
}

void TaskDestructor_EggSaucerMain(struct Task *t)
{
    EggSaucer *boss = (void *)((t)->data);
    VramFree(boss->vram);
    gActiveBossTask = 
# 1797 "src/game/bosses/boss_5.c" 3 4
                     ((void *)0)
# 1797 "src/game/bosses/boss_5.c"
                         ;
}
