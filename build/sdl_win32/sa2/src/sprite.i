...
__attribute__((unused)) void DisplaySprites(Sprite *sprite, Vec2_16 *positions, u8 numPositions)
{
    ...
    if (sprite->dimensions != (void *)-1) {
        ...
        if (x + sprWidth >= 0 && x <= 426 && y + sprHeight >= 0 && y <= 240) {
...
static AnimCmdResult animCmd_GetPalette(void *cursor, Sprite *s)
{
    ...
    if (!(s->frameFlags & ((1) << (18)))) {
        ...
        DmaSet(3, &gRefSpriteTables->palettes[paletteIndex * 16u], &gObjPalette[(s->palId)*16u + (cmd->insertOffset)], (u32)((0x8000 | 0x0000 | 0x0400 | 0x0000 | 0x0000) << 16 | ((cmd->numColors * 2)/(32/8))))
                                     ;
